...
function parseArgs(argv) {
  const args = {dir: '', out: '', pretty: false};
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === '--dir' || a === '--image-dir') args.dir = String(argv[++i] || '').trim();
      ...
      console.log('Usage:\n  node scripts/link_ops_plan_image_roles.mjs --dir <图片文件夹> [--out roles.json] [--pretty]');
      process.exit(0);
...
const plan = await planLinkOpsImageRoles({dir: args.dir});
if (args.out) {
