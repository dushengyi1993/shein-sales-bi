...
assert_effective_systemd_controls() {
  ...
  show_output="$({
    "$SYSTEMCTL_BIN" show "${installed_services[@]}" --no-pager \
      --property=Id,LoadState,ActiveState,SubState,UnitFileState,Result,StateChangeTimestamp,ActiveEnterTimestamp,ExecMainCode,ExecMainStatus,ExecMainStartTimestamp,ExecMainExitTimestamp,NRestarts,ExecCondition,ExecStartPre,RequiresMountsFor,BindPaths,BindReadOnlyPaths,ReadOnlyPaths,InaccessiblePaths
