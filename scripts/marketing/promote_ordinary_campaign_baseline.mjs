...
function parseArgs(argv) {
  const args = {
    ...
    selectionOut: '',
    pricesOut: '',
    selectionOutExplicit: false,
    pricesOutExplicit: false,
    readback: '',
    batch: '',
    registryFile: '',
    registryRoot: '',
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (key === '--approval-manifest') args.approvalManifest = path.resolve(nextValue(argv, i++, key));
    else if (key === '--selection-out') {
      args.selectionOut = path.resolve(nextValue(argv, i++, key));
      args.selectionOutExplicit = true;
    } else if (key === '--prices-out') {
      args.pricesOut = path.resolve(nextValue(argv, i++, key));
      args.pricesOutExplicit = true;
    }
  ...
  if (args.registryFile && (args.selectionOutExplicit || args.pricesOutExplicit)) {
    throw new Error('--selection-out/--prices-out are incompatible with registry mode; immutable registry paths are the sole promoted outputs');
  }
  args.selectionOut ||= args.selection;
  args.pricesOut ||= args.prices;
  if (samePath(args.selectionOut, args.pricesOut)) {
    throw new Error('--selection-out and --prices-out must be different files');
  }
  return args;
}
...
function deriveBaselineId(batch, workFingerprint) {
  const safeBatch = String(batch || '')
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 48) || 'batch';
  return `ordinary-${safeBatch}-${workFingerprint}`;
}
...
async function main() {
  ...
  for (const doc of [selection, prices]) {
    ...
    const planMetadata = {
      ...
      selectionPayloadHash: approval.selectionPayloadHash,
      pricePayloadHash: approval.pricePayloadHash,
    };
    if (authoritative) {
      planMetadata.promotedAt = executedAt;
      delete planMetadata.offlineCandidateAt;
    } else {
      planMetadata.offlineCandidateAt = executedAt;
  ...
  const registry = args.noRegistryPublish
    ? await publishPromotionPairFailureAtomic({
      selectionPath: args.selectionOut,
      pricesPath: args.pricesOut,
      selectionText,
      pricesText,
  ...
  console.log(JSON.stringify({
    ...
    workFingerprint: approval.workFingerprint,
    authoritative,
    sourceSelection: args.selection,
    sourcePrices: args.prices,
    selectionOut: args.noRegistryPublish ? args.selectionOut : registry.registrySelectionPlan,
    pricesOut: args.noRegistryPublish ? args.pricesOut : registry.registryPriceOverrides,
    ...registry,
  }, null, 2));
}
