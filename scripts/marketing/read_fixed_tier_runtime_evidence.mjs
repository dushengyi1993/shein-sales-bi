...
const [links,inventory,policy,stores,costDoc]=await Promise.all(['outputs/bi-portal/sections/linksData.json','outputs/bi-portal/sections/inventoryTrend.json','config/marketing_pricing_policy.json','config/stores.json','tmp/mbrs/marketing-cost-map.json'].map(read));
const registry=verifyMarketingPlanRegistrySync({registryFile:process.env.SHEIN_BI_MARKETING_PLAN_REGISTRY_FILE || '/srv/shein-bi/runtime/marketing-plans/current.json'});
const baselineDoc=await read(registry.priceOverrides);
...
const context=buildLowEtFastSellerPricingContext({linksDataDoc:{storeLinks},inventoryTrendDoc:inventory,baselineDoc,costDoc,marketingPolicy:policy,reportDate:date});
const rows=[];
for (const link of storeLinks) {
  const canonical=link.standard_goods_sn || link.standardGoodsSn || link.canonical;
  if (!fixedTierItem(canonical) && !(standard.doc.inheritedStandards || []).some(r=>r.canonical===canonical)) continue;
...
const high=buildHighClickLowConversionSpecialAudit({linksDataDoc:{storeLinks},inventoryTrendDoc:inventory,priceOverridesDoc:baselineDoc,costDoc,manualRegistry:{entries:[]},marketingPolicy:policy,reportDate:date});
