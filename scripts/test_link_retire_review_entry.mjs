...
import {STORES,sha,EVIDENCE_HOST} from '../lib/link_retire_review_evidence.mjs';
...
try {
  ...
  // The evidence host is whatever lib/production_cloud_host.mjs resolves (the
  // deployed production host, or an environment override), never a literal.
  const raw=Buffer.from(JSON.stringify({schemaVersion:'link-retire-evidence/v1',host:EVIDENCE_HOST,runDate,performanceDate,querySha256:sha(query),generatedAt:'2026-09-07T05:00:00Z',sources:[],rows:[]}));
  await fs.writeFile(e,raw);
  const manifest={schemaVersion:'link-retire-evidence-manifest/v1',host:EVIDENCE_HOST,collectorSha256:sha(await fs.readFile('scripts/collect_link_retire_review_evidence.mjs')),requestSha256:sha(JSON.stringify({runDate,performanceDate,querySha256:sha(query),keys:[]})),evidenceSha256:sha(raw),bytes:raw.length,sourcesSha256:sha('[]')};
  ...
  // An explicit host is accepted by the resolver; the run is still refused here
  // because the bound evidence records a different host (provenance mismatch,
  // not a hardcoded host pin).
  assert.notEqual(run(['--cloud-ssh','other']).status,0);
  assert.equal(run(['--cloud-ssh',EVIDENCE_HOST]).status,0,'an explicit --cloud-ssh equal to the bound host is accepted');
