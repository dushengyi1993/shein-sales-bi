...
const C = {
  ...
  ship_order_track: ['ship_order_id','batch_id','send_city','arrive_city','box_qty','status','status_name','sign_time','tracks','raw_summary'],
...
function mapEndpoint(endpoint, batch, endpointData, sourceFile = '') {
  ...
  switch (endpoint) {
    ...
    case 'ship_order_track':
      return {table: 'fact.et_ship_order_track', columns: C.ship_order_track, conflict: ['ship_order_id'], rows: rows.map(r => { const id = text(r.__parent_id || r.shipOrderId || r.ShipOrderId); return {ship_order_id: id, batch_id: b, send_city: text(r.sendCity), arrive_city: text(r.arriveCity), box_qty: num(r.boxQty), status: text(r.status), status_name: text(r.statusName), sign_time: ts(r.signTime), tracks: compactJson(Array.isArray(r.tracks) ? r.tracks : []), raw_summary: compactJson(r)}; })};
    case 'box_list':
