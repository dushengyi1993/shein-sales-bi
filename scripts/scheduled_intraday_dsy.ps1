...
Push-Location $Root
try {
  "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] SHEIN all-store intraday sync start" | Out-File -FilePath $LogFile -Encoding UTF8
  if ($FeishuBasePaused) {
  ...
  "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] SHEIN all-store intraday sync end, dsy=$DsyExitCode, lgm=$LgmExitCode, monthly=$MonthlyExitCode, compact=$CompactExitCode, dashboard=$DashboardExitCode, report=$ReportExitCode, biPost=$BiPostExitCode, exit=$ExitCode" | Out-File -FilePath $LogFile -Encoding UTF8 -Append
  exit $ExitCode
}
finally {
  Pop-Location
