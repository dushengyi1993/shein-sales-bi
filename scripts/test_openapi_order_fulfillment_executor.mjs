...
try{
 ...
 const dry=await runNode(['scripts/bi_ops_cli.mjs','order-fulfillment','--operation','export-address','--openapi-config',config,'--store-truth',truth,'--store','SMK','--order-no','GSO123']);
 check('dry exits 0', dry.code, 0); check('dry ok', dry.json?.ok, true); check('dry no network', calls.length, 0); const hash=dry.json?.adapterResult?.payloadHash; check('dry has hash', Boolean(hash), true);
 const noConfirm=await runNode(['scripts/bi_ops_cli.mjs','order-fulfillment','--operation','export-address','--openapi-config',config,'--store-truth',truth,'--store','SMK','--order-no','GSO123','--mode','execute','--payload-hash',hash]);
 check('execute without confirm blocks', noConfirm.code !== 0, true); check('no confirm no network', calls.length, 0);
 const exec=await runNode(['scripts/bi_ops_cli.mjs','order-fulfillment','--operation','export-address','--openapi-config',config,'--store-truth',truth,'--store','SMK','--order-no','GSO123','--mode','execute','--confirm','SHEIN_ORDER_FULFILLMENT_SUBMIT','--payload-hash',hash]);
