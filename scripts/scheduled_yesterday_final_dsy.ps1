...
try {
  ...
  & $Node ".\scripts\run_sales_sync_job.mjs" --mode yesterday-final --group DSY --no-monthly --no-compact-display --no-dashboard 2>&1 |
    ForEach-Object { $_ | Out-File -FilePath $LogFile -Encoding UTF8 -Append }
  $DsyExitCode = $LASTEXITCODE
  & $Node ".\scripts\run_sales_sync_job.mjs" --mode yesterday-final --group LGM --no-monthly --no-compact-display --no-dashboard 2>&1 |
    ForEach-Object { $_ | Out-File -FilePath $LogFile -Encoding UTF8 -Append }
  ...
  if ($ExitCode -eq 0 -or $SalesFilesReadyForBi) {
    if ($ExitCode -eq 0) {
      "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Feishu yesterday-final succeeded; refresh BI dual-run slice for $TargetDate." | Out-File -FilePath $LogFile -Encoding UTF8 -Append
    } else {
      "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] Feishu yesterday-final returned $ExitCode but 15 local sales files are ready; refresh BI sales slice anyway for $TargetDate." | Out-File -FilePath $LogFile -Encoding UTF8 -Append
    }
  ...
  if ($ExitCode -ne 0) {
    Send-SyncIssueAlert "yesterday-final" $TargetDate "????????????????????????? 15 ???????? BI ??????"
  }
