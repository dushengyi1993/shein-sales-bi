...
<body>
  ...
  <script>
    ...
    async function refresh(){
      ...
      document.querySelectorAll('[data-finish]').forEach(b=>b.onclick=async()=>{ b.disabled=true; try{ const j=await api('/api/cloud-login/sessions/'+encodeURIComponent(b.dataset.finish)+'/finish',{method:'POST',body:JSON.stringify({token:SESSION_TOKENS.get(String(b.dataset.finish||''))||''})}); const recovery=j?.session?.recovery; msg(recovery?.status==='scheduled' ? ('登录态已验证并关闭窗口。'+recovery.storeKey+' 店 '+recovery.date+' 的缺失数据已进入自动补跑队列，无需再人工续跑。') : '登录态已验证并关闭窗口；当前没有需要补跑的历史缺口。'); await refresh(); }catch(e){ msg('完成失败：'+e.message); b.disabled=false; }});
