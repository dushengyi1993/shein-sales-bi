...
const cases = [
  ...
  [{standard_goods_sn: 'T1', goods_title: ''}, 'T1'],
...
assert.equal(resolveProductDisplayName({standard_goods_sn: 'SM-505A'}).source, 'config/product_catalog.json:model_code');
assert.equal(resolveProductDisplayName({standard_goods_sn: 'T1'}).needsReview, true);
...
assert.equal(enriched.rankings.dailyProducts[1].product_display_name, 'T1');
assert.equal(enriched.rankings.dailyProducts[1].product_display_name_needs_review, true);
