...
if [[ "$PROMOTION_CHANGED_FLAG" == 1 ]]; then
  ...
  # The portal can still be draining the promotion's row locks right after the commit,
  # so a single 60s attempt timed out on 3 of 14 runs on 2026-09-16 (and on 20 of 96
  # slots the day before on the previous host). Retry once with a wider budget before
  # letting the run fail: the section is regenerated by the next slot or prewarm either
  # way, but a failed refresh also leaves this run without its reconciled marker.
  if ! curl -fsS --max-time 180 "$PORTAL_URL/api/bi/section/liveSalesToday?refresh=1" >/dev/null; then
    echo "[today-sales-reconcile] WARN liveSalesToday refresh failed once; retrying in 15s" >&2
    sleep 15
    curl -fsS --max-time 180 "$PORTAL_URL/api/bi/section/liveSalesToday?refresh=1" >/dev/null
  fi
  OCCURRED_AT="$(date -Is)"
