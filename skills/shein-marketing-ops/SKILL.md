...
`2026-06-21` 后的巡检硬规则：不能只依赖 BI。BI 只负责发现经营线索（新链接、新 SKC、订单成交价、曝光、库存），不能证明营销后台是否真的报了普通活动或限时折扣。每日巡检必须有 SHEIN 后台 live 证据：云端负责只读 guard/队列/最终 19 店回读；真实写入默认使用本机后台 headless 店铺 profile，并在每店/每批完成后关闭。仅本机不可用且当天必须闭环时，才用受控云端浏览器 fallback。
...
- 新报名、补报或重建普通活动/限时折扣时，若 ET 当天已匹配运营可售 `<= 10`，且同一标准货号跨 19 店近 30 天有效销量合计 `> 30`，价格档位上移一级：同货号跨 19 店全部链接的最新 7 天曝光 Top5 恢复为普通链接最新已批准价；普通链接按普通目标利润率再加 5 个百分点重算。不得机械把页面折扣率减 5%，不得追溯改写正常旧活动，不得污染批准基准。
...
- 两个通道必须使用同一份终态证据，只能在最终 19 店 stack review、价格栈 readback、终态 repair queue 和最终 guard 全部完成后发送；禁止先发飞书中间结论再修改本任务结果。
