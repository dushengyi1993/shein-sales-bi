...
- 高点击低转化专属折扣使用两个并行入口，且都要求当前在售、`c7_sale_cnt = 0`：A. `c7_eps_uv > 3000` 且 `c7_goods_uv / c7_eps_uv > 4%`；B. `c7_eps_uv >= 3000` 且近 7 天加车访客 `c7_cart_uv >= 20`。缺失销量不得当 0，任一入口所需指标缺失不得猜测。价格按同标准货号最新已批准全局曝光 Top5 商品成本利润率再降 2 个百分点，最低利润率 15%，默认库存 10、周期 7 天。加车入口首批已于 2026-07-29 经用户确认，后续纳入长期自动执行；平台活动库存下限高于用户单批指定数量时必须阻断，不得静默改量。
...
- `2026-07-29` 起，用户长期授权自动补报“已批准并执行中的普通活动在报名截止前新出现的可报差额”。巡检命中上述差额时，不再逐次确认；必须沿用原批准来源生成 supplement 和不可变 approval manifest，完成身份校验、dry-run、真实提交和受影响店回读。平台最低档低于批准价时按平台档提交并记录差额。该授权仅覆盖当前已批准活动批次的截止前补漏，不覆盖未批准的新活动方案、普通活动库存虚增或优惠券。
