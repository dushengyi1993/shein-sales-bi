...
`2026-06-21` 后的巡检硬规则：不能只依赖 BI。BI 只负责发现经营线索（新链接、新 SKC、订单成交价、曝光、库存），不能证明营销后台是否真的报了普通活动或限时折扣。每日巡检必须有 SHEIN 后台 live 证据：云端负责只读 guard/队列/最终 21 店回读；真实写入默认使用本机后台 headless 店铺 profile，并在每店/每批完成后关闭。仅本机不可用且当天必须闭环时，才用受控云端浏览器 fallback。
...
- 高点击低转化专属折扣使用两个并行入口，且都要求当前在售、`c7_sale_cnt = 0`：A. `c7_eps_uv > 3000` 且 `c7_goods_uv / c7_eps_uv > 4%`；B. `c7_eps_uv >= 3000` 且近 7 天加车访客 `c7_cart_uv >= 20`。缺失销量不得当 0，任一入口所需指标缺失不得猜测。价格按同标准货号最新已批准全局曝光 Top5 商品成本利润率再降 2 个百分点，最低利润率 15%，默认库存 10、周期 7 天。加车入口首批已于 2026-07-29 经用户确认，后续纳入长期自动执行；平台活动库存下限高于计划量时走报名库存事务，不能永久抬高库存。
- 同一 `storeKey + SKC` 同时命中高点击专属折扣与普通漏兜底/新品/重新上架时，高点击专属折扣优先；普通兜底计划必须将该键记为 `handled_by_high_click_special_stage` 并排除。queue builder 仍须拒绝任何未被上游解释的跨阶段重复，不能通过关闭去重检查来绕过。
- 高点击专属折扣真实写入前先完成 ET 低库存畅销品价格优先级预判，再登记到人工特殊折扣保护表，随后 dry-run/报名库存事务/事务替换/live readback。worker 写登记前重读最新 7 日指标，已经出单、跌出阈值或下架的 stale 候选直接跳过。
- 人工特殊活动刚创建后的 future 行，只有活动 ID 等于保护登记 `currentActivityId`、价格/库存/截止精确且两小时内开始，才算已排期精确覆盖；不得因此重复救援，其他未来/过期活动继续 fail-closed。

报名库存与低库存畅销品价格的长期规则：

- 普通营销活动或限时折扣报名的真实最低库存只从活动 live 页面、`query_goods` 或 dry-run 读取；BI `platform_saleable_stock` 仅作线索。
- 报名前必须取得同一 `store + SKC/SKU` 库存锁并记录实时 `totalUsableInventory / totalInventoryQuantity / totalLockedQuantity`。若可用库存不足，只临时补到平台要求的精确最低可用库存；完成提交和 live readback 后，无论成功失败都在 `finally` 立即恢复报名之前的原可用库存，恢复覆盖量保留恢复时实时锁定/不可用量。
- 恢复后再次回读库存和活动报名状态。恢复失败、恢复后活动失效/被平台撤销，或锁定量变化导致不能精确恢复原可用库存时 fail closed，禁止写成功。即使 ET 当日运营可售 `<= 10` 也只允许该短窗口补量；窗口存在订单/超卖风险。该授权不允许永久增库存、优惠券补库存或脱离报名事务的库存修改。
- 执行器若不能输出平台最低值来源、补量前三字段快照、库存锁、临时写入、报名回读、`finally` 恢复及恢复后库存/报名双回读，必须报“报名库存事务能力未就绪”并停止；禁止调用旧 ET 门控持久补量路径。
- 当前实现统一使用 `lib/marketing_activity_inventory_transaction.mjs`、`lib/marketing_activity_inventory_openapi.mjs` 和 `lib/marketing_activity_inventory_integration.mjs`；旧 `manage_manual_limited_discount_inventory.mjs --execute` 已永久禁用。普通活动与四类限时折扣入口都必须复用共享事务，不得恢复旧持久补量。
- 新报名、补报或重建普通活动/限时折扣时，若 ET 当天已匹配运营可售 `<= 10`，且同一标准货号跨 21 店近 30 天有效销量合计 `> 30`，价格档位上移一级：同货号跨 21 店全部链接的最新 7 天曝光 Top5 恢复为普通链接最新已批准价；普通链接按普通目标利润率再加 5 个百分点重算。不得机械把页面折扣率减 5%，不得追溯改写正常旧活动，不得污染批准基准。
- 平台最低折扣/最高允许报名价优先，能收回多少收回多少；成本、底价、利润安全线仍是硬门禁。ET 恢复 `> 10` 后，后续新活动恢复原批准档位。优先级固定为“ET 低库存畅销品收回 > 自动高点击专属折扣 > 新链接/新品 Top5 > 普通基准”；人工特殊登记不自动覆盖，单列用户审核。
- Top5 收回价按标准货号统一读取最新批准全量 baseline 的普通链接档价；同一货号跨店统一恢复，不要求同一 `store + SKC` 存在单店历史价。缺该标准货号统一普通档价时才 fail closed。ET 当天完整快照省略零库存商品时，用 09 散件仓最新流水零余额回填为“已匹配、可售 0”；货号代码相同但一侧带中文品名时统一归并。执行前重读当天 ET、跨店 30 天销量与 7 天曝光，证据 hash 漂移则重建或阻断。
...
- `2026-07-29` 起，用户长期授权自动补报“已批准并执行中的普通活动在报名截止前新出现的可报差额”。巡检命中上述差额时，不再逐次确认；必须沿用原批准来源生成 supplement 和不可变 approval manifest，完成身份校验、dry-run、真实提交和受影响店回读。平台最低档低于批准价时按平台档提交并记录差额。该授权仅覆盖当前已批准活动批次的截止前补漏，不覆盖未批准的新活动方案、永久库存增加或优惠券；已获报名授权的普通活动可使用上文事务式临时补量并在 `finally` 恢复。
...
- 限时折扣少量补报后，用 `scripts/marketing/merge_current_marketing_price_scans.mjs` 把受影响店复扫覆盖进最新成功的完整 21 店价格快照，再生成 final guard。系统刚创建、名称和价格命中规则但尚在平台生效延迟中的未来限时折扣算“已排期待生效”，不能重复创建；未来普通活动仍不能替代当前兜底或提前用于订单审计。
...
- live 只读巡检优先云端 session HTTP/scan；每日长期授权限时折扣 repair/write 和每周已授权普通活动写入默认使用本地后台 headless profile。本地浏览器不弹前台、不抢焦点；跨店默认 4 个独立 profile 一批，负载高或 profile 不稳时降为 3 个，批内跨店并行。同店 `dry-run -> hash -> execute -> readback -> 库存恢复` 严格串行，profile、端口、临时目录、救援文件和可变输出不得共享；单店失败只隔离该店，整批终态后逐个清理并验证进程、端口、租约为 0。只有本机不可用且当天必须闭环时才使用受控云端浏览器 fallback，且云端每段最多 1 店/1 组。
...
- 新链接候选必须把 BI `linksData` 与截至报告日各店最新原始 `outputs/shein_links` 快照按 `store+SKC` 合并。原始快照新增键直接补入；同键快照更新且发生“待上架/售罄/下架 → 已上架”时，只更新上架状态、首次/恢复上架时间和来源审计字段，继续保留 BI 的曝光、活动、价格和库存字段，并在 `latestRawLinkOverlay.updatedRowCount/updatedRows` 单列。live 活动扫描只枚举活动商品，不能单独证明“所有在售链接均有活动”，所以不得在原始链接源比 BI sidecar 更新时输出无缺口。原始快照覆盖少于当前启用店铺数或有解析错误时，guard 和计划器都必须 fail closed，不得生成“无缺口”结论。
- 2026-08-02 起，普通营销活动和限时折扣的库存不足统一使用上文报名库存事务：逐 `store + SKC/SKU` 锁定、记录三字段快照、临时补到 live/query/dry-run 的精确最低可用库存、提交/readback、`finally` 恢复原可用库存，再回读库存与报名状态。任何恢复或恢复后验收失败均 fail closed；不得沿用旧“ET 足够才补、ET 不足阻断”或“普通活动库存不授权”的边界。
...
- 自动任务也不得上线 30%/50% 券，不得真实取消优惠券、补预算或无证据取消普通活动。默认自动写入仅限当前长期授权白名单：已批准普通活动截止前新增差额、限时折扣漂移/人工特殊恢复/新链接/重新上架/漏兜底、高点击专属折扣，以及这些已获授权报名事务的临时补量与恢复；全部必须满足身份校验、价格栈校验、库存/平台校验、dry-run 和执行后回读。
...
- 每日最终日报必须双通道交付：飞书群发一段最终结论和一个合并附件；当前 Codex 本任务另外正常输出有排版的人话日报。任何一边都不能替代另一边，也不得因飞书已发送而在本任务静默。
- 两个通道必须使用同一份终态证据，只能在最终 21 店 stack review、价格栈 readback、终态 repair queue 和最终 guard 全部完成后发送；禁止先发飞书中间结论再修改本任务结果。
