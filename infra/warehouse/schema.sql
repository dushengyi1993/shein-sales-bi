...
CREATE INDEX IF NOT EXISTS shein_webhook_receipt_queue_idx ON ops.shein_webhook_receipt (severity, received_at, id) WHERE status IN ('queued','retry');
CREATE INDEX IF NOT EXISTS shein_webhook_receipt_lease_idx ON ops.shein_webhook_receipt (lease_expires_at, id) WHERE status='running';
CREATE INDEX IF NOT EXISTS shein_webhook_receipt_store_received_idx ON ops.shein_webhook_receipt (store_key, received_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS shein_webhook_receipt_status_received_idx ON ops.shein_webhook_receipt (status, received_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS shein_webhook_receipt_business_key_idx ON ops.shein_webhook_receipt (business_key, received_at DESC) WHERE business_key <> '';
...
CREATE INDEX IF NOT EXISTS shein_webhook_store_gate_state_idx ON ops.shein_webhook_store_gate (state, updated_at DESC);
