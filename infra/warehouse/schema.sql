...
AS $$
  ...
  SELECT CASE
    ...
    WHEN key IN ('SK15032') THEN 'SK15032'
    ...
    WHEN key IN ('20700', '270', 'JY20700', 'SK270', 'YJSK270') THEN 'SK270'
    ...
    WHEN key IN ('SKGT3065W') THEN 'SKGT3065W'
...
AS $$
  SELECT CASE dim.product_match_key(value)
    ...
    WHEN 'SKGT3065W' THEN 'SK-GT-3065W蒸汽熨烫机'
...
-- WebAPI status rechecks and OpenAPI sales snapshots can use different
-- technical line keys for the same store/order/product. Resolve each current
-- sales fact to the newest strongly matched recheck instead of silently
-- falling back to stale sales-snapshot status text.
...
box_items AS (
  SELECT
    ...
    -- ET box-item RealQuantity is frequently present as numeric zero while
    -- CaseQuantity carries the actual unit count.  Treat non-positive values
    -- as unusable here; otherwise box fees enter the numerator while their
    -- units disappear from (or reduce) the product quantity denominator.
    sum(CASE
...
-- BEGIN SHEIN WEBHOOK PRODUCT STATE
BEGIN;
...
COMMIT;
-- END SHEIN WEBHOOK PRODUCT STATE
