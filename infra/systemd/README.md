...
chmod +x scripts/cloud_bi_refresh.sh scripts/cloud_db_backup.sh scripts/cloud_et_forwarder_sync.sh scripts/cloud_daily_lark_report.sh
systemctl daemon-reload
systemctl enable --now shein-bi-cloud-today.timer shein-bi-cloud-yesterday.timer shein-bi-db-backup.timer
# ET / 飞书日报在本地 secret 与授权配置完成后再启用：
# systemctl enable --now shein-bi-cloud-et-forwarder.timer shein-bi-cloud-daily-lark-report.timer
```
