...
- `shein-bi-db-backup.timer`：每天 `01:45`，仅在登录态 marker 完整后备份业务库、Metabase 元数据库和生产人工特殊折扣登记，宿主 deadline 为 `02:37`。Profile/WebAPI session 的 AES-256-GCM 归档能力保留，但生产 unit 默认 `SHEIN_BI_BROWSER_STATE_BACKUP_ENABLED=0`，不属于数据库备份 SLA；只有另行完成密钥异机保管、容量实测和恢复演练后才可显式启用。当前 COS 挂载使用公开对象读取，独立远端校验固定为 `anonymous-public`：目标 bucket/region/prefix 由仓库内 JSON + SHA-256 锁定，HTTPS 请求不得携带 Authorization/token，仍须完整流式回读对象并精确匹配长度和 SHA-256。过期数据库备份必须先通过相对路径清单、tar 类型/成员/内容 SHA-256 与 COS sidecar 回读，之后才删除本地副本；COS、归档或内容校验任一失败都保留本地。
