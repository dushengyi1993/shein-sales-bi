...
- 每日经营刷新：每天 `07:10` 只启动一个 `shein-bi-cloud-morning-chain.service`。`run_cloud_morning_chain_job.sh` 原子保存 active run context（含 first-start 绝对 deadline，重启复用不重置）；失败后 service 自动重启并恢复完全相同的 runDate/businessDate，跨日先完成旧业务日再推进当天；完成只认 `daily-operating-refresh` done marker 这一份权威证据；deadline 到期收敛 failed+alert 并停止自动重启（不假完成），次日新 run 不被阻塞。同一 run 内完成19店链接/业务域抓取、只重试失败店、全店仓库合并、OpenAPI/成本/利润补充、一次 Portal 原子发布及当天库存维护。最多两个只读浏览器 worker；第二槽受内存/负载门禁。任何缺店或指标未 ready 都保留上一完整页面，不发布半成品。
