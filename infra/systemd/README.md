...
- 每日经营刷新：每天 `07:10` 只启动一个 `shein-bi-cloud-morning-chain.service`。同一 run 内完成19店链接/业务域抓取、只重试失败店、全店仓库合并、OpenAPI/成本/利润补充、一次 Portal 原子发布及当天库存维护。最多两个只读浏览器 worker；第二槽受内存/负载门禁。任何缺店或指标未 ready 都保留上一完整页面，不发布半成品。
