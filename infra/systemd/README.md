...
## 当前启用集与条件启用集（2026-08-04）

**当前生产应启用**：`shein-bi-portal.service`、`shein-bi-webhook.service`，以及 `shein-bi-cloud-morning-chain`、`shein-bi-cloud-morning-link-chunk-2`、`shein-bi-cloud-morning-link-recovery`、`shein-bi-cloud-morning-supplements`、`shein-bi-cloud-yesterday`、`shein-bi-db-backup`、`shein-bi-cloud-order-closure`、`shein-bi-cloud-rtv-verify`、`shein-bi-daily-inventory-replenishment-guard`、`shein-bi-daily-inventory-replenishment-guard-retry`、`shein-bi-cloud-browser-cleanup`、`shein-bi-cloud-disk-maintenance`、`shein-bi-cloud-marketing-live-guard`、`shein-bi-cloud-marketing-repair`、`shein-bi-cloud-watchdog`、`shein-bi-cloud-et-forwarder`、`shein-bi-cloud-et-storage-fee`、`shein-bi-cloud-session-manager`、`shein-bi-cloud-manual-login-recovery`、`shein-bi-cloud-openapi-stock-refresh`、`shein-bi-cloud-today-sales-reconcile`、`shein-bi-cloud-portal-section-queue` 的 timer。人工登录 `.path` 不再启用，避免任意分钟拉起 Chrome；私有队列由 `:47` 安全机会轮处理。`daily-refresh` 保留为手动全量恢复入口，没有独立 timer。
...
- 晨间链接链路：每天 `08:00` 先抓12店，必须在 `08:27` 前释放；`08:45` 抓剩余店并合并。若仍有缺店，`11:15 / 12:15 / 14:45` 只续跑当天尚未完成的店铺，每段分别在 `:27 / :27 / :57` 前收口；`morning-links-ready` 已存在时完全不打开浏览器。当天 08:00 后 marker 尚未完成时，Portal 低优先级队列一律让路；它也在 `15:15 / 15:45` 库存窗口主动让路。恢复段触及硬截止记为可续跑 partial，不把 systemd 伪报成程序崩溃。`09:12` 补充域仍只在 marker 完整后运行。当天销售走独立轻量快车道，不等待晨间链路。
- 库存维护：`15:15` 主执行依赖 `morning-links-ready` 与 `15:12` 后 `stock-refresh`，最晚 `15:27` 释放；仅当主执行未完成时，`15:45` 运行一次幂等重试并在 `15:57` 前释放。Codex 事后审计固定 `16:20`，计划/结果未生成只能报告未完成，不得写成0条或成功。
...
systemctl enable shein-bi-portal.service shein-bi-webhook.service shein-bi-cloud-morning-chain.timer shein-bi-cloud-morning-link-chunk-2.timer shein-bi-cloud-morning-link-recovery.timer shein-bi-cloud-morning-supplements.timer shein-bi-cloud-yesterday.timer shein-bi-db-backup.timer shein-bi-cloud-order-closure.timer shein-bi-cloud-rtv-verify.timer shein-bi-daily-inventory-replenishment-guard.timer shein-bi-daily-inventory-replenishment-guard-retry.timer shein-bi-cloud-browser-cleanup.timer shein-bi-cloud-disk-maintenance.timer shein-bi-cloud-marketing-live-guard.timer shein-bi-cloud-marketing-repair.timer shein-bi-cloud-openapi-stock-refresh.timer shein-bi-cloud-today-sales-reconcile.timer shein-bi-cloud-portal-section-queue.timer shein-bi-cloud-manual-login-recovery.timer shein-bi-cloud-watchdog.timer
systemctl start shein-bi-portal.service shein-bi-webhook.service shein-bi-cloud-morning-chain.timer shein-bi-cloud-morning-link-chunk-2.timer shein-bi-cloud-morning-link-recovery.timer shein-bi-cloud-morning-supplements.timer shein-bi-cloud-yesterday.timer shein-bi-db-backup.timer shein-bi-cloud-order-closure.timer shein-bi-cloud-rtv-verify.timer shein-bi-daily-inventory-replenishment-guard.timer shein-bi-daily-inventory-replenishment-guard-retry.timer shein-bi-cloud-browser-cleanup.timer shein-bi-cloud-disk-maintenance.timer shein-bi-cloud-marketing-live-guard.timer shein-bi-cloud-marketing-repair.timer shein-bi-cloud-openapi-stock-refresh.timer shein-bi-cloud-today-sales-reconcile.timer shein-bi-cloud-portal-section-queue.timer shein-bi-cloud-manual-login-recovery.timer shein-bi-cloud-watchdog.timer
