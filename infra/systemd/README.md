...
- 晨间链接链路：每天 `08:00` 先抓12店，必须在 `08:27` 前释放；`08:45` 抓剩余店并合并。若仍有缺店，`11:15 / 12:15 / 14:45` 只续跑当天尚未完成的店铺，每段分别在 `:27 / :27 / :57` 前收口；`morning-links-ready` 已存在时完全不打开浏览器。Portal 低优先级队列在这些恢复点和 `15:15 / 15:45` 库存窗口主动让路。`09:12` 补充域仍只在 marker 完整后运行。当天销售走独立轻量快车道，不等待晨间链路。
