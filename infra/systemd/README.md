...
- `shein-bi-webhook.service`：SHEIN OpenAPI Webhook 独立常驻入口与数据库队列 worker，只监听 `127.0.0.1:8792`；公网固定走 Cloudflare 支持的 `8443`，由 UFW/Caddy 先限制 Cloudflare 边缘来源，再由 Nginx 按 SHEIN 官方推送 IP 放行。入口预算 1.2 秒、receipt SQL 预算 0.8 秒，只在 AES 密文可靠落库后回 200。worker 使用专属 `shein_webhook_ops` 与 `/srv/shein-bi/secrets/webhook-warehouse.env`，不得复用 Portal 的 `shein_link_ops`/secret；它没有运营任务表或事实表原始 DML，只能只读回读并调用按单 apply 函数，也没有日汇总/reconciliation 权限；风险事件先封闸后发送 P0 飞书摘要，不调用 sudo/Docker，也绝不直接写 SHEIN；飞书问数服务继续保持暂停。
