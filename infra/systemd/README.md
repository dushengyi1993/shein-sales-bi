...
- `shein-bi-cloud-today.timer`：全天每两小时整点（`00:00/02:00/.../22:00`）刷新当天销售、入仓并生成 BI 门户。
...
- `shein-bi-cloud-et-forwarder.timer`：全天每两小时错峰（`01:20/03:20/.../23:20`）抓取 ET 货代仓/出库单、入仓，并刷新 BI 门户；高频用于订单中心及时显示物流与 ET 出库信息。ET 高频任务禁用全量 section prewarm，避免每小时反复拉起销售/利润等重型缓存，销售整点任务仍会正常预热。需要服务器本地 `config/et_forwarder.local.json` 或 `ET_FORWARDER_USERNAME/ET_FORWARDER_PASSWORD`，密钥不进 GitHub。
- `shein-bi-cloud-link-business.timer`：每天 `08:10` 顺序抓取前一完整日链接/业务域，入仓、体检并刷新 BI 门户。需要服务器私有 SHEIN session / browser session，敏感运行态不进 GitHub。
