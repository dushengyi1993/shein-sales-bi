...
- `shein-bi-cloud-openapi-stock-refresh.timer`：每小时 `:18/:48` 轻量刷新21店商品列表与库存，避开每15分钟销售 reconciliation 的同刻启动；`07:18` 同轮按每店32条的上限优先补齐新商品详情并轮转旧缓存，其余高频轮只复用详情缓存。当前实测单轮约1分钟，`:18` 在 `:20` ET 前收口，`:48` 在 `:50` watchdog/RTV 前收口；拿不到共享机会窗口时允许跳过第二轮，不能挤占 `:32–:43` 全托首页车道。等待每日轮转的新详情不作为故障，真实请求失败、超过21天的详情缓存或库存缺失仍必须告警。SHEIN Webhook 不提供完整的当前虚拟库存，因此店铺×货号矩阵不能依赖日更浏览器快照；本任务只有在21店库存全部成功、无缺失后才重建独立的轻量 `inventoryStock` section，并通过 PostgreSQL `NOTIFY` + SSE 更新已打开页面，不重复生成耗时较长的完整 `linksData`。矩阵仅接受45分钟内 OpenAPI 确认已上架的库存，过期或缺失时显示未知，不回退到旧库存。
- `shein-bi-cloud-session-manager.timer`：每天仅一个 `00:45` 入口，顺序巡检/恢复当前 21 店 WebAPI + SBN 登录态并写入夜间依赖 marker。外层资源 defer(75) 在同一 service/run 内持续退避到 `01:27`；到截止时间仍未启动时写 deferred/failed marker 和 `cloud_ops_alerts/session-manager-last.json`，返回真实非成功。只有当日 `done` marker 与同日 21/21 报告同时成立才幂等跳过；无证据 `warning` 必须重做，不新增第二个 timer 或补跑队列。
...
- `shein-bi-cloud-yesterday.timer`：每天在同一个既有 timer 内于 `02:45/03:05/03:20` 提供有限尝试，`Persistent=false`，仅在登录态和数据库备份 marker 完整后，用官方 OpenAPI 收齐前一天21店完整日切片并复核前两天稳定日；缺 `nightly-backup` marker 返回 75 且保持失败可见，逐店 fetch/load/每日行完整性门禁全部通过后，才调用数据库函数原子晋升最终日切片。任一失败、缺店或缺少每日行都禁止晋升；`yesterday-final` 的同日结构化 marker 让重试快速幂等。
...
- `shein-bi-portal.service`：BI Portal 常驻入口，必须以 `sheinops` 运行并保留 `MemoryHigh=2400M` / `MemoryMax=3000M` / `OOMPolicy=stop` / `Restart=always`，防止问数网关或 section 服务异常占满整机内存。**2026-09-16 调整**：该 cgroup 还容纳 `generate_bi_portal.mjs` 的 section 生成子进程（`KillMode=control-group`，每个子进程只受 `--max-old-space-size=1536` 约束，RSS 可达约 1.8 GiB），旧值 1200M 低于子进程工作集，导致该 cgroup 被内核持续回收、生成进程卡住十余分钟不出结果；这份 stall 会让 `/proc/pressure/memory` 的 full avg10 全局报 ~90，使所有走资源闸门的浏览器/重任务以 `MEMORY_STALL_PRESSURE` 让位。软上限必须高于单个生成子进程的最坏用量，硬上限保持高于软上限。**2026-09-16 二次调整**：修完 1200M 后实测该 cgroup `memory.peak` 达到 1,781.8 MiB（距 1900M 软上限只剩 120 MiB），`memory.events.high` 三小时又涨 1.3 万次，于是再抬到 2400M/3000M、留出约 620 MiB 余量。同类口径适用于所有常驻服务：`shein-bi-cloud-session-manager.service` 的 `MemoryPeak` 实测 1,468,776,448 字节（比它自己的 1400M 软上限还高 0.7 MiB），已同步抬到 1800M/2400M。V8 旧空间通过 `Environment=NODE_OPTIONS=--max-old-space-size=1536` 固定为 1536MiB，作为低于 cgroup 硬上限的最后一道护栏；它不能替代有界内存实现。2026-08-14 的 210,748,631 字节 core 已证明单纯提高到 1536MiB 仍会 OOM，因此 Portal 读取 core 元数据、叠加实时商品对账 warning 和返回 `/data.json` 时必须使用有界流式扫描/顶层 `audit` 替换，不得重新引入整份 `readFile + JSON.parse + JSON.stringify/gzip`。每次启动前由 `ExecStartPre` 从当前 `scripts/bi_app/client.js` / `styles.css` 原子重建正式 `outputs/bi-portal/index.html`，禁止出现源码已发布而页面仍执行旧内嵌前端的情况。Portal 仍需调用经过白名单约束的 `sudo docker` 子命令并与浏览器维护任务共享临时目录，所以不能照抄 Lark bot 的 `NoNewPrivileges` / `PrivateTmp`；其余内核、systemd、umask 护栏由 unit 固化。认证启动前必须等待 `shein-bi-session-secret.service`，运行进程不再创建或修复 secret。Portal 是 `/srv/shein-bi/partner-cli` 的唯一发布写 owner；停机信号先同步关闭 HTTP/upgrade admission 并结束 SSE，再有界排空已接收 handler；只有排空成功才依次停止 worker/bridge 和 store，超时强制断连接并以失败终态退出。
...
- `shein-bi-cloud-marketing-live-guard.service`：每天 `11:00` 只启动一个巡检 coordinator；普通活动、价格栈和 guard 报告的瞬时失败在同一 run 内仅重试失败阶段，不再依赖 13:00/16:00 重跑整套巡检。该服务以 session HTTP 一次读取 21 店普通活动、15% 券 active 集合与当前/未来活动价，生成精确 manifest/hash 与 repair queue；有待修复项时不提前发中间群报，必须等本地 repair/write 与最终回读终态后只发一次最终报告。
- `shein-bi-cloud-marketing-repair.timer`：仅作本机离线后的晚间应急兜底，仍只由现有 `20:45/21:15` 两个时点触发，不新增 timer 或队列。每次先用 21 店 browserless live readback 重建精确剩余队列；存在 backlog 时在同一云端执行器内保持单浏览器、逐组串行，单轮最多 32 组。每组启动前重新检查预算，`22:55` 为 graceful cutoff，且不足 900 秒时不再开新组；外层 hard deadline 固定为 `23:10`，确保在 `23:20` ET 任务前释放资源。未完成组按 exact key 保留为 deferred，第二次 timer 只做幂等 catch-up，不重放已成功组。
- Codex 自动巡检不另设固定晚间汇总。每日待议价、营销、淘汰链接巡检分别按现有 Codex 自动任务的实际时间执行，并在各自任务完成后把同一份人话结论和产出文件发到团队运营群，避免重复消息或提前汇总未完成结果。
- `shein-bi-cloud-marketing-write-pause.conf`：营销写链路的持久暂停开关。2026-09-11 一个 root 建、无原因说明的空标记文件放在 `/run`，既静默停掉整条云端写链路三天，又在重启后消失。现在改为持久 drop-in，装到 `/etc/systemd/system/shein-bi-cloud-marketing-repair.service.d/60-marketing-write-pause.conf`，`ConditionPathExists=!` 指向 root 持有的 `/var/lib/shein-bi-control/marketing-write-pause.json`；暂停用 `sudo node scripts/manage_marketing_write_pause.mjs set --reason "<原因>"`，恢复用 `clear --reason "<原因>"`，两者都强制非占位 reason 并写 `marketing-write-pause.audit.ndjson`。迁移时同时删除旧的 `/run/systemd/system/shein-bi-cloud-marketing-repair.service.d/99-main-7032-pause.conf`；`status` 在运行中返回 0、暂停/过期返回 1。
...
- 本地执行入口：`run_cloud_marketing_fallback_slot.sh` 资源不足时按 `deferred_to_local` 退出，但迁到 fnOS 后不能没人消费这个 deferred。`scripts/run_local_marketing_repair_slot.sh` 就是本地执行者：必须显式带 `SHEIN_BI_MARKETING_REPAIR_EXECUTION_LOCATION=local`，且当天已有 repair queue 和 root 建的 immediate 授权文件才继续；否则退出 75（无事可做），绝不自己扫描/报名/改价。它只导出 `SHEIN_BI_MARKETING_IMMEDIATE_RUN=true` 后 `exec` 同一个 slot，授权校验、deadline、host-heavy 车道和受限 worker 全部复用，不产生第二套逻辑。本地 Codex 自动任务（shein-4、v7）保持只读汇总，执行一律经这个入口。
- 迁移阈值重估：`docs/fnos-migration-threshold-recalibration.md` 列出 host-heavy 负载闸门（`SHEIN_BI_HOST_PRESSURE_<CLASS>_<FIELD>` 环境覆盖，默认值不变、非法值 fail closed、生效值在输出里回显）、写入闸门分钟窗与并发上限、单日执行窗口和 repair timer 时点，以及每项必须量出的实测依据与测量协议；未测量的项不得放宽。
- `shein-bi-cloud-watchdog.timer`：每小时只读巡检。它读取一次 canonical maintenance marker，只有 23 个 guard 文件与有效 `ExecCondition` 均匹配时才按 service class 抑制预期停机；始终检查 always 服务、源码、28-service runtime namespace，以及 marker 对应的本地 attestation/checksum 与 annotated tag。维护结束与普通恢复同轮出现时合并一次通知；失败保留同一 outbox/idempotency 重试。它可以用后续完整 21 店扫描证据收口孤立历史 warning，但必须保留原状态和 recovery。
...
`shein-bi-webhook.service` 保持 `profiles=none`：它只运行 `serve_shein_webhook.mjs`，无 Chrome/浏览器 profile 责任，业务不需要 21 店 profile。policy 表、installer 落盘与 effective `systemctl show` 回读都必须满足 `InaccessiblePaths=/data/shein-bi/profiles /opt/shein-bi/app/profiles`，且不含任何 profile bind。
