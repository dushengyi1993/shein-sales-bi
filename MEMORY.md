...
## 计划任务
- 生产调度以云端 systemd 为准：`shein-bi-cloud-today.timer` 在北京时间 `00/02/04/06/10/12/14/16/18/20/22:00` 刷新当天销售、入仓并生成 BI Portal；`08:00` 由 `shein-bi-cloud-morning-chain.timer` 接管，先刷新销售再触发慢变日更；`shein-bi-cloud-yesterday.timer` 每天 `03:00` 刷新前一天最终销售并复核稳定日；`shein-bi-db-backup.timer` 每天 `02:40` 备份业务库和 Metabase 元数据库。
...
- 云端自动化已覆盖销售 WebAPI 直连、销售入仓、BI Portal 生成、数据库备份、ET 同步、飞书日报手动入口、完整 RTV 复核、统一日更补采、异常通知 watchdog 和只读飞书问数机器人；飞书日报自动发送已停用，HL OpenAPI 销售双跑已退出生产调度，本地 Windows 任务只作回滚参考。
- ET 已启用云端 Linux 高频入口：`scripts/cloud_et_forwarder_sync.sh` / `shein-bi-cloud-et-forwarder.timer`，当前按奇数小时 `:20` 抓 ET 货代仓/出库单并轻量刷新相关 section。ET 服务器侧使用私有 `config/et_forwarder.local.json` 或环境变量账号密码，不能复用 Windows Chrome 保存密码；ET 入仓依赖 Docker/root 环境，服务仍保留 root 执行，但验证码下载的一次性 `fetch failed` 必须进入重试而不是直接中断。飞书日报脚本仍保留为手动入口，但自动发送已停用：生产没有 `shein-bi-cloud-daily-lark-report.timer`，晨间链路 `SHEIN_BI_MORNING_SEND_LARK_REPORT=0`。secret/token/收件人完整 ID 不进 GitHub、文档或聊天。
- 链接/业务域、商品/库存/流量、营销活动/限时折扣/优惠券价格线索和 RTV 换单复核已收口到 `shein-bi-cloud-daily-refresh.service`，由 `08:00` 晨间链路在销售刷新后串行触发；旧分散 `shein-bi-cloud-link-business.timer`、`shein-bi-cloud-rtv-verify.timer`、`shein-bi-cloud-openapi-hl.timer` 在生产机保持 masked，不要重新启用。日更通过服务器私有 `state/shein_browser_sessions/*.local.json` / `state/shein_webapi_sessions/*.local.json` 初始化 headless Chrome，并验证 GSP + SBN 后再抓取；若全店日指标仍全 0，则跳过入仓刷新，避免把未出数日期写入 BI。该服务必须以 `sheinops` 运行，不能用 root 写 SHEIN 店铺 profile。不要再用本机隐藏补抓冒充云端日更；纯 Node 零浏览器直连只是后续优化。
...
- 飞书 Base / 看板写入仍受 `state/feishu-base-sync-paused.flag` 约束；飞书日报脚本和 watchdog 已云端化，但日报自动发送当前停用，不要默认认为本地日报/提醒任务仍在生产运行。
...
## 营销活动报名规则
- SHEIN 营销活动真实写入必须有当前线程明确授权。普通活动默认先首店预填不提交给用户确认；用户确认后，后续店铺可按授权分批提交并 live 回读。每日 heartbeat / 常规巡检仍只能只读，不得真实提交、取消、补预算或改限时折扣。
...
- 优惠券活动不要套普通营销活动报名路径；活动 `34810 / 平台优惠券招商活动` 已验证正确链路是 `#/mbrs/marketing/coupon/detail/34810` -> `继续报名` -> `#/mbrs/marketing/coupon/rule/signup/34810/{levelRuleId}?from=detail`。批量导入 `确定` 会直接真实提报。`2026-06-14` 后 15% 券不再是“配套价格保障券”，只能用于当前计划明确标记的高曝光支持、滞销高库存引流或清货试验；不能把普通活动 selection-plan 或 `MULTI_LEVEL_RULE_GOODS` 全量可报 SKC 当成应报目标。
...
- 每日 guard / 新链接动作卡必须先选中当前最新、已执行且不依赖优惠券保底的全量 `selection-plan + price-overrides`。若误用旧 `all-934` / 旧批次计划，会把已在新计划里的 SKC 误报成“新链接缺兜底”；这种情况应先修正计划选择并重跑 guard，不能直接打开前端创建限时折扣。
...
- 旧 `financeData` section 已下线，线上 `/api/bi/section/financeData` 应返回 `404`；不要再为 V1/旧财务页面预热或生成缓存。当前 `inventoryTrend` 是 SHEIN 前台展示库存趋势 section，不是 ET 实盘库存；2026-06-20 云端验证其文件约 `242KB`（gzip 约 `20KB`），后续如果又说“21MB”，先查线上 section 体积再判断。
...
- V2 已是当前正式 BI Portal，根路径由 `scripts/generate_bi_portal.mjs` 生成的 `outputs/bi-portal/index.html` 承载；线上 `/v1/` 已返回 `410`，`/v2/` 只跳转到根路径。V1 只保留 GitHub release tag `2026.06.18-v1-final-archive`，如需排查旧版必须恢复到隔离环境，不进入生产调度。V2 数据判断和验收必须走云端运行态/线上 section API；仓库快照只作页面启动兼容。
...
## 实际库存与去化口径
- 库存页真相以 ET 货代仓实盘为准：ET 可售/在库/在途/发货申请单/箱明细是库存台账主证据；SHEIN 后台库存只是各店链接的虚拟前台库存，只在“店铺 × 标准货号已上架库存矩阵”等展示链接库存时使用，不能拿来当总库存。
- `inventoryTrend` 来自 SHEIN 前台展示库存快照，只能说明展示库存趋势，不等于 ET 实盘可售；店铺/分组筛选只影响销售速度、风险排序和链接矩阵，不硬拆物理库存。
...
- 2026-06-20 云端全量复核并经用户确认：ET 发货申请单当前全量就是 `95` 单（`fact.et_ship_order_item` 约 `107` 行，箱明细约 `2512` 行），这不是缺采；点名的 `F2602034904752` 已存在。页面和告警不得再把 `ET发货单累计 < ET库存 + 历史销量` 写成“发货单缺采”，只能标为 `ET库存/销量匹配待核`，用于排查货号归并、销量或库存匹配问题。
