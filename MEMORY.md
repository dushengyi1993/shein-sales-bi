...
- GitHub release 只代表源码基线；稳定发布完成时云端 `/opt/shein-bi/app` 必须通过 `check_release_source_state.mjs`，精确等于 release target commit，且不存在源码脏改、隐藏索引标记或缺失的 tracked 文件，随后再以 systemd、Portal、数据库和日志验收生产。Portal 生成物和可变运行态不得进入 Git。
