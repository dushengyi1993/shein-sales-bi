...
- 2026-05-17 `/api/ops-agent/ask` 网页智能体回复接入已部署到云端 BI：Portal 服务改为 `sheinops` 用户运行并使用 `/home/sheinops/.codex`，链接管理中台已重构为独立会话工作流：先多轮对话，聊清楚后再进入任务池；推荐指令来自当前 BI 数据；任务池支持确认、继续优化、执行预备、完成、归档、删除、进度和历史留痕。标题生成入口暂时从任务卡收起，等正式取标题 skill 导入后再接；仍不得执行 SHEIN 写操作。
