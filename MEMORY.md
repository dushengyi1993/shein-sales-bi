...
- 2026-05-17 链接管理对话必须“每轮动态查数”：服务端把最新一句和会话上下文传给只读问数网关，网关每次重新从当前 `outputs/bi-portal/data.json` 里按店铺、货号、SKC、曝光/访客/销量等线索取数；最新一句明确换店/换货号时以最新为准，只有“这个链接/刚才那个/2,223这个”等指代不完整时才沿用上文。若用户发出明确动作命令（如下架、换图、补链、报活动），回复不能只说“没权限不能执行”，系统应自动加入链接运营任务池并说明等待人工确认/执行器预检。
