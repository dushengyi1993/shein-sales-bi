import {resolveFixedTierPrice} from './marketing_fixed_tier_pricing.mjs';
...
export function buildHighClickLowConversionSpecialAudit({
  ...
  for (const row of uniqueLinkRows(linksDataDoc)) {
    ...
    const fixedTier = resolveFixedTierPrice(common, lowEtContext.fixedTierContext);
    if (fixedTier.blocked) {base.blockedRows.push({...common,status:'fixed_tier_pricing_blocked',pricing:fixedTier});continue;}
    const pricing = fixedTier.applies ? {available:true,specialPrice:fixedTier.price,fixedTierPricing:fixedTier.binding} : deriveHighClickSpecialPricing({
      canonical,
      priceOverridesDoc,
      costDoc,
      marketingPolicy,
