...
function modelIdentity(value) {
  const code = modelCodeKey(value);
  if (!code) return null;
  const digits = (code.match(/\d/g) || []).join('');
  if (!digits) return null;
  const lastDigit = code.search(/\d(?!.*\d)/u);
  const suffix = lastDigit >= 0 ? code.slice(lastDigit + 1).replace(/[^A-Z]/g, '') : '';
...
function displayFromTitle(sn, row) {
  ...
  for (const title of titleCandidates(row)) {
    ...
    const titleIdentity = modelIdentity(titleSplit.code);
    const snIdentity = modelIdentity(snText);
    const sameModelIdentity = titleIdentity && snIdentity
      && titleIdentity.digits === snIdentity.digits
      && titleIdentity.suffix === snIdentity.suffix;
    if (titleKey && snKey && (titleKey === snKey || sameModelIdentity) && hasHan(titleSplit.rest)) {
...
function rowStandardGoodsSn(rowOrSn) {
  ...
  return cleanText(rowOrSn.standard_goods_sn || rowOrSn.goods_sn || rowOrSn.raw_goods_sn || '');
}
...
export function resolveProductDisplayName(rowOrSn) {
  ...
  if (titleDisplay) {
    return {displayName: titleDisplay, source: 'title_descriptor', needsReview: Boolean(normalized?.needsReview)};
  }
...
export function enrichProductDisplayNames(data) {
  ...
  function remember(key, displayName) {
    const cleanKey = cleanText(key);
    if (!cleanKey || !displayName) return;
    const current = productDisplayNames.get(cleanKey);
    if (shouldReplaceMapValue(current, displayName)) productDisplayNames.set(cleanKey, displayName);
  }
  ...
  function visit(value) {
    ...
    if (sn) {
      ...
      // Preserve the source key for audit/search while exposing normalized and
      // raw keys for consumers that join ET rows to sales.
      remember(sn, resolved.displayName);
      const normalized = normalizeGoodsSnDetailed(sn, {
        goodsTitle: value.goods_title || value.product_name || value.product_name_cn || value.title || '',
      });
      if (normalized?.canonical && normalized.canonical !== sn) remember(normalized.canonical, resolved.displayName);
      if (value.raw_goods_sn && value.raw_goods_sn !== sn) remember(value.raw_goods_sn, resolved.displayName);
    }
