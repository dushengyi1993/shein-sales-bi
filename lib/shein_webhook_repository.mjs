...
import {humanizeSheinWebhookEvent} from './shein_webhook_handlers.mjs';
...
function rowToFrontendEvent(row) {
  const normalized = parseJson(row.normalized);
  const businessContext = parseJson(row.business_context, null);
  const relatedEventCount = Math.max(1, Number(row.related_count || 1));
  const enriched = {
    ...normalized,
    ...(businessContext ? {productContext: businessContext} : {}),
    ...(relatedEventCount > 1 ? {relatedEventCount} : {}),
  };
  const copy = humanizeSheinWebhookEvent(enriched, {severity: row.severity});
  return {
    id: String(row.id),
    receivedAt: iso(row.received_at),
    processedAt: iso(row.processed_at),
    ...
    summary: copy.summary || row.summary || '',
    businessKey: row.business_key || '',
    actionState: row.action_state || '',
    duplicate: Boolean(row.duplicate || relatedEventCount > 1),
    ...(normalized?.productState && typeof normalized.productState === 'object'
      ? {productState: normalized.productState}
      : {}),
  };
}

function webhookIncidentPartition() {
  return `CASE
    WHEN COALESCE(normalized->>'eventFamily','')='product_shelves' THEN concat_ws('|',
      store_key, event_code, COALESCE(business_key,''), COALESCE(normalized->>'action',''),
      to_char(date_trunc('minute', received_at), 'YYYYMMDDHH24MI')
    )
    ELSE concat('receipt:', id::text)
  END`;
}

function defaultBusinessEventPredicate() {
  return `(COALESCE(normalized->>'appScopedOnly','false') <> 'true'
    AND (
      severity IN ('P0','P1','P2')
      OR status IN ('failed','retry','dead_letter')
    ))`;
}
...
export function createSheinWebhookRepository({pool, env = process.env, PoolClass} = {}) {
    ...
    const result = await withPgClient(source, client => client.query({
      ...
      text: `/* shein_webhook:mark_processed */
        WITH updated AS (
          ...
          WHERE id=$1 AND status='running' AND lease_owner=$11
          RETURNING *
        ), notified AS (
          SELECT pg_notify(
            'shein_bi_live_update',
            json_build_object(
              'receiptId',id,
              'eventCode',event_code,
              'eventFamily',COALESCE(normalized->>'eventFamily',''),
              'storeKey',store_key,
              'businessKey',business_key,
              'entityId',business_key,
              'businessDate',COALESCE(normalized#>>'{warehouseSync,businessDate}',''),
              'orderStatus',COALESCE(normalized#>>'{warehouseSync,orderStatus}',''),
              'orderStatusDesc',COALESCE(normalized#>>'{warehouseSync,orderStatusDesc}',''),
              'cancelledBeforePickup',COALESCE((normalized#>>'{warehouseSync,cancelledBeforePickup}')::boolean,false),
              'salesQuantity',COALESCE((normalized#>>'{warehouseSync,salesQuantity}')::numeric,0),
              'salesSar',COALESCE((normalized#>>'{warehouseSync,salesSar}')::numeric,0),
        ...
        SELECT updated.* FROM updated LEFT JOIN notified ON true`,
  ...
  async function listEvents({allowedStores = [], filters = {}, cursor, limit = 100, includeTechnical = false} = {}) {
    const max = positiveInteger(limit, 'limit', {min: 1, max: 500});
    const values = [];
    const clauses = [allowedStoreScope(allowedStores, values)];
    if (!includeTechnical) clauses.push(defaultBusinessEventPredicate());
    ...
    values.push(max + 1);
    const incidentKey = webhookIncidentPartition();
    const result = await withPgClient(source, client => client.query({
      text: `/* shein_webhook:list_events:safe_projection */
        WITH scoped AS (
          SELECT id, received_at, processed_at, store_key, event_code, normalized,
            severity, status, title, summary, business_key, action_state, duplicate_count,
            row_number() OVER (PARTITION BY ${incidentKey} ORDER BY received_at DESC, id DESC) AS incident_rank,
            count(*) OVER (PARTITION BY ${incidentKey}) AS related_count
          FROM ops.shein_webhook_receipt
          WHERE ${clauses.join(' AND ')}
        ), limited AS (
          SELECT id, received_at, processed_at, store_key, event_code, normalized,
            severity, status, title, summary, business_key, action_state, duplicate_count, related_count
          FROM scoped WHERE incident_rank=1
          ORDER BY received_at DESC, id DESC LIMIT $${values.length}
        )
        SELECT id, received_at, processed_at, store_key, event_code, normalized,
          normalized->>'eventFamily' AS event_type, severity, status, title, summary,
          business_key, action_state, related_count,
          (duplicate_count > 0 OR related_count > 1) AS duplicate,
          COALESCE(
            normalized->'productContext',
            CASE WHEN normalized->>'eventFamily'='product_shelves' AND COALESCE(business_key,'')<>''
              THEN ops.get_shein_webhook_product_context(store_key, business_key, received_at)
              ELSE NULL
            END
          ) AS business_context
        FROM limited
  ...
  async function getProductBusinessContext({storeKey, skc, eventAt = new Date().toISOString()} = {}) {
    const store = required(storeKey, 'storeKey').toUpperCase();
    const productSkc = required(skc, 'skc');
    const at = timestamp(eventAt, 'eventAt') || new Date().toISOString();
    const result = await withPgClient(source, client => client.query({
      name: 'shein-webhook-get-product-business-context',
      text: '/* shein_webhook:get_product_business_context */ SELECT ops.get_shein_webhook_product_context($1,$2,$3::timestamptz) AS context',
      values: [store, productSkc, at],
    }));
    return parseJson(result.rows?.[0]?.context, null);
  }

  async function applyProductState({
    receiptId,
    storeKey,
    skc,
    eventFamily,
    ...
    const result = await withPgClient(source, client => client.query({
      name: 'shein-webhook-apply-product-state',
      text: `/* shein_webhook:apply_product_state */
        SELECT ops.apply_shein_webhook_product_state(
          $1,$2,$3,$4,$5,$6,$7::numeric,$8::timestamptz,$9::jsonb
        ) AS applied`,
      values: [
        positiveInteger(receiptId, 'receiptId', {max: Number.MAX_SAFE_INTEGER}),
        required(storeKey, 'storeKey').toUpperCase(),
        required(skc, 'skc'),
        required(eventFamily, 'eventFamily'),
        required(action, 'action'),
        text(productStatus),
        eventOrder(sourceEventOrder),
        timestamp(eventAt, 'eventAt'),
        json(productContext, 'productContext'),
      ],
    }));
    return Boolean(result.rows?.[0]?.applied);
  }

  async function listTaskReconciliationReceipts({limit = 100} = {}) {
    const max = positiveInteger(limit, 'limit', {min: 1, max: 500});
    const result = await withPgClient(source, client => client.query({
      name: 'shein-webhook-list-task-reconciliation-receipts',
      text: `/* shein_webhook:list_task_reconciliation_receipts */
        SELECT id, received_at, processed_at, store_key, event_code, normalized,
          severity, status, title, summary, business_key, action_state, duplicate_count
        FROM ops.shein_webhook_receipt
        WHERE status='succeeded'
    ...
    return (result.rows || []).map(row => {
      ...
      return receipt ? {...receipt, idempotencyKey: receipt.id} : null;
    }).filter(Boolean);
  }

  async function getTaskReconciliationReceipt(id) {
    const receiptId = positiveInteger(id, 'id', {max: Number.MAX_SAFE_INTEGER});
    const result = await withPgClient(source, client => client.query({
      name: 'shein-webhook-get-task-reconciliation-receipt',
      text: `/* shein_webhook:get_task_reconciliation_receipt */
        SELECT id, received_at, processed_at, store_key, event_code, normalized,
          severity, status, title, summary, business_key, action_state, duplicate_count
        FROM ops.shein_webhook_receipt
  ...
  async function markTaskReconciliation(id, {actionState, taskId = ''} = {}) {
    const receiptId = positiveInteger(id, 'id', {max: Number.MAX_SAFE_INTEGER});
    const state = required(actionState, 'actionState');
    if (!['task_readback_attached', 'task_unmatched', 'task_match_ambiguous'].includes(state)) {
      throw new SheinWebhookRepositoryError(`Unsupported task reconciliation state: ${state}`, {code: 'SHEIN_WEBHOOK_VALIDATION'});
    }
    const result = await withPgClient(source, client => client.query({
      name: 'shein-webhook-mark-task-reconciliation',
      text: '/* shein_webhook:mark_task_reconciliation */ SELECT ops.record_shein_webhook_task_reconciliation($1,$2,$3) AS applied',
      values: [receiptId, state, text(taskId).slice(0, 200)],
    }));
    return Boolean(result.rows?.[0]?.applied);
  }

  async function summary({allowedStores = [], now = new Date(), includeTechnical = false} = {}) {
    ...
    const run = async client => {
      ...
      const businessEvents = includeTechnical ? 'TRUE' : defaultBusinessEventPredicate();
      // Every summary query receives `values`, including queries that do not
      // otherwise use the time boundary. Keep $1 in their SQL so an all/empty
      // store scope cannot produce a zero-parameter statement with one bound
      // value. These queries are intentionally unnamed because their SQL shape
      // changes with the caller's store scope.
      const incidentKey = webhookIncidentPartition();
      const common = `FROM (
        SELECT id, received_at, store_key, event_code, normalized, severity, status, business_key,
          row_number() OVER (PARTITION BY ${incidentKey} ORDER BY received_at DESC, id DESC) AS incident_rank
        FROM ops.shein_webhook_receipt
        WHERE $1::timestamptz IS NOT NULL AND ${scope} AND ${businessEvents}
      ) AS webhook_incidents WHERE incident_rank=1`;
      const [totals, byType, byStore] = await Promise.all([
        client.query({text: `/* shein_webhook:summary_totals */ SELECT
        ...
        client.query({text: `/* shein_webhook:summary_by_type */ SELECT COALESCE(normalized->>'eventFamily','unknown') AS event_type, count(*)::bigint AS total ${common} GROUP BY 1 ORDER BY total DESC, event_type`, values}),
        client.query({text: `/* shein_webhook:summary_by_store */ SELECT store_key, count(*)::bigint AS total ${common} GROUP BY 1 ORDER BY total DESC, store_key`, values}),
      ]);
  ...
  return Object.freeze({
    ...
    storeReceipt, claimNext, markProcessed, markAlerted, release, renew, summary, listEvents, getProductBusinessContext,
    applyProductState,
    listTaskReconciliationReceipts, getTaskReconciliationReceipt, markTaskReconciliation,
