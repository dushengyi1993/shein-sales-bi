...
export function createSheinWebhookRepository({pool, env = process.env, PoolClass} = {}) {
  ...
  async function upsertStoreGate({storeKey, gateType, state, reason = '', sourceReceiptId = null} = {}) {
    ...
    const result = await withPgClient(source, client => client.query({
      ...
      text: `/* shein_webhook:upsert_store_gate */
        WITH applied_gate AS (
          INSERT INTO ops.shein_webhook_store_gate(store_key,gate_type,state,reason,source_receipt_id)
          VALUES ($1,$2,$3,$4,$5)
          ON CONFLICT (store_key,gate_type) DO UPDATE SET state=EXCLUDED.state, reason=EXCLUDED.reason,
            source_receipt_id=EXCLUDED.source_receipt_id, updated_at=clock_timestamp()
          WHERE ops.shein_webhook_store_gate.source_receipt_id IS NULL
        ...
        LIMIT 1`,
      values: [required(storeKey, 'storeKey'), required(gateType, 'gateType'), required(state, 'state'), text(reason), sourceId],
    }));
  ...
  async function listStoreGates({storeKeys = [], blockingOnly = true} = {}) {
    ...
    const result = await withPgClient(source, client => client.query({
      ...
      text: `/* shein_webhook:list_store_gates:safe_projection */
        SELECT store_key, gate_type, state, reason, source_receipt_id, updated_at
  ...
  return Object.freeze({
    ...
    upsertStoreGate, getStoreGate, listStoreGates,
  });
}
