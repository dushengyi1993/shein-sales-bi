...
export function normalizeWebhookBusinessEvent({eventCode, eventPath, payload, storeKey = '', receivedAt = new Date().toISOString()} = {}) {
  ...
  const shelfStates = source.shelfChangeInfos?.flatMap(change => Array.isArray(change?.siteChangeInfos) ? change.siteChangeInfos.map(site => text(site?.shelfState)).filter(Boolean) : []) || [];
...
export function classifyWebhookSeverity(input = {}) {
  ...
  const auditFailure = /audit|review/.test(family) && (isFailure(status) || status === '3');
  ...
  if (['price_abnormal', 'inventory_warning', 'out_of_stock', 'rrp_review', 'rrp_validity'].includes(family)) {
