...
import {readRegularBoundedFile, validateDeployedReleaseMarker} from './source_release_attestation.mjs';
...
/** Retire the temporary receipt only after the formal deployment's terminal readback. */
export function retireEmergencyLocalReleaseReceipt({receiptFile, deploymentMarker, source} = {}) {
  if (!validateDeployedReleaseMarker(deploymentMarker, {requireV3:true}).ok
    || source?.ok !== true || source.head !== deploymentMarker.commit) {
    fail('EMERGENCY_RETIRE_FORMAL_RELEASE_REQUIRED', 'A verified current formal deployment is required');
  }
  const prior=readEmergencyLocalReleaseReceipt(receiptFile);
  if (!prior.exists) return {retired:false,reason:'absent'};
  if (!prior.ok) return {retired:false,reason:'invalid_receipt_preserved'};
  if (Date.parse(prior.receipt.createdAt)>Date.parse(deploymentMarker.recordedAt)) {
    return {retired:false,reason:'newer_receipt_preserved'};
  }
  const archiveDir=path.join(path.dirname(receiptFile),'retired-emergency-releases');
  fsSync.mkdirSync(archiveDir,{recursive:true,mode:0o750});
  const archiveFile=path.join(archiveDir,`${prior.receipt.receiptHash}.json`);
  const before=fsSync.readFileSync(receiptFile);
  if (sha256Hex(before)!==prior.bytesSha256) fail('EMERGENCY_RETIRE_RECEIPT_CHANGED','Emergency receipt changed before retirement');
  try {fsSync.copyFileSync(receiptFile,archiveFile,fsSync.constants.COPYFILE_EXCL);}
  catch (error) {if(error.code!=='EEXIST')throw error;}
  if (!fsSync.readFileSync(archiveFile).equals(before) || !fsSync.readFileSync(receiptFile).equals(before)) {
    fail('EMERGENCY_RETIRE_RECEIPT_CHANGED','Retirement archive or active receipt changed');
  }
  fsSync.unlinkSync(receiptFile);
  return {retired:true,archiveFile,receiptHash:prior.receipt.receiptHash,supersededBy:deploymentMarker.commit};
}
