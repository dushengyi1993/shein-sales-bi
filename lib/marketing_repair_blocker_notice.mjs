...
export function buildMarketingRepairNotificationKey(date, fingerprint) {
  const datePart = String(date || 'unknown').replace(/[^0-9A-Za-z]/g, '').slice(0, 16) || 'unknown';
  const fingerprintPart = String(fingerprint || '').replace(/[^0-9A-Za-z]/g, '').slice(0, 20) || 'none';
  return `mkt-repair-${datePart}-${fingerprintPart}`.slice(0, 50);
}
