...
function storePayloadBinding(store) {
  return {
    ...
    items: store.items.map(item => ({itemHash: item.itemHash, payload: item.payload})),
  };
}

function preflightBatchBinding(preflight) {
  return {
    schemaVersion: preflight.schemaVersion,
    businessDate: preflight.businessDate,
    generatedAt: preflight.generatedAt,
    expiresAt: preflight.expiresAt,
    scanHash: preflight.scanHash,
    decisionsHash: preflight.decisionsHash,
    sourceHashes: preflight.sourceHashes,
    stores: preflight.stores.map(store => ({
      storeKey: store.storeKey,
      payloadHash: store.payloadHash,
      itemHashes: store.items.map(item => item.itemHash),
    })),
  };
}
...
export function buildPreflightDocument({scan, decisions, sourceHashes, generatedAt = new Date().toISOString(), expiresAt}) {
  ...
  const actionByCanonical = new Map();
  ...
  for (const decision of Array.isArray(decisions.decisions) ? decisions.decisions : []) {
    ...
    if (actionByCanonical.has(canonicalGoodsSn)) throw new Error(`duplicate or conflicting decision for ${canonicalGoodsSn}`);
    actionByCanonical.set(canonicalGoodsSn, action);
    canonicalDecisions.push({canonicalGoodsSn, action});
  }
  canonicalDecisions.sort((left, right) => left.canonicalGoodsSn.localeCompare(right.canonicalGoodsSn));
  if (!canonicalDecisions.length) throw new Error('decisions list is empty');
  for (const decision of canonicalDecisions) {
    const matches = scan.rows.filter(row => row.discussStatus === 1 && row.canonicalGoodsSn === decision.canonicalGoodsSn);
    if (!matches.length) throw new Error(`decision matched no current pending rows: ${decision.canonicalGoodsSn}`);
  }
  const items = scan.rows
    .filter(row => row.discussStatus === 1 && actionByCanonical.has(row.canonicalGoodsSn))
    .map(row => {
      const lockedRow = normalizePendingDiscussRow(row.storeKey, row);
      const action = actionByCanonical.get(lockedRow.canonicalGoodsSn);
      if (action === 'accept') {
  ...
  const preflight = {
    ...
    unmatchedPendingCount: scan.rows.filter(row => row.discussStatus === 1).length - items.length,
...
export function verifyPreflightDocument(preflight, {businessDate, now = new Date(), sourceHashes, batchHash} = {}) {
  ...
  try {
    if (sourceHashes && stableStringify(normalizeSourceHashes(sourceHashes)) !== stableStringify(preflight?.sourceHashes || {})) blockers.push(blocker('SOURCE_HASH_DRIFT', 'source/schema/config hashes changed after preflight'));
  } catch (error) {
  ...
  try {
    ...
    if (batchHash && batchHash !== preflight?.batchHash) blockers.push(blocker('BATCH_HASH_CONFIRMATION_MISMATCH', 'confirmed batchHash does not match preflight'));
  } catch (error) {
