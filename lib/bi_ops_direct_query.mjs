...
const STORE_KEYS = new Set([
  ...
  'CX', 'YJ', 'XL', 'QY', 'QH', 'TZ', 'JSH', 'TZZ', 'XC', 'LG', 'HY', 'DSY', 'LGM',
]);
...
export function planBiOpsDirectQuerySections(question, options = {}) {
  ...
  const dailyTrend = /每日|逐日|按天|趋势|daily|trend/iu.test(text);
  ...
  // The compact homepage shard is the current-day baseline and is warmed on
  // every Portal start. Reserve the much larger full rankings shard for
  // explicit period/trend requests so ordinary owner queries do not block on
  // an avoidable profit-backed rebuild.
  if (needsRankings) addSection(sections, intent.fullRankings ? 'rankings' : 'homeRankings');
