...
function eventSourceAt(event, fallback = '') {
  const candidates = [
    event?.timestamp,
...
async function collectSessions({codexHome, projectRoot, state}) {
  ...
  for (const {file} of withStats) {
    ...
    let lastEventAt = String(prior.lastEventAt || '');
    for (const row of chunk.lines) {
      let event;
      try { event = JSON.parse(row.line); } catch { continue; }
      const rowEventAt = eventSourceAt(event, lastEventAt || new Date(Math.max(0, row.offset)).toISOString());
      if (Number.isFinite(Date.parse(String(event?.timestamp || '')))) lastEventAt = rowEventAt;
    ...
    next[rel] = {offset: chunk.nextOffset, cwd, lastEventAt, mtimeMs: chunk.stat.mtimeMs};
  }
