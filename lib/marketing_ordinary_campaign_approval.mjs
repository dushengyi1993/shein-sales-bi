...
export function validateReviewedWorkbookPriceRows(rows, {workbookSha256, businessDate} = {}) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(businessDate || '')) throw Error('Reviewed workbook business date is required');
  for (const row of reviewed) {
    const proof = row.reviewedWorkbookPrice;
    if (proof?.schemaVersion !== 'ordinary-reviewed-workbook-price/v1'
...
export function readReviewedWorkbookPrice(capability, row, reportDate) {
  ...
  return {applies: true, ok: true, price: approved.proof.targetPrice, tier: approved.proof.tier,
...
export async function loadOrdinaryCampaignApproval({
  ...
  if (validated.priceRows.some(row => row.reviewedWorkbookPrice !== undefined)) {
    ...
    if (workbook?.businessDate !== businessDate) throw Error('Reviewed workbook approval is not for the current business date');
