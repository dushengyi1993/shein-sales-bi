...
// Callers must be able to tell an absent release artifact from an unreadable
// one. Collapsing both into OPEN_FAILED made the inventory writer report
// "authority unreadable" for a marker that was merely missing, and hid the
// real ownership/permission fault behind a generic open error.
function fileOpenFailureCode(error) {
  const code = String(error?.code || '');
  if (code === 'ENOENT' || code === 'ENOTDIR') return 'SOURCE_RELEASE_FILE_MISSING';
  if (code === 'EACCES' || code === 'EPERM') return 'SOURCE_RELEASE_FILE_PERMISSION_DENIED';
  return 'SOURCE_RELEASE_FILE_OPEN_FAILED';
}
...
export function readRegularBoundedFile(file, maxBytes, label) {
  ...
  try {
    ...
    throw fail(`${label} cannot be opened safely: ${error.message}`, fileOpenFailureCode(error));
  }
  try {
