...
async function renameWithRetry(temporary, target) {
  for (let attempt = 0; attempt < RENAME_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      const exhausted = attempt === RENAME_MAX_ATTEMPTS - 1;
      if (exhausted || !TRANSIENT_RENAME_ERROR_CODES.has(code)) throw error;
      // Windows antivirus/indexing and readers can briefly retain a handle.
      // Retry the same atomic replacement only; never copy or unlink the target.
