...
import {resolveProductionCloudHost} from './production_cloud_host.mjs';
...
// Resolved once from lib/production_cloud_host.mjs so the delivery channel
// follows the deployed production host instead of a retired alias.
export const CLOUD_TEAM_REPORT_CLOUD_HOST = resolveProductionCloudHost();
...
export function interpretLarkResult({exitCode = 0, stdout = '', stderr = '', executionIdentityVerified = false, spawnError = false, timedOut = false, retryable = false} = {}) {
  ...
  if (Number(exitCode) !== 0) {
    // A CLI/connection failure can happen after acceptance. Only an explicit
    // rejection proves no message was accepted; stderr wording is insufficient.
    if (parsed?.ok === false) return {accepted: false, errorCode: 'lark_response_not_ok'};
    return {accepted: false, errorCode: 'lark_receipt_unknown', unknown: true};
  }
  if (!parsed) {
    return {accepted: false, errorCode: 'lark_receipt_unknown', unknown: true};
