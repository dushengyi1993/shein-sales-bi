...
export function parseJsonFromText(text) {
  ...
  if (!raw) return null;
  // File uploads may print a progress line before a pretty-printed JSON
  // receipt. Try every possible JSON start against the complete remaining
  // suffix before falling back to the historical single-line parser.
  const starts = [];
  const ends = [];
  for (let index = 0; index < raw.length; index += 1) {
    if (raw[index] === '{' || raw[index] === '[') starts.push(index);
    if (raw[index] === '}' || raw[index] === ']') ends.push(index);
  }
