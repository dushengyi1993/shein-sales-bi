...
export const MANUAL_LIMITED_DISCOUNT_SEED_PATH = path.join(MODULE_ROOT, 'config', 'marketing_manual_limited_discount_overrides.json');
export function resolveManualLimitedDiscountRegistryPath({env = process.env, platform = process.platform, home = os.homedir()} = {}) {
  return path.resolve(env.SHEIN_BI_MANUAL_LIMITED_DISCOUNT_REGISTRY
    || path.join(env.SHEIN_BI_RUNTIME_ROOT || (platform === 'linux' ? '/srv/shein-bi/runtime' : path.join(home, '.shein-bi', 'runtime')),
      'marketing_manual_limited_discount_overrides.json'));
}
export const DEFAULT_MANUAL_LIMITED_DISCOUNT_OVERRIDES_PATH = path.resolve(
  resolveManualLimitedDiscountRegistryPath(),
);
...
export async function loadManualLimitedDiscountRegistry(file = DEFAULT_MANUAL_LIMITED_DISCOUNT_OVERRIDES_PATH) {
  ...
  let bytes;
  try {
    bytes = await fs.readFile(resolved, 'utf8');
  } catch (error) {
    // Read compatibility for a first install. Writers still publish only to
    // the requested runtime path under its lock; source is a read-only seed.
    if (error.code !== 'ENOENT' || resolved !== DEFAULT_MANUAL_LIMITED_DISCOUNT_OVERRIDES_PATH
      || process.env.SHEIN_BI_MANUAL_LIMITED_DISCOUNT_REGISTRY) throw error;
    resolved = MANUAL_LIMITED_DISCOUNT_SEED_PATH;
    bytes = await fs.readFile(resolved, 'utf8');
  }
  const doc = JSON.parse(bytes.replace(/^\uFEFF/, ''));
...
export function applyManualLimitedDiscountOverride(row, entry) {
  ...
  return {
    ...
    fixedTierPricing: normalized.fixedTierPricing || row?.fixedTierPricing || null,
  };
}
