...
export function normalizePublishPreparationOverrides(value = {}) {
  ...
  // Present only for an ADDITIONAL link of an already-used 标准货号. When set,
  // every SKU needs its own seller SKU instead of one shared value: the platform
  // rejects a seller SKU that the store already owns and also rejects two SKUs of
  // the same request sharing one seller SKU, even when colour/size differ.
  const uniqueSupplierSkuDiscriminator = safeString(
    value.uniqueSupplierSkuDiscriminator ?? value.unique_supplier_sku_discriminator,
    80,
  );
  ...
  return {
    ...
    standardGoodsSn,
    supplierSku,
    uniqueSupplierSkuDiscriminator,
    supplyPrice,
...
const SUPPLIER_SKU_MAX_LENGTH = 64;
const TASK_IDENTITY_PATTERN = /^lot_(\d{8})(?:\d{6})_([0-9a-z]{4,})$/iu;

/**
 * Seller-SKU suffix parts, taken from the task identity.
 *
 * Production convention for an ADDITIONAL link of an existing 标准货号, read
 * back from the 2026-07-29 batch payloads: `SM961-260729-N07`, i.e.
 * 货号 + YYMMDD + per-link token. Deriving the date from the task id (not from
 * "today") matters: a retry on a later day must produce the SAME seller SKU for
 * the same link, otherwise the payload hash changes and the platform sees a
 * new SKU instead of the same link.
 */
function supplierSkuLinkTokens(discriminator) {
  const raw = safeString(discriminator, 120);
  const match = TASK_IDENTITY_PATTERN.exec(raw);
  if (match) return {date: match[1].slice(2), link: match[2]};
  return {date: '', link: raw.replace(/[^0-9A-Za-z]+/gu, '').slice(0, 8)};
}

export function buildPerSkuSupplierSku({standardGoodsSn, skuIndex = 0, skuTotal = 1, discriminator = ''}) {
  const standardGoodsSnText = safeString(standardGoodsSn, 200);
  if (!standardGoodsSnText) throw new Error('Per-SKU supplier SKU cannot be derived without a standard goods SN');
  const {date, link} = supplierSkuLinkTokens(discriminator);
  const parts = [standardGoodsSnText, date, link].filter(Boolean);
  // The platform also rejects two SKUs of ONE request sharing a seller SKU, even
  // when colour/size differ, so a multi-SKU link disambiguates by position.
  if (skuTotal > 1) parts.push(String(skuIndex + 1));
  return parts.join('-').slice(0, SUPPLIER_SKU_MAX_LENGTH);
}
...
export function applyExplicitPublishPreparationOverrides(payload, rawOverrides = {}) {
  ...
  for (const [skcIndex, skc] of skcList.entries()) {
    ...
    for (const [skuIndex, sku] of skuRows.entries()) {
      if (!sku || typeof sku !== 'object') continue;
      // Additional link of an existing 标准货号: the platform needs a distinct
      // seller SKU per SKU, unique in the store. supplier_code keeps the 标准货号.
      const perSkuSupplierSku = !overrides.supplierSku
        && overrides.uniqueSupplierSkuDiscriminator
        && overrides.standardGoodsSn
        ? buildPerSkuSupplierSku({
            standardGoodsSn: overrides.standardGoodsSn,
            skuIndex,
            skuTotal: skuRows.length,
            discriminator: overrides.uniqueSupplierSkuDiscriminator,
          })
        : '';
      const targetSupplierSku = overrides.supplierSku || perSkuSupplierSku || overrides.standardGoodsSn;
      if (targetSupplierSku) {
        sku.supplier_sku = targetSupplierSku;
        delete sku.supplierSku;
        const sourceKind = overrides.supplierSku
          ? 'unique_override'
          : (perSkuSupplierSku ? 'unique_per_sku' : 'explicit');
        applied.push(`skc_list[${skcIndex}].sku_list[${skuIndex}].supplier_sku.${sourceKind}`);
      }
