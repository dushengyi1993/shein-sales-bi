...
function normalizePlatformIdentifier(value, {kind}) {
  ...
  if (!text) return '';
  if (kind === 'skc') return /^s(?:v|b)\d+$/.test(text) ? text : '';
  if (kind === 'spu') return /^[a-z]\d{10,}$/.test(text) && !/^s(?:v|b)\d+$/.test(text) ? text : '';
...
export function applyApprovedImageBindingsToMaintenancePayload(identity, bindings, {sourceApproved = false} = {}) {
  ...
  if (!skcName) throw new Error('Approved maintenance image binding requires one explicit real sv/sb SKC');
  const normalizedBindings = normalizeApprovedImageBindings(bindings, {sourceApproved});
  if (normalizedBindings.some(row => row.role === 'skuImage') && !skuCodes.length) {
  ...
  const base = {
    ...
    skc_list: [{
      skc_name: skcName,
      ...(skuCodes.length ? {sku_list: skuCodes.map(skuCode => ({sku_code: skuCode}))} : {}),
    }],
  };
  ...
  return {
    ...
    evidence: {
      ...
      touchedFields: ['spu_name', 'is_spu_pic', 'image_info', 'skc_list[].skc_name', 'skc_list[].image_info', ...(skuCodes.length ? ['skc_list[].sku_list[].sku_code', 'skc_list[].sku_list[].image_info'] : [])],
    },
  };
}
