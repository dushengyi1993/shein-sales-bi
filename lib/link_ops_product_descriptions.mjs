...
function extractReviewedV3DocxStructure(documentXml) {
  ...
  const labelledLayout = rows.some(row => /^Main Title 3 - .+/.test(row.text));
  const titleStart = oneReviewedV3RowIndex(rows, row => (labelledLayout ? /^Main Title 3 - .+/ : /^Main Title 3(?:｜|\|)/).test(row.text), 'Main Title 3');
  const titleEnd = rows.findIndex((row, index) => index > titleStart && row.text === (labelledLayout ? '9. 三语核心卖点' : '首测标题'));
  ...
  const englishScore = oneReviewedV3RowIndex(titleRows, row => row.text === (labelledLayout ? '英文标题' : '英文评分'), 'Main Title 3 English label');
  const arabicMeaning = oneReviewedV3RowIndex(titleRows, row => row.text === (labelledLayout ? '阿文标题' : '阿文中文释义'), 'Main Title 3 Arabic label');
  const englishTitle = nonEmptyDocxLine(titleRows[englishScore + (labelledLayout ? 1 : 2)]?.text, 'Main Title 3 English');
  const arabicTitle = nonEmptyDocxLine(titleRows[arabicMeaning + (labelledLayout ? 1 : 2)]?.text, 'Main Title 3 Arabic');
