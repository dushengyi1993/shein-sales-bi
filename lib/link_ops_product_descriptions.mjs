...
export const DESCRIPTION_SOURCE_PROOF_HEADING9 = 'server_verified_html_numbered_heading_9';
 ...
 */
/**
 * True when the payload or task already carries description state that an
 * attribute adoption could invalidate.
 *
 * An attribute adopt provably does not modify the payload, so when no
 * description state exists yet there is nothing for the description lock to
 * protect. Demanding a fully valid description lock there deadlocked the normal
 * order (adopt attribute -> prepare descriptions -> execute): the attribute gate
 * blocked prepare-descriptions, and adopt_existing required the description lock
 * that prepare-descriptions was supposed to create (SK-185, lot_20260914132556_543ce3d4).
 */
export function hasPublishDescriptionState(task, payload) {
  return descriptionPayloadFields(payload).length > 0
    || isPlainObject(task?.descriptionMaterialBinding)
    || isPlainObject(task?.emptyDescriptionAuthorization);
}
...
// Package previews never contribute text or enter the published material.
const DOCX_IGNORED_BINARY_ENTRIES = new Set(['docProps/thumbnail.jpeg', 'docProps/thumbnail.wmf']);
...
const DOCX_REVIEWED_V3_ALLOWED_WORD_TAGS = new Set([
  ...
  'w:tr', 'w:vAlign', 'w:bidi', 'w:rtl',
  'w:bookmarkStart', 'w:bookmarkEnd', 'w:insideH', 'w:insideV', 'w:left', 'w:right',
  'w:tblBorders', 'w:tblCellMar', 'w:tblInd', 'w:tblPrEx', 'w:gridSpan', 'w:keepNext',
]);
...
function parseReviewedV3WordDocumentParagraphs(documentXml) {
  ...
  for (const token of tokens) {
    ...
    if (token.type === 'start') {
      ...
      if (['w:bidi', 'w:rtl'].includes(token.name)) {
        const expectedParent = token.name === 'w:bidi' ? 'w:pPr' : 'w:rPr';
        if (stack.at(-1) !== expectedParent) throw docxError('DOCX direction property must belong to paragraph/run formatting', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
      }
...
function extractReviewedV3DocxStructure(documentXml) {
  ...
  const titleStart = oneReviewedV3RowIndex(rows, row => !row.inTable && /^Main Title 3(?:$| - .+|[｜|].+)/.test(row.text), 'Main Title 3');
  const labelledLayout = /^Main Title 3 - .+/.test(rows[titleStart].text);
  const paragraphLayout = rows[titleStart].text === 'Main Title 3';
  const directTitleLayout = !labelledLayout && !paragraphLayout
    && rows[titleStart + 1]?.inTable && rows[titleStart + 2]?.inTable
    && /^[\x20-\x7e]{80,}$/.test(rows[titleStart + 1]?.text || '')
    && /[\u0600-\u06ff]/.test(rows[titleStart + 2]?.text || '');
  const titleEnd = rows.findIndex((row, index) => index > titleStart && row.text === (labelledLayout || paragraphLayout || directTitleLayout ? '9. 三语核心卖点' : '首测标题'));
  ...
  const englishScore = directTitleLayout ? 0 : oneReviewedV3RowIndex(titleRows, row => row.text === (labelledLayout || paragraphLayout ? '英文标题' : '英文评分'), 'Main Title 3 English label');
  const arabicMeaning = directTitleLayout ? 1 : oneReviewedV3RowIndex(titleRows, row => row.text === (paragraphLayout ? '阿拉伯语标题' : labelledLayout ? '阿文标题' : '阿文中文释义'), 'Main Title 3 Arabic label');
  const titleOffset = labelledLayout || paragraphLayout || directTitleLayout ? 1 : 2;
  const englishTitle = nonEmptyDocxLine(titleRows[englishScore + titleOffset]?.text, 'Main Title 3 English');
  const arabicTitle = nonEmptyDocxLine(titleRows[arabicMeaning + titleOffset]?.text, 'Main Title 3 Arabic');
  ...
  const validateFive = (lines, language) => {
    if (lines.length !== DESCRIPTION_LINE_COUNT
      || lines.some(line => !line || line.trim() !== line || line.includes('\r') || line.includes('\n'))) {
      throw docxError(`DOCX V3 ${language} 卖点必须恰好5个完整原文行`, 'DESCRIPTION_DOCX_SELLING_POINTS_REQUIRED');
    }
    return [...lines];
  };
  if (paragraphLayout) {
    // This reviewed layout uses three labelled groups of five body paragraphs.
    // Delimit whole groups first: never take five rows from a longer group.
    const labels = ['English Selling Points', 'Arabic Selling Points', '中文卖点'];
    const indexes = labels.map(label => oneReviewedV3RowIndex(sellingRows, row => row.text === label, label));
    if (indexes[0] !== 1 || indexes[1] <= indexes[0] || indexes[2] <= indexes[1]) {
      throw docxError('DOCX V3 段落卖点语言分组顺序无效', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
    }
    const groups = indexes.map((index, i) => {
      const group = sellingRows.slice(index + 1, indexes[i + 1] ?? sellingRows.length);
      if (group.some(row => row.inTable || row.lines.length !== 1)) {
        throw docxError('DOCX V3 段落卖点必须逐段单行，禁止隐藏换行', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
      }
      return validateFive(group.map(row => row.text), labels[i]);
    });
    return {title: {en: englishTitle, ar: arabicTitle}, en: {lines: groups[0]}, ar: {lines: groups[1]}, 'zh-cn': {lines: groups[2]}};
  }
  if (directTitleLayout) {
    // The unlabelled table layout has one explicit five-point explanatory row
    // and exactly three copy boxes. Script checks establish a unique language;
    // no translation or inferred business content enters the result.
    if (sellingRows.length !== 5 || sellingRows[1].inTable
      || sellingRows[1].text !== '三种语言独立展示，便于直接复制；每种语言 5 条，逐行事实对应，卖点内容本身不加序号、不加项目符号。') {
      throw docxError('DOCX V3 无标签复制框结构不完整或存在额外段落', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
    }
    const groups = {};
    for (const box of sellingRows.slice(2)) {
      if (!box.inTable) throw docxError('DOCX V3 卖点复制框必须位于表格', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
      const lines = validateFive(box.lines, '复制框');
      const arabic = /\p{Script=Arabic}/u;
      const han = /\p{Script=Han}/u;
      const languages = [
        ['en', lines.every(line => /[A-Za-z]/.test(line) && !arabic.test(line) && !han.test(line))],
        ['ar', lines.every(line => arabic.test(line) && !han.test(line))],
        ['zh-cn', lines.every(line => han.test(line) && !arabic.test(line))],
      ].filter(([, matches]) => matches).map(([language]) => language);
      if (languages.length !== 1 || groups[languages[0]]) throw docxError('DOCX V3 复制框语言缺失、混合或重复', 'DESCRIPTION_DOCX_STRUCTURE_INVALID');
      groups[languages[0]] = {lines};
    }
    return {title: {en: englishTitle, ar: arabicTitle}, ...groups};
  }
  const englishIntro = oneReviewedV3RowIndex(sellingRows, row => labelledLayout ? row.text === 'English Selling Points' : row.text.startsWith('英文卖点评分：'), '英文卖点说明');
  const arabicIntro = oneReviewedV3RowIndex(sellingRows, row => labelledLayout ? row.text === 'Arabic Selling Points' : row.text.startsWith('阿文卖点评分：'), '阿文卖点说明');
  const chineseIntro = oneReviewedV3RowIndex(sellingRows, row => labelledLayout ? row.text === '中文卖点' : row.text.startsWith('中文仅用于内部核对'), '中文卖点说明');
...
function assertOrdinaryDocxPackage(entries) {
  ...
  for (const [name, bytes] of entries) {
    ...
    // WPS stores a scalar typography preference in settings, not body text.
    // Recognize only this exact metadata shape; all other AlternateContent
    // (including any fallback, object or additional child) remains rejected.
    const settingsMetadata = name === 'word/settings.xml'
      && xml.includes('xmlns:wpsCustomData="http://www.wps.cn/officeDocument/2013/wpsCustomData"')
      ? xml.replace(/<mc:AlternateContent><mc:Choice Requires="wpsCustomData"><wpsCustomData:typoFeatureVersion val="[01]"\/><\/mc:Choice><\/mc:AlternateContent>/g, '')
      : xml;
    if (DOCX_FORBIDDEN_XML_RE.test(settingsMetadata) || /TargetMode\s*=\s*["']External["']/i.test(xml)
...
export function validateDescriptionBindingLock(task, payload) {
  ...
  if (![DESCRIPTION_SOURCE_PROOF, DESCRIPTION_SOURCE_PROOF_S9, DESCRIPTION_SOURCE_PROOF_HEADING9, DESCRIPTION_SOURCE_PROOF_DOCX].includes(binding.sourceProof)) {
    blockers.push(`descriptionMaterialBinding.sourceProof 必须为 ${DESCRIPTION_SOURCE_PROOF}、${DESCRIPTION_SOURCE_PROOF_S9}、${DESCRIPTION_SOURCE_PROOF_HEADING9} 或 ${DESCRIPTION_SOURCE_PROOF_DOCX}。`);
  }
...
export function validateUpdateDescriptionBindingLock(task, payload) {
  ...
  if (![DESCRIPTION_SOURCE_PROOF, DESCRIPTION_SOURCE_PROOF_S9, DESCRIPTION_SOURCE_PROOF_HEADING9].includes(binding.sourceProof)) {
    blockers.push(`descriptionMaterialBinding.sourceProof 必须为 ${DESCRIPTION_SOURCE_PROOF}、${DESCRIPTION_SOURCE_PROOF_S9} 或 ${DESCRIPTION_SOURCE_PROOF_HEADING9}。`);
  }
  if (binding.payloadHashAlgorithm !== DESCRIPTION_PAYLOAD_HASH_ALGORITHM) {
    blockers.push(`descriptionMaterialBinding.payloadHashAlgorithm 必须为 ${DESCRIPTION_PAYLOAD_HASH_ALGORITHM}。`);
  }
