...
export async function loadSheinWebhookCredentialRegistry({configFile, config, expectedStoreKeys} = {}) {
  ...
  function resolve(headers = {}) {
    ...
    // that belongs to another app/store, while allowing an unknown hint only
    // when the app itself maps to one unambiguous store.
    if (hintedStore && !effectiveApp.storeKeys.has(hintedStore)) {
    ...
    return {appId: effectiveApp.appId, appSecretKey: effectiveApp.appSecretKey, openKeyId, storeKey, identityScope};
  }
