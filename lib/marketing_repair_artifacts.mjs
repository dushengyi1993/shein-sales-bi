...
export async function inspectMarketingRepairArtifacts({root, queue, env = process.env}) {
  ...
  async function visit(logical, expectedHash) {
    ...
    if (!raw || raw !== raw.trim() || /[\r\n\0]/.test(raw)) throw new Error('Invalid marketing artifact reference');
    let input = raw;
    if (raw.replaceAll('\\', '/').split('/').includes('..')
      && !raw.replaceAll('\\', '/').startsWith('../../../srv/shein-bi/runtime/marketing-plans/')) {
      const physical = path.resolve(root, raw);
      const outputs = runtimeArtifactLocation({root, file: 'outputs', env}).path;
      const relative = path.relative(outputs, physical);
      // Queue references may use the builder's exact canonical spelling.
      // Only approved outputs qualify; never normalize arbitrary traversal.
      if (!relative || relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)
        || raw !== path.relative(path.resolve(root), physical)
        || !path.relative(path.resolve(root), physical).startsWith(`..${path.sep}`)) {
        throw new Error(`Unsupported portable marketing artifact: ${raw}`);
      }
      input = physical;
    }
    const name = safeLogical(runtimeArtifactLocation({root, file: input, env}).logicalPath);
