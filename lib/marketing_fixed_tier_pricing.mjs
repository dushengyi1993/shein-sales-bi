...
export function resolveFixedTierPrice(row, context) {
  ...
  const evidence = {key:key(row),canonical:item.canonical,reportDate:context.reportDate,metrics,age,newListing:recent,rank,ranking,...(rankBounds?{rankBounds}:{}),cost,exception:exception || null,...(reviewed.applies?{reviewedWorkbookPrice:reviewed.binding}:{})};
  if (reviewed.applies) margin=(target-cost.fullUnitCostSar)/target;
  const binding = {schemaVersion:'marketing-fixed-tier-binding/v1',ruleVersion:standard.doc.version,ruleHash:standard.sha256,canonical:item.canonical,tier,price,originalTargetPrice:target,mode:reviewed.applies?'reviewed_workbook_price':fixed?'fixed_sar':inherited?'inherited_user_standard':'missing_standard_full_cost_margin',marginTarget:margin,inheritedSource:inherited || null,platform,evidenceHash:hash(evidence),evidence};
