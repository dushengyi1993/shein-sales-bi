...
import {readReviewedWorkbookPrice,validateReviewedWorkbookPriceRows} from './marketing_ordinary_campaign_approval.mjs';
import {PRICE_VARIATION_POLICY,priceVariationSeed,resolveTierPriceVariation,validateVariationBasis} from './marketing_price_variation.mjs';
...
export function loadFixedTierStandard() {
  ...
  for (const item of doc.reviewedWorkbookBases || []) {
    const k=fixedTierCanonicalKey(item.canonical);
    if (!k || reviewedKeys.has(k) || !/^\d{4}-\d{2}-\d{2}$/.test(item.effectiveDate || '')
      || !/^[a-f0-9]{64}$/.test(item.workbookSha256 || '') || !item.sourceCells?.length || !item.reason) throw Error('reviewed_workbook_basis_invalid');
    validateVariationBasis(item.basis);reviewedKeys.add(k);
  }
...
export function buildFixedTierContext(linksDataDoc, {reportDate = '', standard = loadFixedTierStandard(),costDoc={},baselineDoc={},reviewedWorkbookPriceCapability=null} = {}) {
  ...
  const approvedTiers=new Map(),reviewedBases=new Map();
  // Consumers bind this source document by SHA/approval manifest. A pending
  // execution is a valid price basis; this does not mark that execution done.
  const approvedSource=['user_approved_pending_execution','completed'].includes(baselineDoc.executionStatus)
    && /^[a-f0-9]{64}$/.test(baselineDoc.planMetadata?.workFingerprint || '');
  if (approvedSource && reportDate >= '2026-09-10') for (const row of baselineDoc.items || []) {
    ...
    let tier=null;
    if (proof) {
      validateReviewedWorkbookPriceRows([row],{workbookSha256:proof.workbookSha256,businessDate:proof.businessDate});
      const canonical=fixedTierCanonicalKey(canonicalOf(row));
      const prior=reviewedBases.get(canonical);
      const source={basis:proof.basis,workbookSha256:proof.workbookSha256,sourceCells:proof.sourceCells,
        businessDate:proof.businessDate,reason:proof.reason,workFingerprint:baselineDoc.planMetadata.workFingerprint};
      if (prior?.conflict || (prior && hash(prior.basis)!==hash(source.basis))) reviewedBases.set(canonical,{conflict:true});
      else if (!prior?.conflict) reviewedBases.set(canonical,source);
      tier=proof.tier;
    } else if (binding?.schemaVersion==='marketing-fixed-tier-binding/v1'
      && binding.evidenceHash===hash(binding.evidence) && binding.evidence?.key===key(row)
      && fixedTierCanonicalKey(binding.canonical)===fixedTierCanonicalKey(canonicalOf(row))
      && [0,1,2].includes(binding.tier)) tier=binding.tier;
    if (tier!==null) {
      const prior=approvedTiers.get(key(row));
      if (prior && (prior.tier!==tier || prior.canonical!==fixedTierCanonicalKey(canonicalOf(row)))) approvedTiers.set(key(row),{conflict:true});
      else if (!prior?.conflict) approvedTiers.set(key(row),{tier,canonical:fixedTierCanonicalKey(canonicalOf(row)),workFingerprint:baselineDoc.planMetadata.workFingerprint});
    }
  }
  return {standard,reportDate,byKey,costDoc,baselineKeys,reviewedWorkbookPriceCapability,approvedTiers,reviewedBases,
    classificationCoverageComplete:linksDataDoc?.classificationCoverageComplete!==false};
}
...
function classifyObservedFixedTierLink(row, context) {
  ...
  if (tier === null) {
    if (recent.applies) tier = 1;
      ...
      if (context.classificationCoverageComplete===false) return blocked('fixed_tier_global_exposure_evidence_incomplete');
...
// Pricing and platform eligibility are separate. Missing BI classification
// uses a reviewed tier or the regular tier, without asserting shelf/exposure.
// High-click eligibility is still evaluated separately before special pricing.
export function classifyFixedTierLink(row, context) {
  const observed = classifyObservedFixedTierLink(row, context);
  if (observed.ok || !(context?.reportDate >= '2026-09-10')) return observed;
  const link = context?.byKey?.get(key(row));
  if (link?.fixedTierDuplicateConflict || observed.reason === 'fixed_tier_link_identity_changed') return observed;
  const recoverable = new Set([
    'fixed_tier_missing_or_conflicting_link_evidence', 'fixed_tier_on_shelf_evidence_missing',
    'fixed_tier_missing_classification_metrics', 'fixed_tier_high_click_qualification_unknown',
    'fixed_tier_global_exposure_evidence_incomplete', 'fixed_tier_global_rank_missing',
    'fixed_tier_new_listing_evidence_missing',
  ]);
  if (!recoverable.has(observed.reason)) return observed;
  const reviewed = readReviewedWorkbookPrice(context.reviewedWorkbookPriceCapability,row,context.reportDate);
  if (reviewed.applies && !reviewed.ok) return {ok:false,reason:reviewed.reason};
  const prior=context.approvedTiers?.get(key(row));
  if (prior?.conflict || (prior && prior.canonical!==fixedTierCanonicalKey(canonicalOf(row)))) return {ok:false,reason:'reviewed_pricing_tier_conflict'};
  const exposure=num(link?.c7_eps_uv ?? link?.c7EpsUv), clicks=num(link?.c7_goods_uv ?? link?.c7GoodsUv);
  return {
    ok:true,tier:reviewed.applies ? reviewed.tier : prior?.tier ?? 2,
    metrics:{exposure,clicks,cart:num(link?.c7_cart_uv ?? link?.c7CartUv),sales:num(link?.c7_sale_cnt ?? link?.c7SaleCnt),
      rate:exposure>0 && clicks!==null ? clicks/exposure : num(link?.c7_click_rate ?? link?.c7ClickRate)},
    age:link ? inferNewListingShelfAgeDays(link,context.reportDate).value : null,
    recent:link ? isRecentNewListingLink(link,undefined,context.reportDate) : {applies:false,reason:'link_evidence_missing'},
    rank:null,ranking:[],rankBounds:observed.evidence?.rankBounds || null,
    pricingClassificationFallback:{version:'marketing-missing-metrics/2026-09-10.1',
      source:reviewed.applies?'current_reviewed_workbook_tier':prior?'approved_baseline_tier':'ordinary_tier_without_classification',
      ...(prior?{workFingerprint:prior.workFingerprint}:{}),
      reason:observed.reason,observedShelf:shelf(link),
      eligibility:'requires_live_campaign_identity_inventory_and_membership'},
  };
}
...
export function resolveFixedTierPrice(row, context) {
  ...
  const sourceBasis=context?.reviewedBases?.get(fixedTierCanonicalKey(canonicalOf(row)));
  const publishedBasis=(standard.doc.reviewedWorkbookBases || []).find(r=>context.reportDate>=r.effectiveDate && fixedTierCanonicalKey(r.canonical)===fixedTierCanonicalKey(canonicalOf(row)));
  const reviewedBasis=sourceBasis?.conflict || !publishedBasis || sourceBasis?.businessDate>=publishedBasis.businessDate ? sourceBasis : publishedBasis;
  const defaultRule=standard.doc.missingStandardMargins;
  if (!reviewed.applies && !reviewedBasis && !fixed && !inherited && (!defaultRule || (context?.baselineKeys?.has(fixedTierCanonicalKey(canonicalOf(row))) && (row.fixedTierPricing || row.lowEtFastSellerPricePullback?.fixedTierPricing)?.mode!=='missing_standard_full_cost_margin'))) return {applies:false};
  ...
  if (reviewedBasis?.conflict) return blocked('reviewed_pricing_basis_conflict');
  if (!reviewed.applies && !reviewedBasis && inherited && !fixed && inherited.type!=='inherited_full_cost_margins') return blocked('existing_user_standard_requires_latest_reconciliation');
  if (!reviewed.applies && !reviewedBasis && !fixed && !inherited) {
    ...
    if (context.reportDate < '2026-09-10' && (!proof || !/^[a-f0-9]{64}$/.test(proof.evidenceSha256 || '') || !proof.sourceVersion)) return blocked('existing_user_standard_coverage_unverified');
  }
  ...
  const {tier,metrics,age,recent,rank,ranking,rankBounds,pricingClassificationFallback} = classification;
  ...
  let margin=reviewed.applies?null:reviewedBasis ? (reviewedBasis.basis.kind==='full_cost_margin'?reviewedBasis.basis.tiers[tier]:null) : fixed?null:inherited ? (tier===2?inherited.regularMargin:inherited.top5Margin) : defaultRule.tiers[tier];
  if (!reviewed.applies && margin!==null && (!(margin>=0) || !(margin<1))) return blocked('invalid_inherited_margin');
  let target = reviewed.applies ? reviewed.price : exception ? exception.price : reviewedBasis?.basis.kind==='price' ? reviewedBasis.basis.tiers[tier] : fixed && !reviewedBasis ? item.prices[tier] : Math.ceil((cost.fullUnitCostSar/(1-margin)-Number.EPSILON)*100)/100;
  if(inherited && !reviewedBasis && !fixed && !exception && !reviewed.applies && tier===0) {
  ...
  if (!reviewed.applies && !exception && variationPolicy && context.reportDate>=variationPolicy.effectiveDate) {
    ...
    let basis;
    if (reviewedBasis) basis=reviewedBasis.basis;
    else if (fixed) basis={kind:'price',tiers:item.prices};
    else if (inherited) {
      const topPrice=cost.fullUnitCostSar/(1-inherited.top5Margin);
      ...
      // Preserve this approved link's existing Top-5 margin, without changing
      // other tiers, links, activities, or a later revised inherited standard.
      const approvedTop5Exception = item.canonical === 'SK-7028绞肉机'
        && String(row.storeKey ?? row.store_key ?? '').toUpperCase() === 'CX'
        && row.skc === 'sv260716210808025305436'
        && Number(row.activityId) === 53616
        && Math.abs(inherited.top5Margin - .1386) < 1e-9;
      basis={kind:'full_cost_margin',tiers:[1-cost.fullUnitCostSar/highPrice,inherited.top5Margin,inherited.regularMargin],
        minimumMargins:[1-cost.fullUnitCostSar/(cost.productUnitCostSar/.85),approvedTop5Exception ? .1386 : .15,.15]};
  ...
  const evidence = {key:key(row),canonical:item.canonical,reportDate:context.reportDate,metrics,age,newListing:recent,rank,ranking,...(rankBounds?{rankBounds}:{}),...(pricingClassificationFallback?{pricingClassificationFallback}:{}),...(reviewedBasis?{reviewedBasis}:{}),cost,exception:exception || null,...(reviewed.applies?{reviewedWorkbookPrice:reviewed.binding}:{}),...(priceVariation?{priceVariation}:{})};
  if (reviewed.applies) margin=(target-cost.fullUnitCostSar)/target;
  const binding = {schemaVersion:'marketing-fixed-tier-binding/v1',ruleVersion:standard.doc.version,ruleHash:standard.sha256,canonical:item.canonical,tier,price,originalTargetPrice:target,mode:reviewed.applies?'reviewed_workbook_price':reviewedBasis?'inherited_reviewed_workbook_basis':fixed?'fixed_sar':inherited?'inherited_user_standard':'missing_standard_full_cost_margin',marginTarget:margin,inheritedSource:inherited || null,platform,...(priceVariation?{baselinePrice:priceVariation.baselinePrice,priceVariation}:{}),evidenceHash:hash(evidence),evidence};
...
export function verifyPlatformPriceAudit(row,audit) {
  ...
  // The executor clamps the price in two directions and records both. A pure
  // platform maximum caps the price down, while the platform's rate-intercept
  // supply floor raises it to the lowest value SHEIN still accepts. Verifying
  // only the downward clamp rejected every floor-raised row, so an audit the
  // executor had just produced could never be written back to the registry and
  // the recorded special price stayed permanently out of step with the live
  // platform price (2026-09-20: NM/QY SM-825 at 115.62/116.28 vs live 120.01).
  const floorAdjustment=audit.platformFloorAdjustment;
  const floorRaised=floorAdjustment
    && String(floorAdjustment.skc)===String(audit.skc)
    && num(floorAdjustment.requestedPrice)===original
    && num(floorAdjustment.interceptSupplyPrice)>0
    && num(floorAdjustment.adjustedPrice)===num(audit.platformFloorAdjustment.adjustedPrice);
  const actual=floorRaised
    ? Math.round((Math.max(Math.min(original,audit.platformMaximum),num(floorAdjustment.interceptSupplyPrice))+0.01)*100)/100
    : Math.round(Math.min(original,audit.platformMaximum)*100)/100;
  if (audit.actualPrice!==actual || audit.differenceSar!==Math.round((actual-original)*100)/100) return null;
  if (floorRaised && num(audit.platformFloor)!==num(floorAdjustment.interceptSupplyPrice)) return null;
  return audit;
}
...
export async function verifyFixedTierRescue({root, rescue, reportDate}) {
  ...
  const sourceTime=Date.parse(generated),sameDate=Number.isFinite(sourceTime) && new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Shanghai'}).format(new Date(sourceTime))===reportDate;
  // Crossing midnight does not instantly invalidate a recent 7-day snapshot.
  // Live platform membership/stock remain separate mandatory execution checks.
  const recentSnapshot=reportDate>='2026-09-10' && Date.now()-sourceTime<=36*3600000;
  if (!Number.isFinite(sourceTime) || (!sameDate&&!recentSnapshot) || sourceTime>Date.now()+60_000) return {ok:false,reason:'fixed_tier_links_evidence_stale',rows:[]};
  let links=doc;
  if (rescue.sourceRawLinkHistory) {
    ...
    if (!coverage.complete && reportDate<'2026-09-10') return {ok:false,reason:'fixed_tier_global_store_evidence_incomplete',rows:[]};
    links={classificationCoverageComplete:coverage.complete,storeLinks:mergeMarketingLinkRows([...buildLinkRowIndexFromBi(doc).byLinkKey.values()],raw.rows).rows};
  }
