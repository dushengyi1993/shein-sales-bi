...
export function buildFixedTierContext(linksDataDoc, {reportDate = '', standard = loadFixedTierStandard(),costDoc={},baselineDoc={}} = {}) {
  const byKey = new Map();
  ...
  return {standard, reportDate, byKey,costDoc,baselineKeys};
}

export function fullTierCost(canonical,costDoc={}) {
  const wanted=fixedTierCanonicalKey(canonical);
  const keys=[...new Set([...Object.keys(costDoc.trueCostMap || {}),...Object.keys(costDoc.costMap || {})])].filter(k=>fixedTierCanonicalKey(k)===wanted);
  if (keys.length!==1) return {complete:false,reason:'missing_or_ambiguous_product_cost'};
  const k=keys[0],full=costDoc.trueCostMap?.[k] || {},raw=costDoc.costMap?.[k];
...
export function resolveFixedTierPrice(row, context) {
  ...
  if (!fixed && !inherited && (!defaultRule || (context?.baselineKeys?.has(fixedTierCanonicalKey(canonicalOf(row))) && (row.fixedTierPricing || row.lowEtFastSellerPricePullback?.fixedTierPricing)?.mode!=='missing_standard_full_cost_margin'))) return {applies:false};
  ...
  if (inherited && !fixed && inherited.type!=='inherited_full_cost_margins') return blocked('existing_user_standard_requires_latest_reconciliation');
  if (!fixed && !inherited) {
  ...
  let margin=fixed?null:inherited ? (tier===2?inherited.regularMargin:inherited.top5Margin) : defaultRule.tiers[tier];
  if (margin!==null && (!(margin>=0) || !(margin<1))) return blocked('invalid_inherited_margin');
  let target = exception ? exception.price : fixed ? item.prices[tier] : Math.ceil((cost.fullUnitCostSar/(1-margin)-Number.EPSILON)*100)/100;
  if(inherited && !fixed && !exception && tier===0) {
  ...
  const platform=applyPlatformPriceConstraint({targetPrice:target,platformMaximum:max,fullUnitCostSar:cost.fullUnitCostSar});
  if (!platform.ok) return blocked(platform.reason);
  const price=platform.actualPrice;
  const evidence = {key:key(row),canonical:item.canonical,reportDate:context.reportDate,metrics,age,newListing:recent,rank,ranking,cost,exception:exception || null};
  const binding = {schemaVersion:'marketing-fixed-tier-binding/v1',ruleVersion:standard.doc.version,ruleHash:standard.sha256,canonical:item.canonical,tier,price,originalTargetPrice:target,mode:fixed?'fixed_sar':inherited?'inherited_user_standard':'missing_standard_full_cost_margin',marginTarget:margin,inheritedSource:inherited || null,platform,evidenceHash:hash(evidence),evidence};
