...
export {discoverInventoryJournalFiles} from './inventory_journal_discovery.mjs';
...
export async function appendInventoryReconciliationRecord(journalFile, intent, event) {
  const sourceJournal = path.resolve(intent.journalFile || journalFile);
  if (sourceJournal === path.resolve(journalFile)) return appendDurableJournalRecord(journalFile, event);
  ...
  await appendDurableJournalRecord(journalFile, {
    ...
    sourceJournal, sourceSha256: createHash('sha256').update(sourceBytes).digest('hex'),
    intentHash: inventoryIntentHash(intent), event,
  });
}
