...
} from './inventory_journal_discovery.mjs';
...
export const INVENTORY_EXACT_WAREHOUSE_WRITE_PROFILE = 'daily_inventory_exact_warehouse/v1';
...
// A durable intent that an earlier daily run left pending must not freeze its
// store/SKC/SKU scope forever. Its submission outcome is genuinely unknown,
// but every later plan is a fresh owner-authorized decision derived from
// current-day ET, sales and exposure evidence, and its write is an absolute
// OVERWRITE whose terminal proof is an exact post-write readback. The later
// plan therefore supersedes the abandoned intent instead of skipping the
// scope on every future run. This is recorded honestly: the disposition says
// the original effect stays unknown, and only a strictly later plan may use
// it, so same-day uncertainty keeps its duplicate-submission protection.
const INVENTORY_SUPERSEDED_BY_LATER_PLAN_OUTCOME_KEYS = 'disposition,intentId,kind,logicalActionKey,newRunDate,oldRunDate,originalEffectUnknown,recordedAt,supersedeRule';
export const INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION = 'superseded_by_later_plan';
export const INVENTORY_SUPERSEDED_BY_LATER_PLAN_RULE = 'stale-pending-intent-superseded-by-later-daily-plan/v1';
...
// A superseded intent can never be re-submitted: its exact idempotency key is
// tombstoned so no later run can replay the abandoned request itself. A newer
// plan derives a new key from its own plan hash, so the tombstone blocks the
// old request without blocking today's authorized write.
const INVENTORY_IDEMPOTENCY_TOMBSTONE_DISPOSITIONS = new Set([
  INVENTORY_OWNER_CONFIRMED_SAME_TARGET_SUPERSEDE_DISPOSITION,
  INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION,
]);
...
export function inventoryLogicalActionKey({commandId = '', runDate, storeKey, skc, skuCode, targetUsableInventory, policyVersion, authorizationId = ''}) {
  if (commandId) {
    if (!/^[A-Za-z0-9._:-]{1,160}$/.test(commandId)) throw new Error('INVENTORY_COMMAND_ID_INVALID');
    return stableInventoryHash({commandId, store: storeKey, skc, sku: skuCode, target: Number(targetUsableInventory), actionType: 'VI_OVERWRITE_TO_EXACT_USABLE_TARGET'});
  }
  return stableInventoryHash({runDate, store: storeKey, skc, sku: skuCode, target: Number(targetUsableInventory), actionType: 'VI_OVERWRITE_TO_EXACT_USABLE_TARGET', policyVersion, authorizationId});
}
...
export {discoverInventoryJournalFiles, inventoryJournalDomainDirectories};

// An explicit new command may only reconcile another batch read-only. Moving
// the same command to another path must never bypass its exact plan checks.
export function isForeignInventoryIntent(intent, {runDate, commandId = '', journalFile = ''} = {}) {
  if (!intent) return false;
  if (intent.runDate !== runDate) return true;
  return Boolean(commandId && intent.commandId && commandId !== intent.commandId
    && journalFile && intent.journalFile
    && path.resolve(journalFile) !== path.resolve(intent.journalFile));
}

// Owner-authorized aging rule (2026-09-14), shared by every writer.
//
// A durable intent an earlier daily run left pending must not freeze its
// store/SKC/SKU scope forever. Its submission outcome is genuinely unknown,
// but a strictly later plan is a fresh owner-authorized decision derived from
// current evidence, and its write is an absolute OVERWRITE whose terminal
// proof is an exact post-write readback. A strictly later plan may therefore
// supersede the abandoned intent (recording originalEffectUnknown) instead of
// skipping the scope on every future run.
//
// The predicate deliberately requires a STRICTLY earlier run date. Same-day
// uncertainty keeps its duplicate-submission protection: only an intent whose
// live value already equals its recorded target may be closed same-day, and
// that happens through the exact-readback path, not here.
export function selectAgedPendingInventoryIntent(bundle, {
  scope = {},
  runDate = '',
  currentUsableInventory = null,
  currentWarehouseCodes = [],
} = {}) {
  if (!isValidIsoDate(runDate)) return null;
  const normalizedScope = normalizeInventoryWriteScope(scope);
  // Match on the durable item identity (store/SKC/SKU/invType), exactly like
  // the daily executor's recovery scope. Warehouse is deliberately NOT part of
  // this identity: it is proven from the live readback below instead, because
  // an incoming marketing scope carries no warehouse at all.
  const candidates = [...(bundle?.pending?.values?.() || [])].filter(intent => {
    const existing = inventoryScopeFromIntent(intent);
    return existing.storeKey === normalizedScope.storeKey
      && existing.skc === normalizedScope.skc
      && existing.skuCode === normalizedScope.skuCode
      && existing.invType === normalizedScope.invType;
  });
  if (candidates.length !== 1) return null;
  const [intent] = candidates;
  if (!isValidIsoDate(intent.runDate) || intent.runDate >= runDate) return null;
  if (!String(intent.logicalActionKey || '').trim()) return null;
  // An intent whose live value already matches its target is settled by the
  // exact-readback path instead; superseding it would rewrite history.
  const current = Number(currentUsableInventory);
  if (Number.isFinite(current) && current === Number(intent.targetUsableInventory)) return null;
  // A warehouse-scoped intent may only be aged out when the live readback
  // proves the same single warehouse.
  const intentWarehouse = String(inventoryScopeFromIntent(intent).warehouseCode || '').toUpperCase();
  if (intentWarehouse) {
    const codes = (currentWarehouseCodes || [])
      .map(code => String(code || '').toUpperCase()).filter(Boolean);
    if (codes.length !== 1 || codes[0] !== intentWarehouse) return null;
  }
  return intent;
}
...
function inventoryOverwriteComputationProfile(intent, {requireCurrent = false} = {}) {
  ...
  if (hasVersion && ![INVENTORY_OVERWRITE_COMPUTATION_VERSION, INVENTORY_LEGACY_LOCKED_ONLY_COMPUTATION_VERSION].includes(version)) {
  ...
  try {
    return {
      ...
      quantities: hasVersion
        ? [computeInventoryOverwriteQuantity(Number(intent?.targetUsableInventory), intent?.before || {}, version)]
        : [
            computeInventoryOverwriteQuantity(Number(intent?.targetUsableInventory), intent?.before || {}, INVENTORY_LEGACY_LOCKED_ONLY_COMPUTATION_VERSION),
            computeLegacyInventoryOverwriteQuantity(Number(intent?.targetUsableInventory), intent?.before || {}),
          ],
    };
  } catch { return {valid: false, quantities: []}; }
}
...
function inventoryIntentImmutableMismatch(intent) {
  ...
  const expectedLogicalActionKey = inventoryLogicalActionKey({
  ...
  const exactWarehouseProfile = String(intent?.inventoryWriteProfile || '') === INVENTORY_EXACT_WAREHOUSE_WRITE_PROFILE;
  ...
  const warehouseCodeIsValid = maintenanceInventoryProfile || exactWarehouseProfile
  ...
  const checks = [
    ...
    [!exactWarehouseProfile || (Array.isArray(before?.warehouseCodes) && before.warehouseCodes.length === 1
      && normalizedScopeValue(before.warehouseCodes[0], {upper: true}) === requestWarehouseCode), 'before.warehouseCodes'],
  ];
...
function validateManualResolutionLiveBaseline(value, {journalFile, lineNumber, intentScope} = {}) {
  ...
  if (value.totalInventoryQuantity !== value.totalUsableInventory + value.totalLockedQuantity + value.temporaryInventoryQuantity) fail('conservation');
  return value;
}
...
function validateInventoryTerminalOutcome(entry, intent, {journalFile, lineNumber} = {}) {
  ...
  if (disposition === INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION) {
    if (Object.keys(entry).sort().join(',') !== INVENTORY_SUPERSEDED_BY_LATER_PLAN_OUTCOME_KEYS) fail('laterPlanSupersedeShape');
    if (entry.originalEffectUnknown !== true) fail('originalEffectUnknown');
    if (entry.supersedeRule !== INVENTORY_SUPERSEDED_BY_LATER_PLAN_RULE) fail('supersedeRule');
    if (!isValidIsoDate(entry.oldRunDate) || entry.oldRunDate !== intent.runDate) fail('oldRunDate');
    // Strictly later, never the same day: an uncertain same-day submission
    // must keep the duplicate-submission protection it already has.
    if (!isValidIsoDate(entry.newRunDate) || entry.newRunDate <= entry.oldRunDate) fail('newRunDate');
  }
}
...
// A sealed journal is the immutable artifact of an already published batch, so
// nothing may be appended to it. Shared by the plain append and the
// reconciliation writer, whose duplicate pre-flight must not be able to bypass
// this boundary.
async function assertJournalNotSealed(journalFile) {
  try {
    await fs.access(journalFile + '.sealed.json');
    throw new Error('INVENTORY_JOURNAL_SEALED:write reconciliation into a new batch journal');
  } catch (error) { if (error.code !== 'ENOENT') throw error; }
}

export async function appendDurableJournalRecord(journalFile, entry, {allowManualResolution = false} = {}) {
  const release = await acquireCrossProcessTicketLock(journalFile + '.publication.lock');
  try {
  await assertJournalNotSealed(journalFile);
...
// Fields that legitimately differ when two separate runs record the SAME
// closure of the same intent: recordedAt is when that run wrote its own row,
// and for a later-plan supersede newRunDate is the run that superseded it. Every
// other field is part of the closure identity, so two rows that disagree on any
// of them remain a hard conflict.
const CROSS_JOURNAL_CLOSURE_VOLATILE_EVENT_FIELDS = Object.freeze(['recordedAt', 'newRunDate']);

function crossJournalClosureIdentity(reference) {
  const event = {...(reference?.event || {})};
  for (const field of CROSS_JOURNAL_CLOSURE_VOLATILE_EVENT_FIELDS) delete event[field];
  return {
    schemaVersion: String(reference?.schemaVersion || ''),
    sourceJournal: path.resolve(String(reference?.sourceJournal || '')),
    sourceSha256: String(reference?.sourceSha256 || '').toLowerCase(),
    intentHash: String(reference?.intentHash || '').toLowerCase(),
    event,
  };
}

export function crossJournalClosureHash(reference) {
  return stableInventoryHash(crossJournalClosureIdentity(reference));
}

export function sameCrossJournalClosure(left, right) {
  return crossJournalClosureHash(left) === crossJournalClosureHash(right);
}

/**
 * Find every durable record that already terminates one intent, scanning the
 * whole managed journal domain rather than only the writer's own output
 * directory.
 *
 * An intent can be closed by a different producer than the one currently
 * planning: the daily executor writes into a run-scoped journal, and the ET
 * low-inventory guard keeps its own results directory. A producer whose
 * discovery scope is narrower than the domain therefore used to see a closed
 * intent as pending and record a second, redundant closure.
 *
 * The scan reads and revalidates only what it needs (a torn tail or an
 * unparseable line still fails closed), because the whole-domain lifecycle
 * reader deliberately fails on the historical duplicate this scan exists to
 * prevent from being created again.
 */
export async function findInventoryIntentClosures(intentId, {sourceJournal = '', journalFiles = []} = {}) {
  const id = String(intentId || '').trim();
  if (!id) throw new Error('INVENTORY_JOURNAL_INTENT_ID_REQUIRED');
  const source = String(sourceJournal || '').trim();
  if (!source) throw new Error('INVENTORY_JOURNAL_SOURCE_REQUIRED');
  const sourcePath = path.resolve(source);
  const closures = [];
  for (const journalFile of [...new Set((journalFiles || []).map(file => path.resolve(file)))]) {
    const isSourceJournal = journalFile === sourcePath;
    let text;
    try {
    ...
    for (let index = 0; index < lines.length; index += 1) {
      let entry;
      try { entry = JSON.parse(lines[index]); } catch { throw new Error(`INVENTORY_JOURNAL_INVALID_LINE:${index + 1}`); }
      if (entry?.kind === 'cross_journal_resolution') {
        if (String(entry.event?.intentId || '') !== id) continue;
        if (path.resolve(String(entry.sourceJournal || '')) !== sourcePath) continue;
        closures.push({journalFile, lineNumber: index + 1, closure: 'cross_journal_resolution', reference: entry});
        continue;
      }
      if (!isSourceJournal) continue;
      if (entry?.kind === 'write_outcome' && String(entry.intentId || '') === id) {
        closures.push({journalFile, lineNumber: index + 1, closure: 'source_write_outcome', reference: entry});
        continue;
      }
      if (entry?.kind === INVENTORY_MANUAL_RESOLUTION_KIND && String(entry.intentId || '') === id) {
        closures.push({journalFile, lineNumber: index + 1, closure: 'source_manual_resolution', reference: entry});
      }
    }
  }
  return closures;
...
export async function appendInventoryReconciliationRecord(journalFile, intent, event, {journalFiles = [], additionalDirectories = []} = {}) {
  const target = path.resolve(journalFile);
  const sourceJournal = path.resolve(intent.journalFile || journalFile);
  await assertJournalNotSealed(target);
  if (sourceJournal === target) return appendDurableJournalRecord(journalFile, event, {});
  const sourceBytes = await fs.readFile(sourceJournal);
  const candidate = {
  ...
  const domain = journalFiles?.length
    ? [...new Set([target, ...journalFiles.map(file => path.resolve(file))])]
    : await discoverInventoryJournalFiles(target, {
      includeAll: true,
      additionalDirectories: inventoryJournalDomainDirectories({
        additionalDirectories: [path.dirname(sourceJournal), ...additionalDirectories],
      }),
    });
  const existing = await findInventoryIntentClosures(intent.intentId, {sourceJournal, journalFiles: domain});
  if (existing.length) {
    // The intent is already terminal in the durable domain. Appending a second
    // closure cannot add information and makes the aggregate reader refuse
    // every later run, so record the evidence and leave the journals alone.
    return {
      appended: false,
      skipped: true,
      reason: 'intent_already_terminal_in_journal_domain',
      intentId: String(intent.intentId || ''),
      sourceJournal,
      journalFile: target,
      closures: existing.map(({journalFile: file, lineNumber, closure, reference}) => ({
        journalFile: file,
        lineNumber,
        closure,
        matchesProposedClosure: closure === 'cross_journal_resolution' && sameCrossJournalClosure(reference, candidate),
      })),
    };
  }
  await appendDurableJournalRecord(journalFile, candidate);
  return {
    appended: true,
    skipped: false,
    reason: '',
...
export function buildInventoryBaselineAdoption({commandId, logicalActionKey, fence, scope, before, capturedAt, recordedAt = new Date().toISOString()}) {
  const event = {
    kind: 'manual_baseline_adoption', schemaVersion: 'inventory-manual-baseline-adoption/v1',
    commandId, logicalActionKey, idempotencyKey: `bi-inv-${logicalActionKey.slice(0, 42)}`,
    resolutionId: fence?.event?.resolutionId, historicalIntentId: fence?.event?.intentId,
    originalEffectUnknown: true, scope: normalizeInventoryWriteScope(scope),
    before, capturedAt, recordedAt,
  };
  validateInventoryBaselineAdoption(event);
  return event;
}

function validateInventoryBaselineAdoption(event) {
  const fail = reason => { throw new Error('INVENTORY_JOURNAL_BASELINE_ADOPTION_INVALID:' + reason); };
  if (event.schemaVersion !== 'inventory-manual-baseline-adoption/v1' || event.originalEffectUnknown !== true
    || !/^[A-Za-z0-9._:-]{1,160}$/.test(event.commandId || '') || !event.resolutionId || !event.historicalIntentId
    || !/^[a-f0-9]{64}$/.test(event.logicalActionKey || '') || event.idempotencyKey !== `bi-inv-${event.logicalActionKey.slice(0, 42)}`) fail('identity');
  const scope = normalizeInventoryWriteScope(event.scope);
  if (!scope.storeKey || !scope.skc || !scope.skuCode || !scope.warehouseCode || scope.invType !== 'VI') fail('scope');
  if (!isValidTimestamp(event.capturedAt) || !isValidTimestamp(event.recordedAt)
    || Date.parse(event.recordedAt) - Date.parse(event.capturedAt) < 0
    || Date.parse(event.recordedAt) - Date.parse(event.capturedAt) > 60_000) fail('fresh readback');
  const before = event.before;
  if (!before || before.ok !== true || before.stockRowMissing !== false || before.skuCode !== scope.skuCode
    || before.warehouseCodes?.length !== 1 || before.warehouseCodes[0] !== scope.warehouseCode
    || !['totalInventoryQuantity', 'totalUsableInventory', 'totalLockedQuantity', 'temporaryInventoryQuantity'].every(field => Number.isSafeInteger(before[field]) && before[field] >= 0)
    || before.totalInventoryQuantity !== before.totalUsableInventory + before.totalLockedQuantity + before.temporaryInventoryQuantity) fail('baseline');
}

export async function readInventoryIntentLifecycle(journalFile, {strict = false, maxRunDate = '', allowCrossJournalReferences = false, snapshotFile = ''} = {}) {
  const crossJournalResolutions = [];
  const baselineAdoptions = new Map();
  ...
  let text = '';
  try {
    text = await fs.readFile(snapshotFile || journalFile, 'utf8');
  } catch (error) {
    if (error?.code === 'ENOENT') return {intents, pending, terminalOutcomes, manualResolutions, fences, tombstonedIdempotencyKeys, crossJournalResolutions, baselineAdoptions};
    throw error;
  }
  if (text && !text.endsWith('\n')) throw new Error('INVENTORY_JOURNAL_TORN_TAIL');
  const lines = text.split(/\r?\n/).filter(Boolean);
    ...
    try {
      ...
      if (entry?.kind === 'intent') {
        ...
        if (entry?.intentId && INVENTORY_TERMINAL_DISPOSITIONS.has(entry?.disposition)) {
          ...
          if (INVENTORY_IDEMPOTENCY_TOMBSTONE_DISPOSITIONS.has(entry.disposition)) {
            const intent = intents.get(entry.intentId);
      ...
      } else if (entry?.kind === 'manual_baseline_adoption') {
        if (!allowCrossJournalReferences) throw new Error('INVENTORY_JOURNAL_CROSS_REFERENCE_REQUIRES_AGGREGATE');
        validateInventoryBaselineAdoption(entry);
        if (baselineAdoptions.has(entry.idempotencyKey)) throw new Error('INVENTORY_JOURNAL_BASELINE_ADOPTION_INVALID:duplicate key');
        baselineAdoptions.set(entry.idempotencyKey, entry);
      } else if (entry?.kind === 'cross_journal_resolution') {
        if (!allowCrossJournalReferences) throw new Error('INVENTORY_JOURNAL_CROSS_REFERENCE_REQUIRES_AGGREGATE');
        if (entry.schemaVersion !== 'inventory-cross-journal-resolution/v1'
          || Object.keys(entry).sort().join(',') !== 'event,intentHash,kind,schemaVersion,sourceJournal,sourceSha256'
          || !entry.sourceJournal || !/^[a-f0-9]{64}$/.test(entry.sourceSha256)
          || !/^[a-f0-9]{64}$/.test(entry.intentHash) || entry.event?.kind !== 'write_outcome') {
          throw new Error('INVENTORY_JOURNAL_CROSS_REFERENCE_INVALID');
        }
        crossJournalResolutions.push({...entry, journalFile, lineNumber: index + 1});
  ...
  return {intents, pending, terminalOutcomes, manualResolutions, fences, tombstonedIdempotencyKeys, crossJournalResolutions, baselineAdoptions};
}
...
export async function readInventoryIntentJournals(journalFiles, {
  ...
  currentJournalFile = '',
  quarantineHistoricalDanglingSupersedes = false,
  journalSnapshots = new Map(),
} = {}) {
  const baselineAdoptions = new Map();
  ...
  // Closures applied through a cross-journal reference, keyed by the closed
  // intent. Needed to tell a redundant repeat of one closure apart from two
  // conflicting closures of the same intent.
  const crossJournalClosures = new Map();
  const duplicateCrossJournalResolutions = [];
  const currentJournal = String(currentJournalFile || '').trim()
    ? path.resolve(currentJournalFile)
    : '';
  const quarantinedSupersedes = [];
  if (quarantineHistoricalDanglingSupersedes
    && (!currentJournal || !maxRunDate || !files.includes(currentJournal))) {
    throw new Error('INVENTORY_JOURNAL_QUARANTINE_CONFIG_INVALID:currentJournalFile/maxRunDate');
  }
  for (const journalFile of files) {
    ...
    const lifecycle = await readInventoryIntentLifecycle(journalFile, {strict: true, maxRunDate, allowCrossJournalReferences: true, snapshotFile: journalSnapshots.get(journalFile) || ''});
    ...
    for (const [intentId, rawIntent] of lifecycle.intents.entries()) {
      ...
      if (INVENTORY_IDEMPOTENCY_TOMBSTONE_DISPOSITIONS.has(outcome?.disposition) && rawIntent.idempotencyKey) {
  ...
  for (const record of records) {
    for (const [idempotencyKey, adoption] of record.baselineAdoptions || []) {
      const resolution = [...manualResolutions.values()].find(event => event.resolutionId === adoption.resolutionId && event.intentId === adoption.historicalIntentId);
      const original = intentById.get(adoption.historicalIntentId)?.intent;
      if (!resolution || !original || original.commandId === adoption.commandId || original.idempotencyKey === idempotencyKey
        || inventoryScopeFieldMismatch(adoption.scope, resolution.scope)
        || Date.parse(adoption.capturedAt) < Date.parse(resolution.recordedAt)
        || baselineAdoptions.has(idempotencyKey)) throw new Error('INVENTORY_JOURNAL_BASELINE_ADOPTION_INVALID:resolution binding');
      baselineAdoptions.set(idempotencyKey, adoption);
    }
    for (const reference of record.crossJournalResolutions || []) {
      const event = reference.event;
      const original = intentById.get(event.intentId);
      const invalid = reason => { throw new Error('INVENTORY_JOURNAL_CROSS_REFERENCE_INVALID:' + reason); };
      if (!original || original.intent.journalFile !== path.resolve(reference.sourceJournal)
        || inventoryIntentHash(original.intent) !== reference.intentHash) invalid('intent binding');
      if (createHash('sha256').update(await fs.readFile(journalSnapshots.get(path.resolve(reference.sourceJournal)) || reference.sourceJournal)).digest('hex') !== reference.sourceSha256) invalid('source bytes changed');
      const previousClosure = crossJournalClosures.get(original.key);
      if (previousClosure) {
        // Two journals recorded the same closure of one intent. Producers whose
        // journal discovery was narrower than the managed domain each closed
        // the same abandoned intent, and the second row was already redundant
        // when it was written. Identical closure semantics (the identity ignores
        // only the recording time and the superseding run date) is therefore an
        // idempotent no-op; treating it as a hard conflict would keep every
        // later inventory run permanently fail-closed. Any other disagreement
        // between the two rows is still a conflict.
        validateInventoryTerminalOutcome(event, original.intent, {journalFile: reference.journalFile, lineNumber: reference.lineNumber});
        if (!sameCrossJournalClosure(previousClosure.reference, reference)) invalid('duplicate resolution');
        duplicateCrossJournalResolutions.push({
          intentId: String(event.intentId || ''),
          sourceJournal: path.resolve(String(reference.sourceJournal || '')),
          closureHash: crossJournalClosureHash(reference),
          firstJournalFile: previousClosure.journalFile,
          firstLineNumber: previousClosure.lineNumber,
          duplicateJournalFile: reference.journalFile,
          duplicateLineNumber: reference.lineNumber,
        });
        continue;
      }
      if (terminalOutcomes.has(original.key) || manualResolutions.has(original.key)) invalid('duplicate resolution');
      validateInventoryTerminalOutcome(event, original.intent, {journalFile: reference.journalFile, lineNumber: reference.lineNumber});
      // A stale intent almost always lives in an older day's journal, so the
      // later plan can only close it through this cross-journal reference.
      // readback_matched/rejected close a scope by readback; the later-plan
      // supersede closes an abandoned one so the current plan may write it.
      if (!['readback_matched', 'rejected', INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION].includes(event.disposition)) invalid('unsupported disposition');
      terminalOutcomes.set(original.key, {...event, journalFile: original.intent.journalFile, resolutionJournalFile: reference.journalFile});
      crossJournalClosures.set(original.key, {
        reference,
        journalFile: reference.journalFile,
        lineNumber: reference.lineNumber,
        resolutionJournalFile: reference.journalFile,
      });
      if (INVENTORY_IDEMPOTENCY_TOMBSTONE_DISPOSITIONS.has(event.disposition) && original.intent.idempotencyKey) {
        if (tombstonedIdempotencyKeys.has(original.intent.idempotencyKey)) invalid('duplicate idempotency tombstone');
        tombstonedIdempotencyKeys.set(original.intent.idempotencyKey, {
          key: original.key,
          intent: original.intent,
          event: {...event, journalFile: original.intent.journalFile, resolutionJournalFile: reference.journalFile},
        });
      }
      pending.delete(original.key);
      const originalRecord = records.find(row => row.journalFile === original.intent.journalFile);
      originalRecord.pending.delete(event.intentId);
      originalRecord.terminalOutcomes.set(event.intentId, event);
      const scopeKey = inventoryIntentScopeKey(original.intent);
      pendingByScope.set(scopeKey, (pendingByScope.get(scopeKey) || []).filter(intent => intent.intentId !== event.intentId));
    }
  }
  for (const [key, outcome] of terminalOutcomes.entries()) {
    ...
    if (!olderIntent || !referenced) {
      const canQuarantine = quarantineHistoricalDanglingSupersedes
        && olderIntent
        && !referenced
        && outcome.journalFile !== currentJournal
        && String(olderIntent.runDate || '') < maxRunDate;
      if (!canQuarantine) fail('referencedIntentMissing');

      // Some legacy journals contain a supersede record whose referenced
      // later intent was never persisted.  Do not accept that record as a
      // terminal success, but also do not let one old SKU block every current
      // inventory readback.  Restore the old intent as an item-scoped pending
      // fence in the operational read model; the append-only journal remains
      // untouched and the invalid audit row stays visible here.
      terminalOutcomes.delete(key);
      pending.set(key, olderIntent);
      const scopeKey = inventoryIntentScopeKey(olderIntent);
      if (!pendingByScope.has(scopeKey)) pendingByScope.set(scopeKey, []);
      if (!pendingByScope.get(scopeKey).some(intent => intent.intentId === olderIntent.intentId)) {
        pendingByScope.get(scopeKey).push(olderIntent);
      }
      const record = records.find(candidate => candidate.journalFile === outcome.journalFile);
      const rawIntent = record?.intents?.get(outcome.intentId);
      record?.terminalOutcomes?.delete(outcome.intentId);
      if (rawIntent) record.pending.set(outcome.intentId, rawIntent);
      quarantinedSupersedes.push({
        journalFile: outcome.journalFile,
        intentId: outcome.intentId,
        supersededByIntentId: outcome.supersededByIntentId,
        scopeKey,
        reason: 'referencedIntentMissing',
      });
      continue;
    }
  ...
  return {
    ...
    pendingByScope,
    quarantinedSupersedes,
    duplicateCrossJournalResolutions,
  };
}
...
// A pre-submit business exclusion is auditable only when the exact result was
// durably recorded and the complete journal domain contains no relevant write.
// A generic error/blocked row is deliberately not sufficient evidence.
export function isInventoryPreSubmitExclusion({row, planRow, planHash, commandId = '', lifecycle, entries = []}) {
  const proof = row?.preSubmitExclusion;
  if (row?.state !== 'pre_submit_blocked' || !planRow || proof?.schemaVersion !== 'inventory-pre-submit-exclusion/v1'
    || proof.planHash !== planHash || proof.commandId !== commandId || proof.inventoryPostAttempted !== false
    || !['same_store_on_shelf_changed', 'sold_out_superseded'].includes(proof.reasonCode)
    || !Number.isFinite(Date.parse(proof.capturedAt))
    || !Array.isArray(proof.plannedOnShelfSkcs) || !Array.isArray(proof.observedOnShelfSkcs)
    || !proof.observedOnShelfSkcs.every(x => typeof x === 'string' && x)
    || stableInventoryHash(proof.plannedOnShelfSkcs) !== stableInventoryHash([...(planRow.sameStoreOnShelfSkcs || [])].sort())
    || (proof.reasonCode === 'same_store_on_shelf_changed' && stableInventoryHash(proof.plannedOnShelfSkcs) === stableInventoryHash(proof.observedOnShelfSkcs))
    || (proof.reasonCode === 'sold_out_superseded' && (String(planRow.shelfStatusCode) !== '3' || !proof.observedOnShelfSkcs.length))
    || ['storeKey', 'skc', 'skuCode', 'targetUsableInventory', 'ruleClass'].some(key => row[key] !== planRow[key])
    || (row.writes?.length || row.intentId || row.idempotencyKey || row.logicalActionKey)) return false;
  if (!lifecycle?.intents || !lifecycle?.pending) return false;
  const sameScope = intent => intent.storeKey === row.storeKey && intent.skc === row.skc && intent.skuCode === row.skuCode;
  if ([...lifecycle.pending.values()].some(sameScope)
    || [...lifecycle.intents.values()].some(intent => sameScope(intent) && (intent.planHash === planHash || (commandId && intent.commandId === commandId)))) return false;
  return entries.some(entry => entry.kind === 'result' && entry.planHash === planHash
    && stableInventoryHash(entry.row) === stableInventoryHash(row));
}
...
export function findInventoryWriteFence(bundle, {scope = {}, idempotencyKey = ''} = {}) {
  ...
  for (const candidate of bundle?.fences?.values?.() || []) {
    ...
    if (inventoryWriteScopesOverlap(normalized, event?.scope || {})) {
      const adoption = key ? bundle?.baselineAdoptions?.get(key) : null;
      if (adoption && adoption.resolutionId === event.resolutionId
        && !inventoryScopeFieldMismatch(normalized, adoption.scope)) continue;
      return {
...
export async function submitDurableInventoryWriteOnce({
  ...
  const dispatchOnce = async () => {
    ...
    try {
      ...
      if (error?.code === 'INVENTORY_WRITE_PENDING_CONFLICT') {
        const pendingScope = inventoryScopeFromIntent(error.pendingIntent || {});
        error.inventoryAdmissionRejection = {
          ...
          conflict: {
            intentId: String(error.pendingIntent?.intentId || ''),
            logicalActionKey: String(error.pendingIntent?.logicalActionKey || ''),
            scope: pendingScope,
          },
        };
