...
export async function submitDurableInventoryWriteOnce({
  ...
  const isInventoryV2 = String(intent?.request?.method || 'POST').toUpperCase() === 'POST'
    && String(intent?.request?.pathname || '') === '/open-api/stock/change-inventory/v2';
  if (isInventoryV2 && typeof assertInventoryAdmission !== 'function') {
  ...
  const assertFence = async () => {
    ...
    assertInventoryWriteAllowed(bundle, {
      scope: inventoryScope || inventoryScopeFromIntent(intent),
  ...
  const dispatchOnce = async () => {
    if (isInventoryV2) await assertInventoryAdmission();
    await assertFence();
