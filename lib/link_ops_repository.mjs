...
export function createLinkOpsPostgresRepository({pool, client, env = process.env} = {}) {
  ...
  async function createSession(session, {
    ...
    const messages = Array.isArray(sourceRecord.messages) ? sourceRecord.messages.map(message => jsonClone(message)) : [];
    ...
    const requestHash = linkOpsPayloadHash({
      ...
      session: clean,
      messages: messages.map(message => recordWithoutRepositoryMetadata(message)),
      sortOrder: normalizedSortOrder,
    });
    const created = await transaction(`session:${sessionId}:${idem || 'create'}`, async activeClient => {
      const prior = await readIdempotency(activeClient, scope, idem);
      const replay = replayIdempotent(prior, requestHash, scope, idem);
      if (replay) return replay;
      const result = await activeClient.query({
      ...
      await appendEventInTransaction(activeClient, {
        ...
        payload: {revision: inserted.repositoryRevision, payloadHash, importBatchId: importBatchId || null},
      });
      await writeIdempotency(activeClient, {
    ...
    for (let index = 0; index < messages.length; index += 1) {
      const messageId = requiredText(messages[index].id || messages[index].messageId, `session.messages[${index}].id`);
      await appendMessage(sessionId, messages[index], {
        ownerUser: owner,
        actorUser: actor,
        sequenceNo: index,
        idempotencyKey: `session-create:${sessionId}:${messageId}`,
        importBatchId,
      });
    }
  ...
  async function appendMessage(chatSessionId, message, {
    ...
    return transaction(`session:${sessionId}:message`, async activeClient => {
      ...
      if (nextSequence === null || nextSequence === undefined) {
        ...
        nextSequence = Number(sequence.rows?.[0]?.sequence_no || 0);
      }
      nextSequence = boundedInteger(nextSequence, {fallback: 0, min: 0, label: 'sequenceNo'});
      const result = await activeClient.query({
