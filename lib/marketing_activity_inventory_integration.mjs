...
export async function executeInventoryAwareActivity({
  ...
  const adapter = targets.length ? await adapterFactory({root, storeKey, transactionHash, execute: true}) : {
    resolveTargets: async () => [], acquireLock: async () => async () => {},
    readStock: async () => { throw new Error('No inventory target'); },
    writeStock: async () => { throw new Error('No inventory target'); },
  };
  const resolved = await adapter.resolveTargets(targets);
  return await runActivityInventoryTransaction({
    ...
    readEnrollment,
    validateEnrollment,
    readMutationEvidence,
  });
}

export function classifyActivityInventoryFailureStatus(transaction) {
  if (transaction?.ok === true) return '';
  if (transaction?.currentTransactionUnsubmitted === true
    && transaction?.inventoryAdmissionRejections?.length > 0
    && transaction.inventoryAdmissionRejections.every(isPreDurablePendingScopeRejection)) {
    return 'inventory_admission_scope_blocked';
  }
  if (transaction?.writeAttempted === true && transaction?.safe !== true) return 'inventory_transaction_restore_failed';
  if (transaction?.submitMutationState === 'not_started') return 'pre_submit_failed';
  if (transaction?.submitMutationState === 'unknown') return 'activity_submission_status_unknown';
  if (transaction?.submitMutationState === 'started') return 'submitted_without_exact_readback';
  return transaction?.writeAttempted === true && transaction?.safe !== true
    ? 'inventory_transaction_restore_failed'
    : 'inventory_transaction_or_enrollment_blocked';
}

function isPreDurablePendingScopeRejection(value) {
...
export async function executeLimitedDiscountWithInventoryTransaction({
  ...
  const transaction = await executeInventoryAwareActivity({
    ...
    transactionHash,
    adapterFactory,
    submit: async context => await runSubmit(context),
    readMutationEvidence,
