...
function inferMachinePolicy(ruleKey) {
...
export function normalizeOwnerKnowledgeExperience(input = {}, options = {}) {
  const text = cleanText(input.text || input.statement || input.content || input.message);
  ...
  const sourceAt = timestamp(input.sourceAt || input.at || options.sourceAt);
  ...
  const activation = requestedActivation === 'active' && explicitDurable
    ? 'active'
  ...
  return {
    ...
    scope,
    tags,
    machinePolicy: input.machinePolicy && typeof input.machinePolicy === 'object' && !Array.isArray(input.machinePolicy)
      ? input.machinePolicy
      : inferMachinePolicy(ruleKey),
    source: {
      kind: sourceKind,
      id: sourceId,
      at: sourceAt,
