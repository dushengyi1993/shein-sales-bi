...
export function decideDailyInventoryReplenishment({
  ...
  c7SaleCount,
  allStoreSoldOutBootstrapSeed = false,
  allStoreSoldOutBootstrapLocked = false,
  policy,
} = {}) {
  ...
  if (allStoreSoldOutBootstrapSeed === true || allStoreSoldOutBootstrapLocked === true) {
    const bootstrapTarget = Number(policy?.allStoreSoldOutBootstrap?.targetUsableInventory ?? 10);
    if (!Number.isInteger(bootstrapTarget) || bootstrapTarget < 1 || bootstrapTarget > et) {
      return {action: 'block', reason: 'all_store_sold_out_bootstrap_target_invalid', etSellableInventory: et};
    }
    return {
      action: 'set_exact',
      reason: allStoreSoldOutBootstrapLocked
        ? 'all_store_sold_out_bootstrap_locked_seed'
...
function finiteRankingNumber(value, field) {
  if (value === null || value === undefined || value === '') throw new Error(`missing bootstrap ranking evidence: ${field}`);
  const numeric = Number(value);
  if (!Number.isFinite(numeric)) throw new Error(`missing bootstrap ranking evidence: ${field}`);
  return numeric;
}

export function compareAllStoreSoldOutBootstrapCandidates(left, right) {
  for (const field of ['c7Exposure', 'c7GoodsVisitors', 'c7SaleCount']) {
    const delta = finiteRankingNumber(right?.[field], field) - finiteRankingNumber(left?.[field], field);
    if (delta) return delta;
  }
  return String(left?.storeKey || '').localeCompare(String(right?.storeKey || ''))
    || String(left?.skc || '').localeCompare(String(right?.skc || ''));
}

export function selectAllStoreSoldOutBootstrapSeed(rows, policy) {
  if (policy?.allStoreSoldOutBootstrap?.enabled !== true) return {seed: null, storeCandidates: [], duplicates: []};
  const candidates = (rows || []).map(row => ({...row}));
  for (const row of candidates) {
    if (!row.storeKey || !row.skc) throw new Error('bootstrap candidate identity is incomplete');
    finiteRankingNumber(row.c7Exposure, 'c7Exposure');
    finiteRankingNumber(row.c7GoodsVisitors, 'c7GoodsVisitors');
    finiteRankingNumber(row.c7SaleCount, 'c7SaleCount');
  }
  const byStore = new Map();
  for (const row of candidates) {
    const storeKey = String(row.storeKey).toUpperCase();
    if (!byStore.has(storeKey)) byStore.set(storeKey, []);
    byStore.get(storeKey).push(row);
  }
  const storeCandidates = [];
  const duplicates = [];
  for (const group of byStore.values()) {
    group.sort(compareAllStoreSoldOutBootstrapCandidates);
    storeCandidates.push(group[0]);
    duplicates.push(...group.slice(1));
  }
  storeCandidates.sort(compareAllStoreSoldOutBootstrapCandidates);
  const maxSeeds = Number(policy?.allStoreSoldOutBootstrap?.maxSeedsPerCanonical ?? 1);
  if (maxSeeds !== 1) throw new Error('all-store sold-out bootstrap must select exactly one seed');
  return {seed: storeCandidates[0] || null, storeCandidates, duplicates};
}

export function normalizeOpenApiProductCatalog(rows) {
  const flat = [];
  for (const row of rows || []) {
    const skcs = Array.isArray(row?.skcList) ? row.skcList : [row];
    for (const skc of skcs) {
      const skuValues = Array.isArray(skc?.skuList)
        ? skc.skuList.map(item => item?.skuCode)
        : Array.isArray(skc?.skuCodeList)
          ? skc.skuCodeList
          : Array.isArray(skc?.skuCodes) ? skc.skuCodes : [];
      flat.push({
        spuName: String(row?.spuName || row?.spu || '').trim(),
        skcName: String(skc?.skcName || skc?.skc || '').trim(),
        supplierCode: String(skc?.supplierCode || '').trim(),
        skuCodeList: [...new Set(skuValues.map(value => String(value || '').trim()).filter(Boolean))].sort(),
      });
    }
  }
  return flat.filter(row => row.spuName && row.skcName && row.supplierCode)
    .sort((left, right) => left.spuName.localeCompare(right.spuName)
      || left.skcName.localeCompare(right.skcName)
      || left.supplierCode.localeCompare(right.supplierCode)
      || JSON.stringify(left.skuCodeList).localeCompare(JSON.stringify(right.skuCodeList)));
}
