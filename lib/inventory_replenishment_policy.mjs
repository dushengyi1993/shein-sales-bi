...
/**
 * Index key for same-store on-shelf sibling evidence. The store key is
 * upper-cased and the match key is the alias-aware inventory identity, so the
 * planner and the executor address the same bucket for the same store+product
 * and can never disagree through case folding or collapse two products that
 * config/product_aliases.json keeps separate.
 */
export function sameStoreOnShelfIndexKey(storeKey, matchKey) {
  return `${String(storeKey || '').toUpperCase()}::${String(matchKey || '').toUpperCase()}`;
}

/**
 * Portal link-coverage matrix rows publish their per-link four-state as
 * `skc:状态` pairs joined by " / " (scripts/fetch_shein_links.mjs
 * buildCoverageRows -> fact.product_store_coverage.skc_list). Only 已上架
 * (ON_SHELF) is on-shelf here: 已售罄 (SOLD_OUT) is a link that is still listed
 * but has no sellable inventory, and the inventory policy deliberately does not
 * treat it as an on-shelf sibling.
 */
export function listCoverageSkcStatuses(value) {
  const rows = [];
  for (const part of String(value || '').split(' / ')) {
    const separator = part.lastIndexOf(':');
    if (separator <= 0) continue;
    const skc = part.slice(0, separator).trim();
    const statusName = part.slice(separator + 1).trim();
    if (!skc || !statusName) continue;
    rows.push({skc, statusName});
  }
  return rows;
}

/**
 * Builds the same-store on-shelf sibling index that BOTH the daily planner and
 * the guarded executor must use. Deriving it from a different evidence base on
 * each side produced a permanent, false
 * `same_store_on_shelf_changed` pre-submit exclusion: the plan rows come from
 * the full per-store OpenAPI product snapshot, while
 * `linksData.storeLinks` is only a per-store ranked *presentation* sample
 * (portal LIMIT), so every on-shelf sibling outside that sample looked like a
 * post-plan change on every single run.
 *
 * Evidence = four-state on-shelf links from `storeLinks` unioned with the
 * complete per store+canonical link-coverage matrix `skc_list`. The OpenAPI
 * product cache is deliberately NOT used as a sibling source: its product-list
 * shelf status cannot distinguish 已售罄 from 已上架, so it would report a
 * sold-out link as an on-shelf sibling.
 */
export function buildSameStoreOnShelfSkcIndex({storeLinks = [], matrix = []} = {}) {
  const index = new Map();
  const add = (storeKey, matchKey, skc) => {
    if (!matchKey || !skc) return;
    const key = sameStoreOnShelfIndexKey(storeKey, matchKey);
    if (!index.has(key)) index.set(key, new Set());
    index.get(key).add(String(skc));
  };
  const identity = row => {
    const raw = row?.standard_goods_sn ?? row?.standardGoodsSn ?? row?.raw_goods_sn ?? row?.rawGoodsSn;
    return resolveInventoryIdentityKey(raw) || canonicalInventoryKey(raw);
  };
  for (const row of Array.isArray(storeLinks) ? storeLinks : []) {
    if (resolveInventoryShelfStatus(row).code !== '1') continue;
    add(row?.store_key ?? row?.storeKey, identity(row), row?.skc);
  }
  for (const row of Array.isArray(matrix) ? matrix : []) {
    const matchKey = identity(row);
    for (const entry of listCoverageSkcStatuses(row?.skc_list ?? row?.skcList)) {
      if (entry.statusName !== '已上架' && !/ON_SHELF/i.test(entry.statusName)) continue;
      add(row?.store_key ?? row?.storeKey, matchKey, entry.skc);
    }
  }
  return index;
}
