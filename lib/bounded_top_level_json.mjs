...
export function decodeUtf8Strict(buffer) {
  ...
  try {
    ...
    fail('INVALID_UTF8', String(error?.message || 'Invalid UTF-8 encoding'));
  }
}
...
class StreamingUtf8Validator {
  ...
  push(byte, offset) {
    if (this.remaining > 0) {
      if (byte < this.nextMin || byte > this.nextMax) {
        fail('INVALID_UTF8', `Invalid UTF-8 continuation byte at byte ${offset}`);
      }
      this.remaining -= 1;
      this.nextMin = 0x80;
      this.nextMax = 0xbf;
      if (this.remaining === 0) this.sequenceStart = -1;
      return;
    }

    if (byte <= 0x7f) return;
    this.sequenceStart = offset;
    ...
    fail('INVALID_UTF8', `Invalid UTF-8 leading byte at byte ${offset}`);
  }

  finish() {
    if (this.remaining !== 0) {
      fail('INVALID_UTF8', `Truncated UTF-8 sequence at byte ${this.sequenceStart}`);
    }
  }
}
 ...
 * budget. Fatal UTF-8 validity, structural framing, duplicate requested keys
 * and truncated input fail closed. Returned ranges use byte offsets: [start, end).
 */
export async function scanBoundedTopLevelJson(readable, limits) {
  const captureLimits = normalizeLimits(limits);
  const utf8 = new StreamingUtf8Validator();
