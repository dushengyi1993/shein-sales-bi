...
async function loadOpenApiProductDetail(sourceStore, skc) {
  const store = normalizeStoreKey(sourceStore);
  ...
  const details = asArray(data?.detailResults);
  let detail = details.find(item => {
    const info = item?.info || item?.data || {};
    return item?.ok !== false && spuName && safeString(info?.spuName || info?.spu_name, 160) === safeString(spuName, 160);
  }) || null;
  if (!detail) {
    detail = details.find(item => {
      const info = item?.info || item?.data || {};
      return item?.ok !== false && openApiSkcList(info).some(row => rowSkcName(row) === skc);
    }) || null;
  }
  const info = detail?.info || detail?.data || null;
  if (!info) return null;
  const skcInfo = openApiSkcList(info).find(row => rowSkcName(row) === skc) || openApiSkcList(info)[0] || null;
  return {
    file,
    row: normalizedRow,
    info,
...
function applyOpenApiDetailToPayload(payload, openApiDetail, canonicalDraft) {
  ...
  const supplierCode = firstNonEmpty(skcInfo.supplierCode, skcInfo.supplier_code, info.supplierCode, info.supplier_code);
  if (supplierCode !== '') {
    targetSkc.supplier_code = safeString(supplierCode, 240);
    applied.push('skc_list.supplier_code');
  }
...
export async function buildProductDraftFromSnapshots({
  ...
  const openApiDetail = await loadOpenApiProductDetail(store, skc);
  ...
  if (openApiDetail?.file) {
    sourcePointers.push({
      type: 'openapi_product_detail_snapshot',
