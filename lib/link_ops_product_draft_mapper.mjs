import {areDistinctProductModels, buildProductAliasContext, resolveExplicitProductAlias, PRODUCT_MODEL_ATTRIBUTE_ID, normalizeProductAttributeId} from './link_ops_product_attribute_binding.mjs';
...
import {
  ...
  sha256StableJson,
} from './shein_openapi_product_cache.mjs';
...
export function resolveLinkOpsOutputDir() {
  return path.resolve(process.env.SHEIN_BI_OUTPUT_DIR || path.join(ROOT, 'outputs'));
}
...
async function listDateCandidates(sourceStore) {
  ...
  const dates = new Set();
  const aggregateDir = path.join(resolveLinkOpsOutputDir(), 'shein_links', store);
  ...
  const rawDir = path.join(resolveLinkOpsOutputDir(), 'shein_links_raw', store);
...
async function loadAggregateSnapshot(sourceStore, date) {
  const file = path.join(resolveLinkOpsOutputDir(), 'shein_links', normalizeStoreKey(sourceStore), `${date}.json`);
  if (!(await exists(file))) return null;
  return {file, data: await readJson(file)};
}

async function loadRawSnapshots(sourceStore, date) {
  const dir = path.join(resolveLinkOpsOutputDir(), 'shein_links_raw', normalizeStoreKey(sourceStore), date);
...
function normalizeOpenApiAttribute(row) {
  ...
  const normalizedId = normalizeNumericOrString(attributeId);
  const numericId = Number(normalizedId);
  // Only positive safe-integer IDs may reach publishOrEdit; reject source
  // placeholders, fractions, non-numeric strings, and unsafe large integers.
  if (normalizedId === undefined || !Number.isSafeInteger(numericId) || numericId <= 0) return null;
  const out = {};
  out.attribute_id = normalizedId;
...
export async function loadOpenApiProductDetail(sourceStore, skc, {
  cacheDir = '',
  env = process.env,
  ...
  const store = normalizeStoreKey(sourceStore);
  const file = resolveOpenApiProductCacheFile(store, {rootDir: ROOT, cacheDir, env});
  if (!(await exists(file))) return null;
  let cache;
  try {
    cache = await readOpenApiProductCache(file, {
      expectedStore: store,
      env,
      maxAgeMs,
      now,
  ...
  const requestedSkc = safeString(skc, 160);
  const rows = asArray(data?.normalizedRows);
  const matchingRows = rows.filter(row => String(row?.skc || row?.skcName || row?.skc_name || '') === requestedSkc);
  // The exact source lock is case-sensitive and unique. Do not use broad JSON
  // containment or the first normalized/detail SKC as a substitute.
  if (matchingRows.length !== 1) return null;
  const normalizedRow = matchingRows[0];
  if (normalizedRow?.storeKey && normalizeStoreKey(normalizedRow.storeKey) !== store) return null;
  ...
  if (!currentDetail) {
    ...
    if (conflict) return includeConflict ? {detail: null, conflict} : null;
  }
  ...
  if (!info) return null;
  const matchingSkcs = openApiSkcList(info).filter(row => rowSkcName(row) === requestedSkc);
  if (matchingSkcs.length !== 1) return null;
  const skcInfo = matchingSkcs[0];
  const detailContentSha256 = sha256StableJson({
    info,
    matchedSkcName: requestedSkc,
  });
  const sourceDetailHash = sha256StableJson({
    storeKey: store,
    sourceSkc: requestedSkc,
    spuName: safeString(spuName, 160),
    info,
    skcInfo,
  });
  const sourceDetailLock = {
    source: detailSource,
    matchedSpuName: openApiDetailSpuName(detail),
    matchedSkcName: requestedSkc,
    detailFetchedAt: typeof detailFetchedAt === 'string' ? detailFetchedAt : '',
    detailContentSha256,
    schemaVersion: 'shein-openapi-source-detail-lock/v1',
    ...
    generatedAt: typeof detailFetchedAt === 'string' ? detailFetchedAt : cache.generatedAt,
  };
  // The source-precedence branch historically exposed {detail, conflict},
  // while the cache loader is also a useful direct detail reader. Return the
  // detail fields directly and keep non-enumerable compatibility views so
  // both callers observe the same exact source lock without creating a JSON
  // cycle in persisted draft summaries.
  const detailResult = {
    file,
    row: normalizedRow,
    info,
  ...
  Object.defineProperty(detailResult, 'detail', {value: detailResult, enumerable: false});
  Object.defineProperty(detailResult, 'conflict', {value: null, enumerable: false});
  return detailResult;
}
...
function buildPayloadDraft(canonicalDraft) {
  ...
  payload.skc_list = [{
    source_skc: safeString(firstSkc.sourceSkc, 160),
...
export async function buildProductDraftFromSnapshots({
  ...
  donorStore = '',
  donorSkc = '',
  donorOpenApiDetail = null,
  reviewedImages = null,
  reviewedDescriptions = null,
} = {}) {
  const store = normalizeStoreKey(sourceStore);
  ...
  const openApiLookup = await loadOpenApiProductDetail(store, skc, {cacheDir, env, includeConflict: true, now});
  const openApiDetail = openApiLookup?.detail
    || (openApiLookup && !Object.prototype.hasOwnProperty.call(openApiLookup, 'conflict') ? openApiLookup : null);
  ...
  // C1: Strict cross-link donor attribute supplementation (e.g. XL draft + TZZ attributes)
  const donorStoreKey = normalizeStoreKey(donorStore);
  const donorSkcKey = safeString(donorSkc, 120);
  let resolvedDonorDetail = donorOpenApiDetail;
  if (donorStoreKey && donorSkcKey && !resolvedDonorDetail) {
    try {
      const donorLookup = await loadOpenApiProductDetail(donorStoreKey, donorSkcKey, {cacheDir, env, now});
      resolvedDonorDetail = donorLookup?.detail || (donorLookup && !donorLookup.conflict ? donorLookup : null);
    } catch (err) {
      mappingBlockers.push({code: 'DONOR_LOOKUP_FAILED', message: '加载 donor (' + donorStoreKey + '/' + donorSkcKey + ') 详情失败: ' + safeString(err?.message || err, 200)});
    }
  }
  if (donorStoreKey && donorSkcKey) {
    if (!resolvedDonorDetail?.info) {
      mappingBlockers.push({code: 'DONOR_DETAIL_MISSING', message: 'donor (' + donorStoreKey + '/' + donorSkcKey + ') 缺少有效官方 OpenAPI 详情，禁止借用属性事实。'});
    } else {
      const donorSupplierCode = firstNonEmpty(resolvedDonorDetail.skcInfo?.supplierCode, resolvedDonorDetail.skcInfo?.supplier_code, resolvedDonorDetail.info?.supplierCode, resolvedDonorDetail.info?.supplier_code);

      // 1. Resolve alias and canonical codes via buildProductAliasContext
      let aliasCtx = null;
      try {
        const aliasPath = env?.SHEIN_PRODUCT_ALIASES_FILE || path.join(ROOT, 'config', 'product_aliases.json');
        const catalogPath = env?.SHEIN_PRODUCT_CATALOG_FILE || path.join(ROOT, 'config', 'product_catalog.json');
        const aliasRaw = await fs.readFile(aliasPath, 'utf8').catch(() => null);
        const catalogRaw = await fs.readFile(catalogPath, 'utf8').catch(() => null);
        if (aliasRaw && catalogRaw) {
          aliasCtx = buildProductAliasContext({
            aliasRegistryJson: JSON.parse(aliasRaw),
            catalogJson: JSON.parse(catalogRaw),
            aliasRegistrySource: aliasPath,
            catalogSource: catalogPath,
          });
        }
      } catch (err) {
        mappingBlockers.push({code: 'ALIAS_CONTEXT_LOAD_FAILED', message: '加载商品别名注册表失败: ' + safeString(err?.message || err, 200)});
      }

      const taskAlias = aliasCtx ? resolveExplicitProductAlias(aliasCtx, supplierCode) : {ok: false, canonical: ''};
      const donorAlias = aliasCtx ? resolveExplicitProductAlias(aliasCtx, donorSupplierCode) : {ok: false, canonical: ''};

      if (!taskAlias.ok) {
        mappingBlockers.push({code: 'TASK_PRODUCT_ALIAS_UNRESOLVED', message: '任务货号(' + supplierCode + ') 别名解析未通过，缺少有效商品身份'});
      } else if (!donorAlias.ok) {
        mappingBlockers.push({code: 'DONOR_PRODUCT_ALIAS_UNRESOLVED', message: 'donor 货号(' + donorSupplierCode + ') 别名解析未通过，缺少有效商品身份'});
      } else if (taskAlias.canonical !== donorAlias.canonical) {
        mappingBlockers.push({code: 'DONOR_CANONICAL_PRODUCT_MISMATCH', message: 'donor canonical(' + donorAlias.canonical + ') 与任务 canonical(' + taskAlias.canonical + ') 不是同一商品，禁止借用属性。'});
      } else if (areDistinctProductModels(supplierCode, donorSupplierCode) || areDistinctProductModels(taskAlias.canonical, donorAlias.canonical)) {
        // 2. Strict model variant check (3065 vs 3065W)
        mappingBlockers.push({code: 'DISTINCT_PRODUCT_MODEL_BORROWING_FORBIDDEN', message: 'donor 货号(' + donorSupplierCode + ') 与任务货号(' + supplierCode + ') 属于不同型号（3065 与 3065W 不混），禁止互借属性。'});
      } else {
        // 3. Strict category matching check (fail closed if category missing or mismatch)
        const sourceCat = normalizeNumericOrString(firstNonEmpty(openApiDetail?.info?.categoryId, openApiDetail?.info?.category_id));
        const donorCat = normalizeNumericOrString(firstNonEmpty(resolvedDonorDetail.info?.categoryId, resolvedDonorDetail.info?.category_id));
        if (!sourceCat || !donorCat || String(sourceCat) !== String(donorCat)) {
          mappingBlockers.push({code: 'DONOR_CATEGORY_MISMATCH', message: 'donor 类目(' + donorCat + ') 与任务类目(' + sourceCat + ') 不一致或缺失，禁止借用属性。'});
        } else {
          // 4. Strict product model attribute matching
          const sourceModelAttr = openApiProductAttributes(openApiDetail?.info).find(a => normalizeProductAttributeId(a.attribute_id) === PRODUCT_MODEL_ATTRIBUTE_ID);
          const donorModelAttr = openApiProductAttributes(resolvedDonorDetail.info).find(a => normalizeProductAttributeId(a.attribute_id) === PRODUCT_MODEL_ATTRIBUTE_ID);
          const sourceModelVal = safeString(sourceModelAttr?.attribute_value ?? sourceModelAttr?.attribute_extra_value, 200);
          const donorModelVal = safeString(donorModelAttr?.attribute_value ?? donorModelAttr?.attribute_extra_value, 200);
          if (sourceModelVal && donorModelVal && sourceModelVal !== donorModelVal) {
            mappingBlockers.push({code: 'DONOR_PRODUCT_MODEL_MISMATCH', message: 'donor Product Model(' + donorModelVal + ') 与任务(' + sourceModelVal + ') 不是同一商品，禁止借用属性。'});
          } else {
            // 5. Extract donor attributes and supplement missing ones
            const donorSaleAttrIds = openApiSaleAttributeIds({skcInfo: resolvedDonorDetail.skcInfo});
            const donorAttrs = openApiProductAttributes(resolvedDonorDetail.info, {excludeAttributeIds: donorSaleAttrIds});
            if (donorAttrs.length) {
              const existingAttrs = asArray(openapiPublishPayloadDraft.product_attribute_list);
              const existingAttrIds = new Set(existingAttrs.map(a => String(a.attribute_id)));
              const missingAttrs = donorAttrs.filter(a => !existingAttrIds.has(String(a.attribute_id)));
              if (missingAttrs.length || !existingAttrs.length) {
                openapiPublishPayloadDraft.product_attribute_list = [...existingAttrs, ...missingAttrs];
                openApiApplied.applied.push('product_attribute_list.donor_supplemented');
                sourcePointers.push({
                  type: 'donor_product_attributes',
                  storeKey: donorStoreKey,
                  skc: donorSkcKey,
                  canonicalCode: donorAlias.canonical,
                  supplementedCount: missingAttrs.length,
                  totalCount: openapiPublishPayloadDraft.product_attribute_list.length,
                });
              }
            }
          }
  ...
  // Reviewed descriptions combination
  if (reviewedDescriptions) {
    const descRows = Array.isArray(reviewedDescriptions) ? reviewedDescriptions : asArray(reviewedDescriptions.multi_language_desc_list || reviewedDescriptions.descriptions);
    if (descRows.length) {
      openapiPublishPayloadDraft.multi_language_desc_list = descRows;
      openApiApplied.applied.push('multi_language_desc_list.reviewed');
    }
  }
  // Reviewed images combination
  if (reviewedImages && Array.isArray(reviewedImages) && reviewedImages.length) {
    const targetSkc = openapiPublishPayloadDraft.skc_list?.[0];
    if (targetSkc) {
      targetSkc.image_info = { image_info_list: payloadImageInfoList(reviewedImages, 12) };
      openApiApplied.applied.push('skc_list.image_info.reviewed');
    }
  }
  canonicalDraft.openApiDetail = openApiDetail ? {
    ...
    detailFetchedAt: safeString(openApiDetail?.detailFetchedAt, 80),
    sourceDetailHash: openApiDetail.sourceDetailHash || '',
    sourceDetailLock: openApiDetail.sourceDetailLock || null,
    cacheMetadata: openApiDetail.cacheMetadata || null,
  ...
  const sourceDetailLock = openApiDetail ? {
    ...
    schemaVersion: openApiDetail.sourceDetailLock?.schemaVersion || 'shein-openapi-source-detail-lock/v1',
    hashAlgorithm: openApiDetail.sourceDetailLock?.hashAlgorithm || 'sha256-stable-json-v1',
    sourceStore: store,
    sourceSkc: skc,
    spuName: safeString(firstNonEmpty(openApiDetail?.info?.spuName, openApiDetail?.info?.spu_name), 160),
    sourceDetailHash: openApiDetail.sourceDetailHash || '',
    cacheFile: openApiDetail.sourceDetailLock?.cacheFile || '',
    cacheHash: openApiDetail.sourceDetailLock?.cacheHash || '',
    generatedAt: openApiDetail.sourceDetailLock?.generatedAt || openApiDetail.detailFetchedAt,
  } : null;
  return {
    ok: true,
    ...
    sourceDetailHash: openApiDetail?.sourceDetailHash || sourceDetailLock?.sourceDetailHash || '',
...
export function summarizeDraftForExecutor(result) {
  ...
  return {
    ...
    sourceDetailHash: result.sourceDetailHash || draft.openApiDetail?.sourceDetailHash || '',
     sourceDetailLock: result.sourceDetailLock || draft.openApiDetail?.sourceDetailLock || null,
