...
import {appendInventoryReconciliationRecord, inventoryLogicalActionKey, inventoryRecoveryScopeKey,
  discoverInventoryJournalFiles, readInventoryIntentJournals, selectAgedPendingInventoryIntent,
  submitDurableInventoryWriteOnce, INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION,
  INVENTORY_SUPERSEDED_BY_LATER_PLAN_RULE} from './durable_inventory_write.mjs';
...
export async function readMarketingInventoryJournalDomain({journalFile, runDate, client = new SheinOpenApiClient({inventoryJournalFile: journalFile})}) {
  const files = await discoverInventoryJournalFiles(journalFile, {includeAll: true,
    additionalDirectories: client.inventoryJournalDomainDirectories()});
  const bundle = await readInventoryIntentJournals(files, {allowMultiplePendingByScope: true,
    maxRunDate: runDate, currentJournalFile: journalFile, quarantineHistoricalDanglingSupersedes: true});
  return {files, bundle};
}
...
export async function createMarketingActivityInventoryOpenApiAdapter({
  ...
  const client = new SheinOpenApiClient({
    ...
    ...(execute ? {inventoryFenceReader: () => readMarketingInventoryJournalDomain({client, journalFile, runDate})} : {}),
  });
  ...
  if (execute) {
    const {bundle} = await readMarketingInventoryJournalDomain({client, journalFile, runDate});
...
export async function writeMarketingInventoryOnce({client, storeKey, journalFile, transactionHash, runDate,
  ...
  const readBundle = async () => (await readMarketingInventoryJournalDomain({client, journalFile, runDate})).bundle;
  ...
  const outcome = await submitDurableInventoryWriteOnce({
    ...
    prepareUnderLock: async () => {
      ...
      // run abandoned must not fence this scope forever. This transaction is a
      // strictly later, owner-authorized OVERWRITE with an exact post-write
      // readback, so it supersedes the abandoned intent (recording
      // originalEffectUnknown) instead of failing closed every single run.
      // Same-day uncertainty is untouched: only a strictly earlier run date
      // qualifies, and a scope whose live value already equals the abandoned
      // target is left to the exact-readback path.
      const aged = selectAgedPendingInventoryIntent(bundle, {
        scope: {storeKey, skc: target.skc, skuCode: target.skuCode, invType: 'VI'},
        runDate,
        currentUsableInventory: before.totalUsableInventory,
        currentWarehouseCodes: before.warehouseCodes,
      });
      if (aged) {
        await appendInventoryReconciliationRecord(journalFile, aged, {
          kind: 'write_outcome',
          intentId: aged.intentId,
          logicalActionKey: aged.logicalActionKey,
          disposition: INVENTORY_SUPERSEDED_BY_LATER_PLAN_DISPOSITION,
          originalEffectUnknown: true,
          oldRunDate: aged.runDate,
          newRunDate: runDate,
          supersedeRule: INVENTORY_SUPERSEDED_BY_LATER_PLAN_RULE,
          recordedAt: new Date().toISOString(),
        });
        bundle = await readBundle();
        if (bundle.pending.has(`${path.resolve(aged.journalFile)}\u0000${aged.intentId}`)) {
          throw new Error('MARKETING_INVENTORY_STALE_INTENT_SUPERSEDE_FAILED');
        }
      }
