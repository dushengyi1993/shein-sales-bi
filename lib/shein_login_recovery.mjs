...
export async function clickSheinLoginOnce({evaluate, dispatchMouseEvent}) {
  const target = await evaluate(`
    ...
    const btn = buttons.find(x => !x.disabled && x.text === '我已知晓，继续登录')
      ...
      // Saved-password profiles often show visually empty inputs until the
      // login button is clicked. Click once first, then let
      // readSheinLoginState decide whether it is a real empty-credential block.
      || (!/验证码|短信验证|手机验证|安全验证|滑块|拖动滑块|人机验证|captcha|verify/i.test(text) && buttons.find(x => !x.disabled && x.text === '登录'))
      || (!/验证码|短信验证|手机验证|安全验证|滑块|拖动滑块|人机验证|captcha|verify/i.test(text) && buttons.find(x => !x.disabled && x.text.includes('登录') && x.text.length <= 12))
