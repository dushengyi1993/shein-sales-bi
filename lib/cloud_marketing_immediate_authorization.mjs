...
export async function verifyImmediateAuthorizationContinuation({
  ...
  if (queueSnapshotBytes) {
    ...
    const sourceGuardPath = resolveSourceGuardPath(root, original.sourceGuard);
    const sourceGuard = await readRegularFile(sourceGuardPath, 'snapshot sourceGuard');
    const pair = assertQueuePair(original);
    if (sourceGuard.sha256 !== pair.sourceGuardHash) fail('snapshot source guard mismatch');
...
export async function verifyConsumedFallbackAdmissionSnapshot(options = {}) {
  ...
  for (const [stage, before] of Object.entries(original.stages || {})) {
    ...
    if (!after?.resultPath || after.workFingerprint !== before.workFingerprint
      || !['pending', 'completed', 'blocked'].includes(after.status)) fail('unproven stage progress after receipt admission');
    const result = await readJsonFile(resolveSourceGuardPath(options.root, after.resultPath), 'stage progress');
    ...
    progressedGroups += result.value.results.length;
    if (stage === 'manualSpecialRestore') {
      ...
      if (result.value.dryRunOnly !== false || result.value.totals?.processed !== rows.length
        || !rows.every(row => row.ok === true || row.terminalBlocked === true || row.recoverableDeferred === true || row.deferred === true)
        || (after.status === 'completed' && (result.value.totals.remainingItems !== 0 || !rows.every(row => row.ok === true)))
        || (after.status === 'pending' && !(result.value.totals.remainingItems > 0))) fail('manual progress result is inconsistent');
    } else if (stage === 'fallbackRepair') {
      const doc = result.value;
      if (doc.dryRunOnly !== false || doc.totals?.storesProcessed !== doc.results.length
        || !doc.results.every(row => row.ok === true || row.blocked || row.deferred === true)
  ...
  return {...receipt, remainingUnstartedGroups: Math.max(0, receipt.maxGroups - progressedGroups)};
}
...
export async function verifyUnstartedRepairStageContinuation(options = {}) {
  ...
  if (hashBytes(originalBytes) !== receipt.queueStateSha256 || receipt.status !== 'consumed'
    || receipt.gracefulRemainingSec < 900) fail('unstarted repair requires the original consumed queue and graceful budget');
  ...
  if (JSON.stringify(original.stages[stage]) !== JSON.stringify(live.stages?.[stage])) {
    fail('previously progressed stage requires transaction continuation', 'IMMEDIATE_AUTHORIZATION_TRANSACTION_CONTINUATION_REQUIRED');
  }
  ...
  for (const entry of exactPlan.entries) {
    ...
    if (await findPersistedMarketingTransactionContinuation(scope)) fail('persisted transaction requires transaction continuation',
      'IMMEDIATE_AUTHORIZATION_TRANSACTION_CONTINUATION_REQUIRED');
    const evidence = await readLimitedDiscountMutationEvidence({...scope, sourceRescuePath: entry.path});
    if (evidence.reason !== 'mutation evidence unavailable: ENOENT') fail('persisted, submitted or unknown operation evidence blocks unstarted recovery');
    if (hashBytes(await fs.readFile(entry.path)) !== entry.contentHash) fail('unstarted rescue changed during evidence check');
  }
  ...
  for (const name of journals.filter(name => name.endsWith('.json'))) {
    const journal = await readJsonFile(path.join(journalDir, name), 'persisted operation');
    if (journal.value.runPayloadHash === expectedWorkFingerprint
      || journal.value.createAttempt?.workFingerprint === expectedWorkFingerprint) fail('persisted subset operation blocks unstarted recovery');
  }
  ...
  const targetSkcs = new Set(exactPlan.entries.flatMap(entry => (entry.rescue.rows || []).map(row => String(row.skc || ''))));
...
export async function findImmediateAuthorizationContinuation({
  ...
  const names = (await fs.readdir(path.dirname(source)))
    .filter(name => name.startsWith(prefix) && name.endsWith('.json') && !name.endsWith('.queue-snapshot.json'));
  const relevant = [];
