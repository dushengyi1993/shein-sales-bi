...
async function readQueueBinding({queueFile, root, date, sourceGuardFile}) {
  ...
  if (date && queueDate !== date) fail(`queue business date mismatch: expected=${date} actual=${queueDate}`);
...
function assertRecordBinding(record, queue, {
  ...
  if (record.queueFile !== queue.queueFile) fail(`authorization queue file mismatch: authorization=${record.queueFile} current=${queue.queueFile}`);
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (record.date === normalizeDate(date) && recordSource === source) {
        relevant.push({receiptFile, record});
      }
    } catch (error) {
