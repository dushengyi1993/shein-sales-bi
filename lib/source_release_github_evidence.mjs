...
export function validateSourceReleaseTrustPolicy(policy) {
  ...
  if (policy.schemaVersion !== SOURCE_RELEASE_TRUST_POLICY_SCHEMA
    ...
    || policy.sourceRelease.requireOwnerEnforcement !== true) {
...
export function decideSourceReleaseState(state = {}) {
  ...
  if (!Number.isSafeInteger(release.id) || release.id < 1
    || release.tagName !== state.version || release.title !== state.expectedTitle
...
export async function fetchRepositoryTrustEvidence({client, policy} = {}) {
  ...
  if ((policy.sourceRelease.requireImmutableReleases && immutable?.enabled !== true)
    ...
    throw fail('SOURCE_RELEASE_IMMUTABLE_POLICY_DISABLED', 'GitHub immutable releases policy is not fully enabled and owner-enforced');
  }
  return {id: metadata.id, fullName: metadata.full_name, immutable};
}

async function findExactRelease(client, policy, tag) {
  ...
  if (matching.length !== 1) {
    throw fail('SOURCE_RELEASE_RELEASE_IDENTITY_INVALID', `Expected one GitHub Release for ${tag}; found ${matching.length}`);
  }
  return matching[0];
}
...
export async function verifyRemoteSourceReleaseEvidence({
  ...
  const release = await findExactRelease(client, policy, tag);
  ...
  const releaseState = decideSourceReleaseState({
    ...
    expectedCommit: commit,
    observedMain: commit,
    expectedTitle: `${policy.sourceRelease.releaseTitlePrefix}${tag}`,
    attestationSha256: verified.attestationSha256,
    expectedAssets,
