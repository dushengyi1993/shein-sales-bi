...
export function salesExclusionReason(row) {
  if (String(row?.salesExclusionReason || '').trim()) return String(row.salesExclusionReason).trim();
...
export function isValidSalesGoodsRow(row) {
  if (row?.isValidSale === false || String(row?.isValidSale).toLowerCase() === 'false') return false;
  return toNumber(row?.number ?? row?.quantity) > 0
    && toNumber(row?.currencyPrice) > 0
...
export function salesAmountSar(row) {
  return isValidSalesGoodsRow(row) ? toNumber(row?.currencyPrice) : 0;
}
...
export function summarizeSalesGoodsRows(goodsRows = []) {
  ...
  return {
    ...
    excludedSalesSar: excludedRows.reduce((sum, row) => sum + toNumber(row?.currencyPrice), 0),
