 ...
 * history fails closed; v1 records are never migrated.
 */
export function validateV1ProductAttributeHistory(task, binding) {
  ...
  if (!matches) {
    return {
      ok: false,
      blockers: [blocker('PRODUCT_ATTRIBUTE_REFRESH_V1_HISTORY_INVALID', 'v1 刷新要求历史事件字段与当前绑定逐项一致（缺失/冲突/不完整均拒绝，不做迁移）')],
    };
  }
...
export function validateProductAttributeRefreshEvent(task, {requireFreshRevision = true} = {}) {
  ...
  const matchingEntries = refreshEntries.filter(entry => {
    const entryDonor = entry?.donor && typeof entry.donor === 'object' ? entry.donor : {};
  ...
  if (matchingEntries.length !== 1) {
    blockers.push(blocker(
      ...
      `refresh 历史事件按当前事件/绑定身份过滤后必须恰好匹配 1 条（匹配 ${matchingEntries.length}，总 ${refreshEntries.length}）`,
    ));
  }
