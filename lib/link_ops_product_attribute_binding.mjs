...
import {isSheinSkc} from './shein_product_identifiers.mjs';
...
export function areDistinctProductModels(codeA, codeB) {
  const sA = String(codeA ?? '').toUpperCase().trim();
  const sB = String(codeB ?? '').toUpperCase().trim();
  if (!sA || !sB || sA === sB) return false;
  const has3065WA = /(?:^|[^A-Z0-9])(?:SK[-_]?)?(?:GT[-_]?)?3065W(?:[^A-Z0-9]|$)/.test(sA) || sA.includes('3065W');
  const has3065WB = /(?:^|[^A-Z0-9])(?:SK[-_]?)?(?:GT[-_]?)?3065W(?:[^A-Z0-9]|$)/.test(sB) || sB.includes('3065W');
  const has3065A = sA.includes('3065');
  const has3065B = sB.includes('3065');
  if (has3065A && has3065B && (has3065WA !== has3065WB)) {
    return true;
  }
  return false;
}
...
export function isValidProductAttributeDonorSkc(value) {
...
export function evaluateDonorProductAttributeEvidence({
  ...
  if (!isValidProductAttributeDonorSkc(skc)) {
  ...
  if (areDistinctProductModels(taskRawCode, spuInfo.supplierCode)
    || areDistinctProductModels(taskModelValue, spuInfo.donorModelValue)
    || areDistinctProductModels(taskAlias.canonical, donorAlias.canonical)) {
    return {
      ok: false,
      blockers: [blocker('DISTINCT_PRODUCT_MODEL_BORROWING_FORBIDDEN', 'donor (' + spuInfo.supplierCode + ') 与任务 (' + taskRawCode + ') 属于不同型号（3065 与 3065W 不混），禁止互借属性事实')],
      evidence: null,
      attributeValueId: null,
      donorSpu: search.donorSpu,
      donorSupplierCode: spuInfo.supplierCode,
...
export function validateProductAttributeBindingLock(task, payload) {
  ...
  if (!isValidProductAttributeDonorSkc(donorSkc)) {
  ...
  const currentPayloadHash = sha256StableJson(payload ?? {});
  if (String(binding.newPayloadHash || '').toLowerCase() !== currentPayloadHash) {
