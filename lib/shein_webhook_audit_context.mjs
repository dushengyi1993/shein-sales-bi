...
function collectProductSkcRows(value, {depth = 0, seen = new Set(), rows = []} = {}) {
  if (!value || typeof value !== 'object' || seen.has(value) || depth > 7 || rows.length >= 1000) return rows;
  seen.add(value);
  if (!Array.isArray(value) && text(value.skcName ?? value.skc_name) && text(value.supplierCode ?? value.supplier_code)) rows.push(value);
  for (const child of Array.isArray(value) ? value : Object.values(value)) collectProductSkcRows(child, {depth: depth + 1, seen, rows});
  return rows;
}
...
export async function createSheinWebhookAuditContextProvider({
  ...
  return Object.freeze({
    async getAuditContext({storeKey, skc, productId, version, documentId, auditFailureReason} = {}) {
      ...
      if (!supplierCode) {
        try {
          const response = await client.request(SEARCH_PRODUCT_PATH, {method: 'POST', body: {pageNum: 1, pageSize: 10, skcNameList: [targetSkc]}, headers: {language: 'zh-cn'}});
