...
function normalizeShelfState(code) {
...
export async function createSheinWebhookAuditContextProvider({
  ...
  async function getProductState({storeKey, skc} = {}) {
    ...
    if (spu && credential) {
      ...
      detail = collectSkcDetailRows(data).find(row => text(row.skcName ?? row.skc_name) === targetSkc) || exact;
    ...
    return Object.freeze({
      ...
      supplierCode,
      ...(state || {}),
      firstShelfTime: text(siteState?.firstShelfTime ?? siteState?.first_shelf_time),
      lastShelfTime: text(siteState?.lastShelfTime ?? siteState?.last_shelf_time),
