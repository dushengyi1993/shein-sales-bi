...
function localizedProductTitle(row) {
  const titles = Array.isArray(row?.skcTitle)
    ? row.skcTitle
    : Array.isArray(row?.skc_title) ? row.skc_title : [];
  const normalized = titles.map(item => typeof item === 'string'
    ? {language: '', title: text(item)}
    : {
      language: text(item?.language ?? item?.lang ?? item?.languageCode).toLowerCase(),
      title: text(item?.title ?? item?.name ?? item?.value),
    }).filter(item => item.title);
  return (normalized.find(item => /^(?:zh(?:-cn|_cn)?|cn)$/i.test(item.language)) || normalized[0])?.title || '';
}
...
export async function createSheinWebhookAuditContextProvider({
  ...
  async function getProductIdentity({storeKey, skc} = {}) {
    ...
    return Object.freeze({
      ...
      productName: localizedProductTitle(exact),
