...
export async function loadCouponTargetEligibilityPlan({
  ...
  for (const item of planItems) {
    ...
    const row = {
      ...
      fullCost: decision.row?.fullCost ?? '',
      minMarginFloor: decision.row?.minMarginFloor ?? '',
...
export function classifyCouponEligibilityRow(row, {targetDiscountPct = 15} = {}) {
  ...
  // P2-#6: check structured couponPolicy field first, before regex fallback
  const structuredPolicy = String(row?.couponPolicy || '').trim().toLowerCase();
  if (structuredPolicy === 'traffic' || structuredPolicy === 'allowed15_traffic') {
    return {
      ...basis,
      allowed15: true,
      category: 'allowed15_traffic',
      reason: 'structured_coupon_policy_traffic',
    };
  }
  if (structuredPolicy === 'forbidden' || structuredPolicy === 'no_coupon') {
    return {
      ...basis,
      allowed15: false,
      category: 'forbidden',
      reason: 'structured_coupon_policy_forbidden',
    };
  }
  if (structuredPolicy === 'price_guarantee') {
    return {
      ...basis,
      allowed15: false,
      category: 'price_guarantee_coupon_blocked',
      reason: 'structured_coupon_policy_price_guarantee_blocked',
    };
  }
  ...
  if (factor !== null) {
    if (Math.abs(factor - targetFactor) <= TARGET_FACTOR_TOLERANCE) {
      if (allowsOptionalTraffic15(row, compactCombo)) {
        return {
          ...basis,
          allowed15: true,
          category: 'allowed15_traffic',
          reason: 'explicit_optional_traffic_coupon_policy',
        };
      }
      return {
        ...basis,
        allowed15: false,
        category: 'price_guarantee_coupon_blocked',
        reason: 'coupon_factor_15pct_is_not_guaranteed_price_layer',
      };
    }
    if (factor < 1) {
      return {
  ...
  if (allowsTarget15(compactCombo) || allowsOptionalTraffic15(row, compactCombo)) {
    return {
      ...basis,
      allowed15: true,
      category: 'allowed15_traffic',
...
export function isOptionalTrafficCouponPlanRow(planRow) {
  const compactCombo = normalizeCombo(planRow?.combo || '');
  return allowsOptionalTraffic15(planRow || {}, compactCombo);
}

export function plannedCouponSafetyFloor(planRow, options = {}) {
  const normalizedOptions = typeof options === 'number'
    ? {fallbackMinMargin: options}
    : (options || {});
  const fallbackMinMargin = numberOrNull(normalizedOptions.fallbackMinMargin) ?? 0.15;
  const cost = numberOrNull(planRow?.cost);
  const fullCost = numberOrNull(planRow?.fullCost);
  const minMarginFloor = numberOrNull(planRow?.minMarginFloor) ?? fallbackMinMargin;
  const candidates = [];
  if (cost !== null && minMarginFloor < 1) {
    candidates.push(round2(cost / (1 - minMarginFloor)));
  }
  if (fullCost !== null) candidates.push(fullCost);
...
export function classifyLimitedDiscountCouponStack(limitedRow, planRow, options = {}) {
  ...
  if (out.diff < -priceToleranceSar) {
    out.decision = isTrafficCoupon ? 'coupon_final_below_safety_floor' : 'coupon_final_below_target';
    out.shouldCancelCoupon = true;
    return out;
  }
  out.allowSubmit = true;
  out.decision = isTrafficCoupon
    ? (Math.abs(out.diff) <= priceToleranceSar ? 'traffic_coupon_final_matches_safety_floor' : 'traffic_coupon_final_above_safety_floor')
    : (Math.abs(out.diff) <= priceToleranceSar ? 'coupon_final_matches_target' : 'coupon_final_above_target');
  return out;
}
...
function summarizeCouponTargetEligibilityMaps(plan) {
  ...
  for (const storeKey of [...stores].sort()) {
    ...
    out.push({
      ...
      priceGuaranteeCouponBlocked: view.categoryCounts.price_guarantee_coupon_blocked || 0,
      allowed15Traffic: view.categoryCounts.allowed15_traffic || 0,
...
function extractPriceOverridePaths(doc) {
  ...
  for (const source of sourceFileValues(doc?.sourceFiles)) {
    if (/price-overrides/i.test(String(source))) paths.push(source);
  }
  return unique(paths);
}

function sourceFileValues(value) {
  if (!value) return [];
  if (Array.isArray(value)) return value.flatMap(sourceFileValues);
  if (typeof value === 'object') return Object.values(value).flatMap(sourceFileValues);
...
function allowsTarget15(compactCombo) {
  // Historical combos such as `普通活动 + 仅15%券` used to mean
  // `ordinary activity price * couponFactor = final target`. SHEIN coupons are
  // not guaranteed to trigger for every order, so that wording is no longer an
  // execution allowance by itself. Only explicit traffic / inventory use cases
  // below may opt into coupon submission.
  return /(?:可选|流量|引流|高曝光|曝光Top5|曝光前五|滞销|高库存|清货|试验).*15%?券/.test(compactCombo)
    || /15%?券.*(?:可选|流量|引流|高曝光|曝光Top5|曝光前五|滞销|高库存|清货|试验)/.test(compactCombo);
}

function allowsOptionalTraffic15(row, compactCombo) {
  if (hasTrafficCouponText(compactCombo)) return true;
  const explicitCouponFields = [
    row?.couponPurpose,
    row?.couponPolicy,
    row?.couponUseCase,
    row?.couponStrategy,
    row?.couponAllowReason,
    row?.couponReason,
    row?.trafficCouponReason,
  ].flatMap(value => Array.isArray(value) ? value : [value]);
  if (explicitCouponFields.some(value => hasTrafficIntentText(value))) return true;
  const noteFields = [
    row?.note,
    row?.remark,
    row?.remarks,
  ...
  return noteFields.some(value => hasTrafficCouponText(value));
}

function hasTrafficCouponText(value) {
  const text = String(value || '')
    .normalize('NFKC')
    .replace(/\s+/g, '');
  return /(?:可选|流量|引流|高曝光|曝光Top5|曝光前五|滞销|高库存|清货|试验).*15%?券/.test(text)
    || /15%?券.*(?:可选|流量|引流|高曝光|曝光Top5|曝光前五|滞销|高库存|清货|试验)/.test(text)
    || /coupon(?:Purpose|Policy|UseCase|Strategy)?[:=]?(?:traffic|high[_-]?exposure|slow[_-]?moving|stagnant|clearance)/i.test(text);
}

function hasTrafficIntentText(value) {
  const text = String(value || '')
    .normalize('NFKC')
    .replace(/\s+/g, '');
  return /(?:可选|流量|引流|高曝光|曝光Top5|曝光前五|滞销|高库存|清货|试验)/.test(text)
    || /(?:traffic|high[_-]?exposure|slow[_-]?moving|stagnant|clearance)/i.test(text);
}
