...
import {resolveFormalInventoryWriterAuthority} from './source_release_inventory_authority.mjs';
...
export const DEFAULT_INVENTORY_CUTOVER_WRITER_SERVICES = Object.freeze([
...
async function readBoundedJson(file, maxBytes, label) {
  ...
  try {
    ...
    // A release artifact that is absent is not an authority fault: the caller
    // already handles "marker missing" through the emergency/absent path. Only
    // an unreadable or invalid artifact may fail closed as UNREADABLE.
    if (error?.code === 'ENOENT' || error?.code === 'SOURCE_RELEASE_FILE_MISSING') {
...
function normalizeServiceState(unit, raw = {}) {
  const controlPid = text(raw.ControlPID);
  const state = {
    unit,
  ...
  const activeGeneration = state.activeState === 'active'
    && /^\d+$/u.test(state.mainPid) && Number(state.mainPid) > 0
    && Boolean(state.execMainStartTimestamp);
  const activatingGeneration = state.activeState === 'activating'
    && state.subState === 'start'
    && /^\d+$/u.test(state.mainPid) && Number(state.mainPid) > 0
    && Boolean(state.execMainStartTimestamp);
  const inactiveGeneration = state.activeState === 'inactive'
    && state.subState === 'dead'
    && /^\d+$/u.test(state.mainPid) && Number(state.mainPid) === 0;
  const failedQuiescent = state.activeState === 'failed'
    && state.subState === 'failed'
    && /^\d+$/u.test(state.mainPid) && Number(state.mainPid) === 0
    && /^\d+$/u.test(controlPid) && Number(controlPid) === 0;
  if (state.loadState !== 'loaded' || (!activeGeneration && !activatingGeneration && !inactiveGeneration && !failedQuiescent)
    || !/^\d+$/u.test(state.nRestarts)) {
    fail('INVENTORY_CUTOVER_SERVICE_STATE_INVALID', `writer service is neither active nor quiescent:${unit}`, state);
  }
  ...
  for (const unit of names) {
    ...
    try {
      ({stdout} = await execFileImpl('systemctl', [
        'show', '--no-pager',
        '--property=LoadState,ActiveState,SubState,MainPID,ControlPID,ExecMainStartTimestamp,NRestarts',
        unit,
...
const COMPATIBILITY_RELEASE_IDENTITY_KEYS = Object.freeze([
...
function sameCompatibilityReleaseIdentity(left, right) {
  const a = inventoryCompatibilityAuthority(left);
  const b = inventoryCompatibilityAuthority(right);
  return COMPATIBILITY_RELEASE_IDENTITY_KEYS.every(key => a[key] === b[key]);
}
...
export async function captureInventoryCutoverDeploymentAuthority({
  ...
  if (deployment.exists && deployment.json?.inventoryWriterAuthority !== undefined) {
    const formalMarkerValidation = validateDeployedReleaseMarker(deployment.json, {requireV3: true});
    if (!formalMarkerValidation.ok) {
      fail('INVENTORY_CUTOVER_AUTHORITY_INVALID', `formal deployment marker with inventoryWriterAuthority is invalid: ${formalMarkerValidation.issues.join(',')}`, {
        issues: formalMarkerValidation.issues,
    ...
    try {
      formalAuthority = await resolveFormalInventoryWriterAuthority({
        ...
        markerFile: deployment.file,
        cwd,
        releaseAttestationRoot,
        trustPolicyFile,
      });
    } catch (error) {
      fail('INVENTORY_CUTOVER_AUTHORITY_INVALID', error.message, {error});
    }
    if (!formalAuthority?.ok) {
      fail('INVENTORY_CUTOVER_AUTHORITY_INVALID', 'formal inventoryWriterAuthority failed resolution');
    }
  }
  ...
  let expectedCommit = '';
  if (formalAuthority?.ok) {
    expectedCommit = deployment.json.commit;
  } else if (emergencyValid) {
    expectedCommit = emergency.receipt.commit;
  } else if (formalValid) {
    expectedCommit = deployment.json.commit;
  }
  ...
  const source = sourceInspector({cwd, expectedCommit});
  ...
  let bundleSha256;
  if (formalAuthority?.ok && deployment.json.commit === source.head) {
  ...
  } else if (emergencyValid && emergency.receipt.commit === source.head) {
  ...
  for (let index = 0; index < records.length; index += 1) {
    ...
    if (record.kind === 'compatibility_rotation_staged') {
      if (!activeRecord || pendingStage || !exactKeys(record, [
        ...
        || !HASH_RE.test(text(record.currentStateHash))) {
    ...
    if (record.kind === 'compatibility_rotation_finalized') {
      if (!activeRecord || !pendingStage || !exactKeys(record, [
        ...
        || !HASH_RE.test(text(record.currentStateHash))
        || !sameCompatibilityReleaseIdentity(record.authority, pendingStage.candidateAuthority)) {
...
export async function requireCurrentInventoryCutoverActivation(options = {}) {
  ...
  if (!sameCompatibilityReleaseIdentity(state.activeAuthority, authority)) {
    if (state.compatibility.pendingStage
      && sameCompatibilityReleaseIdentity(state.compatibility.pendingStage.candidateAuthority, authority)) {
...
export async function stageInventoryCompatibilityRotation({
  ...
  return withInventoryCutoverLock(async () => {
    ...
    const currentIsActive = sameCompatibilityAuthority(current, state.activeRecord.authority)
      || sameCompatibilityReleaseIdentity(current, state.activeRecord.authority);
    const currentIsCandidate = state.pendingStage
      && sameCompatibilityReleaseIdentity(current, state.pendingStage.candidateAuthority);
    // A deployment can legitimately happen before the compatibility journal is
    // rotated (for example, after a release command was interrupted).  Permit
    // recovery only when the caller's candidate is the exact current release
    // identity.  This keeps the external guard fail-closed for arbitrary drift
    // while allowing the supported stage/finalize path to repair a stale active
    // generation without resurrecting an old receipt or checkout.
    const currentIsRequestedCandidate = sameCompatibilityAuthority(current, candidate);
    if (!currentIsActive && !currentIsCandidate && !currentIsRequestedCandidate) {
    ...
    if (!currentIsActive && !currentIsRequestedCandidate) {
      fail('INVENTORY_CUTOVER_ROTATION_CURRENT_AUTHORITY_INVALID', 'stage requires active generation or exact current candidate recovery');
    }
    const recordedAt = now().toISOString();
...
export async function finalizeInventoryCompatibilityRotation({
  ...
  return withInventoryCutoverLock(async () => {
    ...
    if (!sameCompatibilityReleaseIdentity(current, state.pendingStage.candidateAuthority)) {
