...
export function createOwnerKnowledgeService({repository, authorityId = 'dushengyi', distributionPublisher = null} = {}) {
  ...
  async function listReviews({actor, status = 'pending', limit = 500} = {}) {
    ...
    const reviewableSources = new Set(['owner_task_completion', 'owner_codex_turn_completion', 'owner_bi_task_completion', 'historical_gpt_audit', 'owner_explicit_reconcile']);
    return versions.map(row => publicRule(row.record)).filter(Boolean).filter(rule => reviewableSources.has(String(rule.source?.kind || ''))).map(rule => ({
      rule,
  ...
  async function status() {
    ...
    const reviewableSources = new Set(['owner_task_completion', 'owner_codex_turn_completion', 'owner_bi_task_completion', 'historical_gpt_audit', 'owner_explicit_reconcile']);
    const pendingCandidates = versions.filter(row => row.record?.activation === 'candidate'
      && reviewableSources.has(String(row.record?.source?.kind || ''))
      && !reviewedVersionIds.has(String(row.record?.versionId || row.recordId))).length;
    return {
      authorityId: principal,
