...
export async function runActivityInventoryTransaction({
  ...
  try {
    ...
    try {
      ...
      // A submit that reports the target already exactly covered performed no
      // write, so there is no post-submit enrollment to read back. Requiring a
      // fresh readback here downgraded a verified success into
      // submit_or_pre_submit_failed, which then left the row unaccounted for in
      // the terminal report and kept the whole repair queue pending
      // (2026-09-18: 33 fallback rows, no daily report for ten days).
      const submitPerformedNoWrite = result?.full?.status === 'already_exactly_covered'
        || result?.full?.alreadyCovered === true
        || result?.alreadyCovered === true;
      if (submitPerformedNoWrite) {
        evidence.submitAttempted = false;
        evidence.submitMutationState = 'not_started';
        evidence.remoteMutationStarted = false;
        evidence.noWriteRequired = true;
      }
      if (!submitPerformedNoWrite) evidence.enrollmentReadback = await readEnrollment({
        phase: 'after_submit_before_restore',
        transactionHash: evidence.transactionHash,
        submitResult: result,
      });
      const enrollment = submitPerformedNoWrite ? {ok: true, noWriteRequired: true} : validateEnrollment(evidence.enrollmentReadback, {
        phase: 'after_submit_before_restore',
        submitResult: result,
      });
      if (!enrollment?.ok) {
