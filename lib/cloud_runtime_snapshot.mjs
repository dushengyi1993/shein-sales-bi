...
export function buildCloudRuntimeSnapshot({
  ...
  const intentionallyPausedTimers = [];
  ...
  for (const name of CLOUD_TIMER_UNITS) {
    ...
    } else if (intentionalCloudTimerPause(name, unit)) {
      intentionallyPausedTimers.push(intentionalCloudTimerPause(name, unit));
    } else {
