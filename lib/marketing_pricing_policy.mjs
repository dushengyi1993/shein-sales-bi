...
export const DEFAULT_MARKETING_PRICING_POLICY = Object.freeze({
  ...
  topTreatmentCostFallback: {
    enabled: true,
    costMapPath: 'tmp/mbrs/marketing-cost-map.json',
    defaultBaseMarginPct: 30,
    selectionCostBasis: 'product_cost_excluding_storage',
    missingStorageDoesNotBlock: true,
  },
  limitedDiscount: {
  ...
  newListingWithin7Days: {
    ...
    platformNewLabelPatterns: ['新款', '新品', 'New Arrivals', 'new arrival'],
  ...
  relistedWithoutActiveMarketing: {
    enabled: true,
    historyLookbackDays: 60,
    onShelfOnly: true,
    requireLatestSourceHasNoActivity: true,
    requireNoBiMarketingSignal: true,
    requireCompleteLiveMarketingScan: true,
    futureMarketingDoesNotSuppressCurrentFallback: true,
    pricingTreatment: 'same_as_global_exposure_top5',
    limitedDiscount: {
      autoFallback: true,
      durationDays: 7,
      activityNamePrefix: '重新上架无活动Top5兜底限时折扣',
    },
  },
});
...
export async function loadMarketingPricingPolicy(file) {
  ...
  return normalizeMarketingPricingPolicy(configured);
}

export function normalizeMarketingPricingPolicy(configured = {}) {
...
export function buildExposureTopLinkIndex(bi, policy = DEFAULT_MARKETING_PRICING_POLICY) {
  ...
  const rankedRowCount = [...rowsByGroup.values()].reduce((sum, rows) => sum + rows.length, 0);
  return {
    enabled,
    topN,
    metricFields,
    rowCount: candidates.length,
    positiveMetricRowCount,
    rankedRowCount,
...
export function deriveTopTreatmentTargetFromCost({canonical, costDoc, policy = DEFAULT_MARKETING_PRICING_POLICY} = {}) {
  const cfg = policy?.topTreatmentCostFallback || DEFAULT_MARKETING_PRICING_POLICY.topTreatmentCostFallback;
  if (cfg?.enabled === false) return {price: null, source: 'cost_fallback_disabled', available: false};
  const costEvidence = findMarketingCostEvidence(costDoc, canonical);
  if (!Number.isFinite(costEvidence.productUnitCostSar) || costEvidence.productUnitCostSar <= 0) {
    return {
      price: null,
      source: 'missing_product_cost',
      available: false,
      ...costEvidence,
    };
  }
  const baseMargin = pctConfigToRatio(cfg?.defaultBaseMarginPct ?? 30);
  const targets = marginTargetsForExposurePolicy(baseMargin, policy);
  const floorMargin = pctConfigToRatio(policy?.targetFloorMarginPct ?? policy?.exposureTopLinks?.floorMarginPct ?? 15);
  const topMargin = Number.isFinite(targets?.topMargin) ? Math.max(floorMargin, targets.topMargin) : floorMargin;
  const rawPrice = costEvidence.productUnitCostSar / (1 - topMargin);
  const floorPrice = costEvidence.productUnitCostSar / (1 - floorMargin);
  const price = ceil2(Math.max(rawPrice, floorPrice));
  return {
    price,
    source: 'derived_from_product_cost_top_treatment_margin',
    available: Number.isFinite(price) && price > 0,
    productUnitCostSar: costEvidence.productUnitCostSar,
    storageUnitCostSar: costEvidence.storageUnitCostSar,
    trueUnitCostSar: costEvidence.trueUnitCostSar,
    storageMethod: costEvidence.storageMethod,
    selectionCostBasis: cfg?.selectionCostBasis || 'product_cost_excluding_storage',
    missingStorageDoesNotBlock: cfg?.missingStorageDoesNotBlock !== false,
    baseMargin,
    topMargin,
    floorMargin,
    costKey: costEvidence.costKey,
  };
}

export function findMarketingCostEvidence(costDoc, canonical) {
  const wanted = canonicalCostKey(canonical);
  const trueCostMap = costDoc?.trueCostMap || {};
  const costMap = costDoc?.costMap || {};
  const keys = [...new Set([...Object.keys(trueCostMap), ...Object.keys(costMap)])];
  const costKey = keys.find(key => canonicalCostKey(key) === wanted)
    || keys.find(key => canonicalCostKey(key).startsWith(wanted) || wanted.startsWith(canonicalCostKey(key)))
    || '';
  const trueCost = costKey ? (trueCostMap[costKey] || {}) : {};
  const productUnitCostSar = numberOrNull(trueCost?.unitCostSar) ?? numberOrNull(costKey ? costMap[costKey] : null);
  const storageUnitCostSar = numberOrNull(trueCost?.storageUnitCostSar ?? trueCost?.storageUnitCostSar30d);
  const trueUnitCostSar = numberOrNull(trueCost?.trueUnitCostSar)
    ?? (productUnitCostSar !== null && storageUnitCostSar !== null ? round2(productUnitCostSar + storageUnitCostSar) : null);
  return {
    costKey,
    productUnitCostSar,
    storageUnitCostSar,
    trueUnitCostSar,
    storageMethod: String(trueCost?.storageMethod || (storageUnitCostSar === null ? 'missing' : 'known')).trim(),
  };
}

function canonicalCostKey(value) {
  return String(value || '')
    .normalize('NFKC')
    .replace(/\s+/g, '')
    .replace(/[()（）【】\[\]_:：/\\-]/g, '')
...
export function isRecentNewListingLink(link, policy = DEFAULT_MARKETING_PRICING_POLICY, reportDate = '') {
  ...
  const platformNewLabel = platformNewListingLabelInfo(link, policy);
  if (age.value !== null && age.value > windowDays) {
    return {applies: false, reason: 'outside_window', shelfAgeDays: age.value, shelfAgeSource: age.source, windowDays, platformNewLabel};
  }
  if (!platformNewLabel.applies) {
    if (age.value === null) return {applies: false, reason: 'missing_shelf_age', shelfAgeDays: null, shelfAgeSource: age.source, windowDays, platformNewLabel};
  }
  if (cfg.requireNoOrdinaryMarketing !== false && hasOrdinaryMarketingEvidence(link)) {
    return {applies: false, reason: 'ordinary_marketing_exists', shelfAgeDays: age.value, shelfAgeSource: age.source, windowDays, platformNewLabel};
  }
  return {
    applies: true,
    reason: platformNewLabel.applies ? 'platform_new_label_no_ordinary_marketing' : 'recent_new_listing_no_ordinary_marketing',
    shelfAgeDays: age.value,
    shelfAgeSource: platformNewLabel.applies ? `platform_label:${platformNewLabel.source}` : age.source,
    windowDays,
    platformNewLabel,
  };
}

export function platformNewListingLabelInfo(link, policy = DEFAULT_MARKETING_PRICING_POLICY) {
  const cfg = policy?.newListingWithin7Days || DEFAULT_MARKETING_PRICING_POLICY.newListingWithin7Days;
  const patterns = Array.isArray(cfg.platformNewLabelPatterns) && cfg.platformNewLabelPatterns.length
    ? cfg.platformNewLabelPatterns
    : DEFAULT_MARKETING_PRICING_POLICY.newListingWithin7Days.platformNewLabelPatterns;
  const fields = [
    ['skc_label', link?.skc_label],
    ['inventory_goods_level', link?.inventory_goods_level],
    ['activity_label', link?.activity_label],
    ['shelf_status_name', link?.shelf_status_name],
    ['skc_tags', Array.isArray(link?.skc_tags) ? link.skc_tags.join(' ') : link?.skc_tags],
  ];
  for (const [source, value] of fields) {
    const text = String(value || '').trim();
    if (!text) continue;
    const matched = patterns.find(pattern => new RegExp(escapeRegExp(pattern), 'i').test(text));
    if (matched) return {applies: true, source, value: text, pattern: matched};
  }
  return {applies: false, source: '', value: '', pattern: ''};
}
...
export function marketingLinkKey(storeKey, skc) {
  return exposureLinkKey(storeKey, skc);
}
