...
function normalizedLogicalPath(value, label = 'logical path') {
  ...
  if (slashPath.startsWith('/') || /^[A-Za-z]:/.test(slashPath)) {
    throw new Error(`${label} must be a relative logical path without absolute, drive, or UNC prefixes`);
  }
  const normalized = path.posix.normalize(slashPath);
  if (!normalized || normalized === '.') {
    throw new Error(`${label} must resolve to a non-empty logical path`);
  }
  return normalized;
}
...
// Keep fingerprints in the logical checkout namespace. A physical runtime
// path is admitted only when the checkout domain actually aliases that root.
async function resolveManifestPath(root, file, label) {
  const raw = String(file || '');
  if (!raw || raw !== raw.trim() || /[\r\n\0]/.test(raw)
    || raw.replaceAll('\\', '/').split('/').includes('..')) {
    throw new Error(`${label} has an invalid path`);
  }
  const absolute = path.resolve(raw);
  let logical = absolute;
  let boundary = await fs.realpath(root);
  for (const domain of ['outputs', 'state']) {
    const base = path.join(root, domain);
    let actual;
    try { actual = await fs.realpath(base); }
    catch (error) { if (error.code === 'ENOENT') continue; throw error; }
    const configured = process.env[`SHEIN_BI_${domain.toUpperCase()}_ROOT`]
      || (root === '/opt/shein-bi/app' ? `/data/shein-bi/${domain}` : actual);
    for (const candidate of [base, path.resolve(configured), actual]) {
      const relative = path.relative(candidate, absolute);
      if (!relative || relative.startsWith('..') || path.isAbsolute(relative)) continue;
      const [baseStat, mappedStat] = await Promise.all([fs.stat(base), fs.stat(configured)]);
      if (!baseStat.isDirectory() || !mappedStat.isDirectory()
        || baseStat.dev !== mappedStat.dev || baseStat.ino !== mappedStat.ino) {
        throw new Error(`${label} runtime root mapping mismatch`);
      }
      logical = path.join(base, relative);
      boundary = actual;
      break;
    }
    if (boundary === actual) break;
  }
  assertInside(root, logical, label);
  const [inputStat, logicalStat] = await Promise.all([fs.lstat(absolute), fs.lstat(logical)]);
  if (inputStat.isSymbolicLink() || logicalStat.isSymbolicLink()
    || inputStat.dev !== logicalStat.dev || inputStat.ino !== logicalStat.ino) {
    throw new Error(`${label} must be the same non-symlink runtime artifact`);
  }
  assertInside(boundary, await fs.realpath(logical), label);
  // Also open regular files with the existing non-symlink check before callers
  // use their logical name. Directory arguments are only used as plan roots.
  if (logicalStat.isFile()) await readRegularFile(logical, label);
  return logical;
}
...
async function readRegularFile(file, label) {
  ...
  try {
    ...
    if (!same(stat, opened)) throw new Error(`${label} identity changed before read`);
    const bytes = await handle.readFile();
    const after = await handle.stat();
    const current = await fs.lstat(file);
    if (!same(opened, after) || !same(after, current) || current.isSymbolicLink()) {
...
async function loadGuardPriceOverridesBinding({root, guard}) {
  ...
  const rawPath = String(guard?.targetPlanSelection?.priceOverrides ?? '');
  const logicalPath = normalizedLogicalPath(rawPath, 'Guard targetPlanSelection.priceOverrides');
...
export async function loadExactHighClickSpecialPlan({root, planPath, guardPath, date}) {
  const resolvedRoot = path.resolve(root);
  const resolvedPlanPath = await resolveManifestPath(resolvedRoot, planPath, 'repair plan');
  const resolvedGuardPath = await resolveManifestPath(resolvedRoot, guardPath, 'guard report');
  ...
  const planText = (await readRegularFile(resolvedPlanPath, 'repair artifact')).toString('utf8');
  const plan = JSON.parse(planText);
  const guardText = (await readRegularFile(resolvedGuardPath, 'repair artifact')).toString('utf8');
...
export async function loadExactManualRepairPlan({root, planPath, guardPath, date}) {
  ...
  for (const rawEntry of rawEntries) {
    ...
    const rescueText = (await readRegularFile(rescuePath, 'repair artifact')).toString('utf8');
...
export async function loadExactDriftRepairManifest({root, planDir, guardPath, date}) {
  const resolvedRoot = path.resolve(root);
  const resolvedPlanDir = await resolveManifestPath(resolvedRoot, planDir, 'drift plan directory');
  const resolvedGuardPath = await resolveManifestPath(resolvedRoot, guardPath, 'guard report');
  ...
  const manifest = JSON.parse((await readRegularFile(manifestPath, 'repair artifact')).toString('utf8'));
...
export async function loadExactFallbackRepairPlan({root, planPath, guardPath, date}) {
  ...
  const guardRelative = normalizedRelative(resolvedRoot, resolvedGuardPath);
  const planGuardPath = path.resolve(resolvedRoot, String(plan.sourceGuard || ''));
  assertInside(resolvedRoot, planGuardPath, 'fallback plan sourceGuard');
  if (!plan.sourceGuard || normalizedAbsolute(planGuardPath) !== normalizedAbsolute(resolvedGuardPath)) {
    throw new Error(`Fallback plan sourceGuard mismatch: expected=${guardRelative} actual=${plan.sourceGuard || 'missing'}`);
  }
  const guardLogicalPath = normalizedLogicalPath(plan.sourceGuard, 'Fallback plan sourceGuard');
  const actualGuardLogicalPath = normalizedLogicalPath(guardRelative, 'Fallback guard path');
  if (guardLogicalPath !== actualGuardLogicalPath) {
    throw new Error(`Fallback plan sourceGuard identity drift: expected=${actualGuardLogicalPath} actual=${guardLogicalPath}`);
  }
  ...
  const planPriceOverridesRawPath = String(plan.sourcePriceOverrides ?? '');
  const planPriceOverridesLogicalPath = normalizedLogicalPath(planPriceOverridesRawPath, 'Fallback plan price-overrides source');
  const planPriceOverridesPath = path.resolve(resolvedRoot, planPriceOverridesRawPath);
  if (planPriceOverridesLogicalPath !== priceBinding.relativePath) {
    throw new Error(
      `Fallback plan price-overrides declared source mismatch: guard=${priceBinding.relativePath} plan=${plan.sourcePriceOverrides || 'missing'}`,
    );
  }
  ...
  for (const rawEntry of rawEntries) {
    ...
    const declaredRescuePath = normalizedLogicalPath(rawEntry?.path, 'Fallback rescue path');
    const rescuePath = path.resolve(resolvedRoot, String(rawEntry?.path || ''));
    assertInside(resolvedRoot, rescuePath, 'fallback rescue file');
    const actualRescuePath = normalizedLogicalPath(normalizedRelative(resolvedRoot, rescuePath), 'Fallback rescue path');
    if (declaredRescuePath !== actualRescuePath) {
      throw new Error(`Fallback rescue path identity drift: declared=${declaredRescuePath} actual=${actualRescuePath}`);
    }
    const pathKey = normalizedAbsolute(rescuePath);
    if (seenPaths.has(pathKey)) throw new Error(`Duplicate fallback rescue path in plan: ${rawEntry.path}`);
    seenPaths.add(pathKey);
    const rescueText = (await readRegularFile(rescuePath, 'Fallback rescue file')).toString('utf8');
    const rescue = JSON.parse(rescueText);
    if (String(rescue.storeKey || '').trim().toUpperCase() !== storeKey) {
    ...
    const rescuePriceOverridesRawPath = String(rescue.sourcePriceOverrides ?? '');
    const rescuePriceOverridesLogicalPath = normalizedLogicalPath(rescuePriceOverridesRawPath, 'Fallback rescue price-overrides source');
    const rescuePriceOverridesPath = path.resolve(resolvedRoot, rescuePriceOverridesRawPath);
    if (rescuePriceOverridesLogicalPath !== priceBinding.relativePath) {
      throw new Error(
        `Fallback rescue price-overrides declared source mismatch: guard=${priceBinding.relativePath} rescue=${rescue.sourcePriceOverrides || 'missing'}`,
      );
    }
    ...
    entries.push({
      ...
      storeKey,
      path: rescuePath,
      relativePath: declaredRescuePath,
      contentHash: hashText(rescueText),
      rescue,
    });
  }
