...
import {
  MARKETING_PLAN_REGISTRY_ENV,
  validateMarketingPlanPairFilesSync,
  verifyMarketingPlanRegistrySync,
  resolveMarketingPlanRegistryFile,
} from './marketing_plan_registry.mjs';
...
export function resolveCurrentMarketingPlanPair({
  ...
  if (targetPlanExplicit !== priceOverridesExplicit) {
    throw new Error('Operational marketing plan selection requires both exact --target-plan and --price-overrides files; inferred pairs are offline-only.');
  }
  if (targetPlanExplicit && priceOverridesExplicit) {
    return explicitMarketingPlanPair({targetPlan, priceOverrides, root, strategy: 'explicit_both'});
  }
  const configuredRegistryFile = resolveMarketingPlanRegistryFile({registryFile});
  if (configuredRegistryFile) {
    const expectedStoreKeys = enabledStoreKeysForPlanSelection(root);
    if (!expectedStoreKeys.length) {
      throw new Error(`Marketing plan registry requires at least one enabled store in ${path.join(root, 'config', 'stores.json')}`);
    }
    const registry = verifyMarketingPlanRegistrySync({
      registryFile: configuredRegistryFile,
      registryRoot: path.dirname(configuredRegistryFile),
      expectedStoreKeys,
      allowEnabledStoreSubset: true,
    });
    return {
      targetPlan: registry.targetPlan,
      priceOverrides: registry.priceOverrides,
      ...
      selectionPlanHash: registry.selectionPlanHash,
      priceOverridesHash: registry.priceOverridesHash,
      selectionPayloadHash: registry.selectionPayloadHash,
      pricePayloadHash: registry.pricePayloadHash,
      workFingerprint: registry.workFingerprint,
      rowCount: registry.rowCount,
      priceRowCount: registry.rowCount,
      storeCoverage: registry.storeKeys,
      storeCoverageComplete: registry.storeCoverageComplete,
      missingEnabledStoreKeys: registry.missingEnabledStoreKeys,
      activityBatch: registry.activityBatch,
      promotedAt: registry.promotedAt,
      scoreReasons: ['registry_current_baseline_exact_pointer'],
      rejectedCandidates: [],
    };
  }
  const registryHint = process.env[MARKETING_PLAN_REGISTRY_ENV]
    ? ` registry=${process.env[MARKETING_PLAN_REGISTRY_ENV]}`
    : '';
  throw new Error(`Operational current marketing plan resolution requires a verified durable registry current.json; registry is not configured${registryHint}. Local tmp/mtime discovery is offline-only and was not selected.`);
}

export function discoverOfflineMarketingPlanPair({
  root = process.cwd(),
  nowMs = Date.now(),
} = {}) {
  const candidate = selectLatestMarketingPlanPair(root, nowMs);
  if (!candidate) {
    throw new Error('No non-authoritative offline marketing plan pair found under tmp/marketing-signup; offline discovery never establishes a current plan.');
  }
  const evidence = validateMarketingPlanPairFilesSync({
    selectionPath: candidate.targetPlan,
    priceOverridesPath: candidate.priceOverrides,
  ...
  return {
    ...candidate,
    ...evidence,
    strategy: 'offline_non_authoritative',
    authority: 'offline',
    authoritative: false,
    isCurrent: false,
    current: false,
    currentSource: 'tmp/marketing-signup-mtime-discovery',
    registryFile: '',
    registryHash: '',
    selectionPlanHash: evidence.selectionSha256,
    priceOverridesHash: evidence.priceOverridesSha256,
  };
}

function explicitMarketingPlanPair({targetPlan, priceOverrides, root, strategy}) {
  const expectedStoreKeys = enabledStoreKeysForPlanSelection(root);
  if (!expectedStoreKeys.length) {
    throw new Error(`Operational marketing plan pair requires at least one enabled store in ${path.join(root, 'config', 'stores.json')}`);
  }
  const evidence = validateMarketingPlanPairFilesSync({
    selectionPath: targetPlan,
    priceOverridesPath: priceOverrides,
    requireCurrentBaseline: true,
    expectedStoreCount: expectedStoreKeys.length,
    expectedStoreKeys,
  });
  return {
    targetPlan: evidence.selectionPath,
    priceOverrides: evidence.priceOverridesPath,
    strategy,
    registryFile: '',
    registryHash: '',
    selectionPlanHash: evidence.selectionSha256,
    priceOverridesHash: evidence.priceOverridesSha256,
    selectionPayloadHash: evidence.selectionPayloadHash,
    pricePayloadHash: evidence.pricePayloadHash,
    workFingerprint: evidence.workFingerprint,
    rowCount: evidence.rowCount,
    priceRowCount: evidence.rowCount,
    storeCoverage: evidence.storeKeys,
    activityBatch: evidence.activityBatch,
    promotedAt: evidence.promotedAt,
    scoreReasons: ['explicit_plan_pair'],
    rejectedCandidates: [],
  };
}
...
function selectLatestMarketingPlanPair(root, nowMs = Date.now()) {
  ...
  for (const targetPlan of discoverSelectionPlans(marketingSignupDir)) {
...
function discoverSelectionPlans(marketingSignupDir) {
  const files = [];
  const inspect = dir => {
    for (const entry of fsSync.readdirSync(dir, {withFileTypes: true})) {
      if (entry.isFile() && /^selection-plan-.*\.json$/i.test(entry.name)) {
        files.push(path.join(dir, entry.name));
      }
    }
  };
  inspect(marketingSignupDir);
  // Current owner-approved baselines are intentionally kept in dated/locked
  // runtime folders. Looking only at the signup root silently fell back to a
  // removed 2026-06 legacy file after the runtime tree was cleaned. One level
  // is sufficient for the managed layout and avoids crawling large scan trees.
  for (const entry of fsSync.readdirSync(marketingSignupDir, {withFileTypes: true})) {
    if (!entry.isDirectory()) continue;
    inspect(path.join(marketingSignupDir, entry.name));
  }
  return files;
}
...
function marketingPlanCandidateMeta(file, priceFile, enabledStores = [], root, nowMs = Date.now()) {
  ...
  const planStatuses = [selectionDoc?.planMetadata?.status, priceDoc?.planMetadata?.status]
    .map(value => String(value || '').trim().toLowerCase());
  if (planStatuses.includes('offline_candidate')) rejectReasons.push('plan_metadata_offline_candidate');
  ...
  if (name.includes('all-ready')) {
    add(-700, 'legacy_all_ready');
    rejectReasons.push('legacy_all_ready_filename');
  }
