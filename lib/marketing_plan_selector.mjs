...
export function resolveCurrentMarketingPlanPair({
  ...
  if (configuredRegistryFile) {
    ...
    if (![19, 21].includes(expectedStoreKeys.length)) {
      throw new Error(`Marketing plan registry requires exactly 21 enabled stores in ${path.join(root, 'config', 'stores.json')}; got=${expectedStoreKeys.length}`);
    }
...
function explicitMarketingPlanPair({targetPlan, priceOverrides, root, strategy}) {
  const expectedStoreKeys = enabledStoreKeysForPlanSelection(root);
  if (![19, 21].includes(expectedStoreKeys.length)) {
    throw new Error(`Operational marketing plan pair requires exactly 21 enabled stores in ${path.join(root, 'config', 'stores.json')}; got=${expectedStoreKeys.length}`);
  }
  const evidence = validateMarketingPlanPairFilesSync({
