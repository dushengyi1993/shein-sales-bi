...
export function resolveCurrentMarketingPlanPair({
  ...
  if (configuredRegistryFile) {
    ...
    if (expectedStoreKeys.length !== 19) {
      throw new Error(`Marketing plan registry requires exactly 19 enabled stores in ${path.join(root, 'config', 'stores.json')}; got=${expectedStoreKeys.length}`);
    }
...
function explicitMarketingPlanPair({targetPlan, priceOverrides, root, strategy}) {
  const expectedStoreKeys = enabledStoreKeysForPlanSelection(root);
  if (expectedStoreKeys.length !== 19) {
    throw new Error(`Operational marketing plan pair requires exactly 19 enabled stores in ${path.join(root, 'config', 'stores.json')}; got=${expectedStoreKeys.length}`);
  }
  const evidence = validateMarketingPlanPairFilesSync({
