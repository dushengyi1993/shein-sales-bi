...
import {acquireFileLock} from './link_ops_json_repository.mjs';
...
export async function writeOpenApiProductCacheAtomically(file, payload, {storeKey, generatedAt = ''} = {}) {
  const release = await acquireFileLock(path.resolve(file), {timeoutMs: 10000, staleMs: 60000});
  try {
    return await writeOpenApiProductCacheUnlocked(file, payload, {storeKey, generatedAt});
  } finally { await release(); }
}

// All cache publishers use the same cross-process lock. Re-read inside the
// lock so a targeted read-through never overwrites an intervening collection.
export async function updateOpenApiProductCacheAtomically(file, transform, {storeKey} = {}) {
  const release = await acquireFileLock(path.resolve(file), {timeoutMs: 10000, staleMs: 60000});
  try {
    let current = null;
    const exists = await fs.stat(file).then(() => true, error => { if (error.code === 'ENOENT') return false; throw error; });
    if (exists) current = (await readOpenApiProductCache(file, {expectedStore: storeKey})).data;
    const payload = await transform(current);
    return await writeOpenApiProductCacheUnlocked(file, payload, {storeKey});
  } finally { await release(); }
}

async function writeOpenApiProductCacheUnlocked(file, payload, {storeKey, generatedAt = ''} = {}) {
