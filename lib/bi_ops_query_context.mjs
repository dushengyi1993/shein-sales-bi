...
async function readJsonBoundedAdmitted({file, maxBytes, maxAggregateBytes, aggregateBytes, signal, gateBytes, admitted}) {
  ...
  try {
    ...
    throwIfQueryAborted(signal);
    if (!admitted && stat.size >= gateBytes) {
    ...
    if (stat.size > remaining) {
      return {ok: false, status: 'aggregate_too_large', size: stat.size, maxAggregateBytes, remaining: Math.max(0, remaining)};
    }
    ...
    throwIfQueryAborted(signal);
    const raw = await readQueryFileAtOpenedSize(handle, stat.size, signal);
    ...
    return {
      ok: true,
      value: JSON.parse(decodeUtf8Strict(raw).replace(/^\uFEFF/, '')),
      size: actualBytes,
    };
  } catch (error) {
    throwIfQueryAborted(signal);
