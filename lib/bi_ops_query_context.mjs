...
import {
  biSectionArtifactIdentityMatches,
  readBiProfitBundleManifest,
  BI_SECTION_PARSE_GATE_BYTES,
  withBiSectionParseSlot,
} from './bi_section_cache.mjs';
...
export const DEFAULT_BI_OPS_SECTION_MAX_BYTES = 64 * 1024 * 1024;
export const DEFAULT_BI_OPS_AGGREGATE_MAX_BYTES = 128 * 1024 * 1024;

const SECTION_KEY_SET = new Set(BI_OPS_SECTION_KEYS);
const STORE_KEYS = new Set(['DL', 'DX', 'FY', 'LQ', 'NM', 'HL', 'JY', 'ZL', 'TS', 'MZ', 'CX', 'YJ', 'XL', 'QY', 'QH', 'TZ', 'JSH', 'TZZ', 'XC', 'LG', 'HY', 'DSY', 'LGM']);
const RESERVED_SECTION_DATA_KEYS = new Set(['generatedAt', '__sections']);
const DANGEROUS_OBJECT_KEYS = new Set(['__proto__', 'prototype', 'constructor']);
const PROFIT_QUERY_ARTIFACT = 'profit.query';
const PROFIT_QUERY_ARTIFACT_KIND = 'profit-query-compact-v1';
const PROFIT_QUERY_RETAINED_PATHS = Object.freeze([
  'profit.dailyStoreProducts',
  'profit.monthGroups',
  'profit.products',
  'profit.storeStorageDaily',
]);
const PROFIT_QUERY_OMITTED_PATHS = Object.freeze([
...
function sectionArtifactFile(sectionsDir, artifactName) {
  const artifact = String(artifactName || '').trim();
  if (!/^[A-Za-z0-9_-]+(?:\.[A-Za-z0-9_-]+)*$/u.test(artifact)) {
    throw new Error(`Unsafe BI section artifact path: ${artifactName}`);
  }
  const root = path.resolve(sectionsDir);
  const file = path.resolve(root, `${artifact}.json`);
  if (path.dirname(file) !== root) throw new Error(`Unsafe BI section artifact path: ${artifactName}`);
  return file;
}

function throwIfQueryAborted(signal) {
...
function queryFileIdentity(stat) {
  return {
...
function queryFileIdentityMatches(left, right) {
...
function queryIdentityChanged(message) {
  const error = new Error(message);
  error.code = 'BI_QUERY_IDENTITY_CHANGED';
  return error;
}

async function verifyQueryIdentityReceipt(receipt) {
  if (!receipt) return;
  let current;
  try {
    current = await fs.stat(receipt.file, {bigint: true});
  } catch {
    throw queryIdentityChanged(`BI query final pathname is unavailable: ${receipt.file}`);
  }
  if (!current.isFile() || !queryFileIdentityMatches(queryFileIdentity(current), receipt.identity)) {
    throw queryIdentityChanged(`BI query final pathname no longer names the opened file: ${receipt.file}`);
  }
}

// Test-only FileHandle lifecycle seam. Production workers never set this; the
// focused lifecycle test uses it to observe cleanup, land cancellation, and
// coordinate a normal atomic publication immediately before fs.open. It never
// alters budget, read, or parse semantics.
let _fileHandleLifecycleHooks = null;
export function __setFileHandleLifecycleHooks(hooks) {
  _fileHandleLifecycleHooks = hooks || null;
}

async function readQueryFileAtOpenedSize(handle, openedSize, signal) {
  const raw = Buffer.allocUnsafe(openedSize);
  let offset = 0;
  while (offset < openedSize) {
    throwIfQueryAborted(signal);
    const length = Math.min(256 * 1024, openedSize - offset);
    const {bytesRead} = await handle.read(raw, offset, length, offset);
    throwIfQueryAborted(signal);
    if (bytesRead === 0) break;
    offset += bytesRead;
  }
  return offset === openedSize ? raw : raw.subarray(0, offset);
}

// The path stat is only the cheap admission check. Once a file is classified
// as large, the slot owns open, fstat, read, strict decode, JSON.parse, and
// close together; no payload bytes are read while waiting for admission.
async function readJsonBounded({file, maxBytes, maxAggregateBytes, aggregateBytes, signal, parseGateBytes}) {
  throwIfQueryAborted(signal);
  let admissionStat;
  try {
    admissionStat = await fs.stat(file);
  } catch (error) {
    throwIfQueryAborted(signal);
  ...
  if (!admissionStat.isFile()) return {ok: false, status: 'not_file'};
  if (admissionStat.size > maxBytes) return {ok: false, status: 'too_large', size: admissionStat.size, maxBytes};
  const remaining = maxAggregateBytes - aggregateBytes;
  if (admissionStat.size > remaining) {
    return {
      ok: false,
      status: 'aggregate_too_large',
  ...
  const gateBytes = finitePositive(parseGateBytes, BI_SECTION_PARSE_GATE_BYTES);
  const read = admitted => readJsonBoundedAdmitted({
    file,
    maxBytes,
    maxAggregateBytes,
    aggregateBytes,
  ...
  return admissionStat.size >= gateBytes
    ? withBiSectionParseSlot(() => read(true), {signal})
    : read(false);
}

async function closeQueryFileHandle(handle, {file, signal, admitted}) {
  await handle.close().catch(() => {});
  if (_fileHandleLifecycleHooks?.onClosed) {
    try {
      await _fileHandleLifecycleHooks.onClosed({handle, file, signal, admitted});
    } catch {
      // Test-only observation must not change reader cleanup semantics.
    }
  }
}

async function readJsonBoundedAdmitted({file, maxBytes, maxAggregateBytes, aggregateBytes, signal, gateBytes, admitted}) {
  throwIfQueryAborted(signal);
  let handle;
  let bytesRead = 0;
  try {
    if (_fileHandleLifecycleHooks?.onBeforeOpen) {
      await _fileHandleLifecycleHooks.onBeforeOpen({file, signal, admitted});
    }
    throwIfQueryAborted(signal);
  ...
  // The handle is open from this point: every exit path (cancellation, early
  // budget returns, read/decode errors, success) must await handle.close().
  // The first cancellation check now lives inside the try, so an abort that
  // lands immediately after fs.open still runs the finally close instead of
  // leaking an unclosed FileHandle to the GC finalizer.
  try {
    if (_fileHandleLifecycleHooks?.onOpened) {
      await _fileHandleLifecycleHooks.onOpened({handle, file, signal, admitted});
    }
    throwIfQueryAborted(signal);
    const stat = await handle.stat({bigint: true});
    if (!stat.isFile()) return {ok: false, status: 'not_file'};
    const openedIdentity = queryFileIdentity(stat);
    const openedSize = Number(stat.size);
    throwIfQueryAborted(signal);
    if (!admitted && openedSize >= gateBytes) {
      const unadmittedHandle = handle;
      handle = null;
      await closeQueryFileHandle(unadmittedHandle, {file, signal, admitted});
      throwIfQueryAborted(signal);
      return withBiSectionParseSlot(() => readJsonBoundedAdmitted({
        file,
        maxBytes,
        maxAggregateBytes,
        aggregateBytes,
    ...
    if (openedSize > maxBytes) return {ok: false, status: 'too_large', size: openedSize, maxBytes};
    const remaining = maxAggregateBytes - aggregateBytes;
    throwIfQueryAborted(signal);
    if (openedSize > remaining) {
      return {ok: false, status: 'aggregate_too_large', size: openedSize, maxAggregateBytes, remaining: Math.max(0, remaining)};
    }
    if (_fileHandleLifecycleHooks?.onBeforePayloadRead) {
      await _fileHandleLifecycleHooks.onBeforePayloadRead({handle, file, stat, signal, admitted});
    }
    throwIfQueryAborted(signal);
    const raw = await readQueryFileAtOpenedSize(handle, openedSize, signal);
    bytesRead = raw.length;
    if (_fileHandleLifecycleHooks?.onAfterPayloadRead) {
      await _fileHandleLifecycleHooks.onAfterPayloadRead({handle, file, stat, byteLength: raw.length, signal, admitted});
    }
    throwIfQueryAborted(signal);
    const actualBytes = raw.length;
    if (actualBytes > maxBytes) return {ok: false, status: 'too_large', size: actualBytes, maxBytes};
    const actualRemaining = maxAggregateBytes - aggregateBytes;
    if (actualBytes > actualRemaining) {
      return {ok: false, status: 'aggregate_too_large', size: actualBytes, maxAggregateBytes, remaining: Math.max(0, actualRemaining)};
    }
    let value;
    let parseFailure = null;
    try {
      const decoded = decodeUtf8Strict(raw);
      if (_fileHandleLifecycleHooks?.onAfterDecode) {
        await _fileHandleLifecycleHooks.onAfterDecode({handle, file, stat, byteLength: actualBytes, signal, admitted});
      }
      throwIfQueryAborted(signal);
      value = JSON.parse(decoded.replace(/^\uFEFF/, ''));
      if (_fileHandleLifecycleHooks?.onAfterParse) {
        await _fileHandleLifecycleHooks.onAfterParse({handle, file, stat, byteLength: actualBytes, signal, admitted});
      }
    } catch (error) {
      parseFailure = error;
    }
    ...
    let finalPathStat;
    try {
      if (_fileHandleLifecycleHooks?.onBeforeFinalPathStat) {
        await _fileHandleLifecycleHooks.onBeforeFinalPathStat({handle, file, stat, byteLength: actualBytes, signal, admitted});
      }
      throwIfQueryAborted(signal);
      finalHandleStat = await handle.stat({bigint: true});
      finalPathStat = await fs.stat(file, {bigint: true});
    } catch (error) {
      throwIfQueryAborted(signal);
      return {
        ok: false,
        ...
        size: actualBytes,
        detail: String(error?.code || error?.message || 'final pathname unavailable').slice(0, 240),
      };
    }
    if (!finalHandleStat.isFile()
      || !finalPathStat.isFile()
      || actualBytes !== openedSize
      || !queryFileIdentityMatches(queryFileIdentity(finalHandleStat), openedIdentity)
      || !queryFileIdentityMatches(queryFileIdentity(finalPathStat), openedIdentity)) {
      return {ok: false, status: 'identity_changed', size: actualBytes};
    }
    ...
    return {
      ...
      value,
      size: actualBytes,
      identityReceipt: {file: path.resolve(file), identity: openedIdentity},
    };
  } catch (error) {
    throwIfQueryAborted(signal);
    // A cancellation is not a data error: propagate it so the caller can tell
    // an aborted read apart from a missing/malformed/oversized file.
    if (error?.name === 'AbortError') throw error;
    if (error?.code === 'ABORT_ERR') throw error;
    if (error?.code === 'INVALID_UTF8') {
      return {ok: false, status: 'invalid_utf8', size: bytesRead, detail: String(error?.message || error).slice(0, 240)};
    }
    return {
      ok: false,
      status: error instanceof SyntaxError
        ? 'invalid_json'
        : error?.code === 'BI_QUERY_IDENTITY_CHANGED' ? 'identity_changed' : 'read_error',
      detail: String(error?.message || error).slice(0, 240),
    };
  } finally {
    if (handle) {
      await closeQueryFileHandle(handle, {file, signal, admitted});
    }
  }
}
...
function maxBusinessDate(rows, field) {
  let latest = '';
...
function crossGenerationBusinessDateMatch(section, payload, coreMeta) {
  if (section !== 'linksData') return false;
  const expected = String(coreMeta?.dates?.linkDate || coreMeta?.dates?.factLinkDate || coreMeta?.dates?.martLinkDate || '').slice(0, 10);
  const actual = String(payload?.dates?.linkDate || '').slice(0, 10)
    || maxBusinessDate(payload?.storeLinks, 'link_date')
    || maxBusinessDate(payload?.links, 'link_date');
  return Boolean(expected && actual && expected === actual);
}

function validateProfitQueryArtifactMetadata(raw, payload, sectionGeneratedAt, coreGeneratedAt) {
  if (!isPlainObject(raw.artifactMeta)) return {ok: false, status: 'invalid_query_artifact'};
  const meta = raw.artifactMeta;
  const expectedKeys = ['kind', 'logicalSection', 'sourceArtifact', 'sourceGeneratedAt', 'retainedPaths', 'omittedPaths'];
  const actualKeys = Object.keys(meta);
  if (actualKeys.length !== expectedKeys.length || actualKeys.some(key => !expectedKeys.includes(key))) {
    return {ok: false, status: 'invalid_query_artifact'};
  }
  if (meta.kind !== PROFIT_QUERY_ARTIFACT_KIND
    || meta.logicalSection !== 'profit'
    || meta.sourceArtifact !== 'profit'
    || String(meta.sourceGeneratedAt || '') !== String(sectionGeneratedAt || '')
    || (coreGeneratedAt && String(meta.sourceGeneratedAt || '') !== coreGeneratedAt)
    || JSON.stringify(meta.retainedPaths) !== JSON.stringify(PROFIT_QUERY_RETAINED_PATHS)
    || JSON.stringify(meta.omittedPaths) !== JSON.stringify(PROFIT_QUERY_OMITTED_PATHS)) {
    return {ok: false, status: 'invalid_query_artifact'};
  }
  const profit = payload?.profit;
  if (!isPlainObject(profit)) return {ok: false, status: 'invalid_query_artifact'};
  if (!PROFIT_QUERY_RETAINED_PATHS.every(pathKey => {
    const field = pathKey.slice('profit.'.length);
    return Object.hasOwn(profit, field);
  })) return {ok: false, status: 'invalid_query_artifact'};
  if (PROFIT_QUERY_OMITTED_PATHS.some(pathKey => Object.hasOwn(profit, pathKey.slice('profit.'.length)))) {
    return {ok: false, status: 'invalid_query_artifact'};
  }
  return {ok: true, provenance: {
    sourceArtifact: 'profit',
    sourceGeneratedAt: String(meta.sourceGeneratedAt || ''),
    kind: meta.kind,
  }};
}
...
function validateSectionPayload(raw, section, coreMeta, options = {}) {
  ...
  const expectedEnvelopeSection = String(options.artifactSection || section);
  if (options.artifactKind === 'profit-query-compact' && !envelope) {
    return {ok: false, status: 'invalid_query_artifact'};
  }
  if (envelope && String(raw.section || '') !== expectedEnvelopeSection) return {ok: false, status: 'section_mismatch'};
  ...
  let crossGeneration = false;
  if (coreGeneratedAt && sectionGeneratedAt && sectionGeneratedAt !== coreGeneratedAt) {
    if (!crossGenerationBusinessDateMatch(section, payload, coreMeta)) {
      return {ok: false, status: 'stale_generation', generatedAt: sectionGeneratedAt, expectedGeneratedAt: coreGeneratedAt};
    }
    // Link/business data is daily while the core is regenerated with hourly
    // sales. Accept the prior section envelope only when its own link_date
    // exactly matches the core's declared link business date.
  ...
  if (options.artifactKind === 'profit-query-compact') {
    const compact = validateProfitQueryArtifactMetadata(raw, payload, sectionGeneratedAt, coreGeneratedAt);
    if (!compact.ok) return compact;
    artifactProvenance = compact.provenance;
  }
  ...
  return {
    ...
    envelope,
    crossGeneration,
    artifact: String(options.artifactName || section),
    artifactKind: String(options.artifactKind || 'portal'),
    provenance: artifactProvenance || {
      sourceArtifact: String(options.artifactName || section),
      sourceGeneratedAt: sectionGeneratedAt,
      kind: String(options.artifactKind || 'portal'),
    },
  };
}
...
export function classifyBiOpsQuery(question) {
  ...
  add('links', /链接|\bskc\b|曝光|访客|点击|转化|流量|上架|下架|补链|覆盖|折后价|活动价|促销价|link|traffic|visitor|impression|conversion/iu);
...
function mergeSectionData(target, source, section, sourceByPath, sourceLabel = section) {
  for (const [key, value] of Object.entries(source || {})) {
    ...
    if (key === 'rankings' && isPlainObject(value)) {
      ...
      sourceByPath.rankings = sourceLabel;
      for (const child of Object.keys(safeRankings)) sourceByPath[`rankings.${child}`] = sourceLabel;
      continue;
    }
...
async function loadOneSection({sectionsDir, section, artifactName = section, artifactKind = 'portal', artifactSection = section, coreMeta, maxSectionBytes, maxAggregateBytes, aggregateBytes, allowUnversionedSections, signal, parseGateBytes}) {
  const file = artifactKind === 'profit-query-compact'
    ? sectionArtifactFile(sectionsDir, artifactName)
    : sectionFile(sectionsDir, section);
  throwIfQueryAborted(signal);
  const bundleArtifact = artifactKind === 'profit-query-compact'
    ? PROFIT_QUERY_ARTIFACT
    : ['profit', 'homeProfit'].includes(String(section || '')) ? String(section) : '';
  const identityRoot = bundleArtifact ? path.dirname(path.resolve(sectionsDir)) : '';
  if (bundleArtifact) {
    const bundle = await readBiProfitBundleManifest(identityRoot, coreMeta.generatedAt);
    if (!bundle) {
      return {
        section,
        ...
        expectedGeneratedAt: String(coreMeta.generatedAt || ''),
        size: Number((await fs.stat(file).catch(() => null))?.size || 0),
      };
    }
  }
  if (artifactKind === 'profit-query-compact') {
    if (!String(coreMeta.generatedAt || '')) {
      return {
        section,
        artifact: artifactName,
        artifactKind,
    ...
    if (!exists) return {section, artifact: artifactName, artifactKind, status: 'missing', size: 0};
    const identityMatches = await biSectionArtifactIdentityMatches(identityRoot, artifactName, artifactSection, coreMeta.generatedAt);
    if (!identityMatches) {
      return {
        section,
        artifact: artifactName,
  ...
  // One open FileHandle carries the fstat budget checks, the cancellable read,
  // and the post-read UTF-8 re-check, so the size a section is judged against
  // is the size of the exact bytes that were actually read. A section that
  // fits its own file but exceeds the remaining aggregate budget is reported
  // as aggregate_too_large and is never parsed, so it cannot be mistaken for
  // available data or for a per-file too_large error.
  const read = await readJsonBounded({file, maxBytes: maxSectionBytes, maxAggregateBytes, aggregateBytes, signal, parseGateBytes});
  if (!read.ok) {
    return {
      section,
      artifact: artifactName,
      ...
      size: read.size || 0,
      detail: read.detail || '',
      ...(read.status === 'aggregate_too_large' ? {maxAggregateBytes: read.maxAggregateBytes, remaining: read.remaining} : {}),
      ...(read.status === 'too_large' ? {maxBytes: read.maxBytes} : {}),
    };
  }
  const validated = validateSectionPayload(read.value, section, coreMeta, {
    allowUnversionedSections,
    artifactName,
    artifactKind,
    artifactSection,
  });
  ...
  if (bundleArtifact && !await readBiProfitBundleManifest(identityRoot, coreMeta.generatedAt)) {
    return {
      section,
      artifact: artifactName,
      artifactKind,
  ...
  return {
    ...
    size: read.size || 0,
    data: validated.data,
    crossGeneration: validated.crossGeneration === true,
    provenance: validated.provenance,
    identityReceipt: read.identityReceipt,
  };
}

/**
 * Load the BI core plus only the section shards relevant to one question.
 * Missing, malformed, oversized, stale, or aggregate-budget-exceeding sections
 * are reported and skipped; the required core file remains a hard failure and
 * the aggregate object budget (core plus actually loaded sections) is a hard
 * cap that cannot be bypassed by many individually small files.
 */
export async function loadBiOpsQueryData(questionOrOptions, maybeOptions = {}) {
  ...
  const maxAggregateBytes = finitePositive(options.maxAggregateBytes, DEFAULT_BI_OPS_AGGREGATE_MAX_BYTES);
  const parseGateBytes = finitePositive(options.parseGateBytes, BI_SECTION_PARSE_GATE_BYTES);
  ...
  if (isPlainObject(options.coreData)) {
    ...
    const read = await readJsonBounded({file: dataPath, maxBytes: maxCoreBytes, maxAggregateBytes: Number.MAX_SAFE_INTEGER, aggregateBytes: 0, signal, parseGateBytes});
    ...
    identityReceipts.push(read.identityReceipt);
  }
  throwIfQueryAborted(signal);
  if (coreBytes > maxAggregateBytes) {
    throw new Error(`BI query aggregate budget exceeded: core is ${coreBytes} bytes but maxAggregateBytes is ${maxAggregateBytes}`);
  }
  ...
  const coreMeta = {generatedAt: coreGeneratedAt, mode: coreMode, dates: isPlainObject(core.dates) ? core.dates : {}};
  ...
  // Serial loads accumulate the object budget as core bytes plus the bytes of
  // every section that is actually loaded. This works because loads are strict
  // sequential awaits; the cap is what bounds peak resident memory even when
  // many individually small sections are requested.
  let aggregateBytes = coreBytes;

  const attempt = async (section, candidate = {}) => {
    throwIfQueryAborted(signal);
    const artifactName = String(candidate.artifactName || section);
    const artifactKind = String(candidate.artifactKind || 'portal');
    const artifactSection = String(candidate.artifactSection || section);
    const attemptKey = `${section}|${artifactName}`;
    if (attemptedArtifacts.has(attemptKey)) return null;
    attemptedArtifacts.add(attemptKey);
    ...
    const entry = {
      ...
      artifact: result.artifact || artifactName,
      artifactKind: result.artifactKind || artifactKind,
      status: result.status,
      generatedAt: result.generatedAt || '',
      expectedGeneratedAt: result.expectedGeneratedAt || '',
      size: result.size || 0,
      crossGeneration: result.crossGeneration === true,
    };
    if (result.provenance) entry.provenance = result.provenance;
    if (result.status === 'aggregate_too_large') {
      entry.maxAggregateBytes = result.maxAggregateBytes;
      entry.remaining = result.remaining;
    } else if (result.status === 'too_large') {
      entry.maxBytes = result.maxBytes;
    }
    attempts.push(entry);
    if (result.status === 'loaded') {
      aggregateBytes += result.size || 0;
      loaded.push({
        section: result.section,
        artifact: result.artifact || artifactName,
        artifactKind: result.artifactKind || artifactKind,
      ...
      loadedData.set(result.section, result.data);
      identityReceipts.push(result.identityReceipt);
    }
    return result;
  };
  ...
  for (const group of plan.groups || []) {
    for (const section of group.candidates || []) {
      throwIfQueryAborted(signal);
      const candidates = section === 'profit'
        ? [
          {artifactName: PROFIT_QUERY_ARTIFACT, artifactKind: 'profit-query-compact', artifactSection: PROFIT_QUERY_ARTIFACT},
          {artifactName: 'profit', artifactKind: 'portal', artifactSection: 'profit'},
        ]
        : [{artifactName: section, artifactKind: 'portal', artifactSection: section}];
      let loadedCandidate = false;
      for (const candidate of candidates) {
        const result = await attempt(section, candidate);
        if (result?.status === 'loaded') {
          loadedCandidate = true;
          break;
        }
      }
      if (loadedCandidate) break;
    }
  }
  ...
  for (const result of orderedLoaded) {
    throwIfQueryAborted(signal);
    const sectionData = loadedData.get(result.section);
    if (sectionData === undefined) continue;
    const sourceLabel = result.artifactKind === 'profit-query-compact'
      ? `${result.artifact}:compact`
      : result.section;
    mergeSectionData(data, sectionData, result.section, sourceByPath, sourceLabel);
    // Drop the parsed section reference as soon as it is merged so peak
    // retention stays bounded by the aggregate budget instead of growing with
    // the whole loaded set.
    loadedData.delete(result.section);
  }
  throwIfQueryAborted(signal);
  synthesizeDailySalesSummary(data, sourceByPath);

  if (_fileHandleLifecycleHooks?.onBeforeQueryFinalReadback) {
    await _fileHandleLifecycleHooks.onBeforeQueryFinalReadback({
      files: identityReceipts.filter(Boolean).map(receipt => receipt.file),
      signal,
    });
  }
  throwIfQueryAborted(signal);
  for (const receipt of identityReceipts) {
    throwIfQueryAborted(signal);
    await verifyQueryIdentityReceipt(receipt);
  }
  ...
  return {
    ...
    meta: {
      ...
      loadedArtifacts: orderedLoaded.map(result => ({
        section: result.section,
      ...
      sectionProvenance: Object.fromEntries(orderedLoaded.map(result => [
        result.section,
        result.provenance || {sourceArtifact: result.artifact, kind: result.artifactKind},
      ])),
      aggregateBytes,
      maxSectionBytes,
      maxAggregateBytes,
      crossGenerationSections: orderedLoaded.filter(result => result.crossGeneration).map(result => result.section),
      sourceByPath,
    },
  };
}
...
function availabilityFromMeta(meta) {
  ...
  return {
    ...
    crossGenerationSections: [...(meta.crossGenerationSections || [])],
    sectionIssues: issues,
    sources,
    priorityRule: 'matching-generation or exact business-date-compatible section > core; rankings > homeRankings',
  };
}
