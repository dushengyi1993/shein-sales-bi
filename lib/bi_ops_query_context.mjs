...
async function loadOneSection({sectionsDir, section, coreMeta, maxSectionBytes, maxAggregateBytes, aggregateBytes, allowUnversionedSections, signal}) {
...
export async function loadBiOpsQueryData(questionOrOptions, maybeOptions = {}) {
  ...
  const attempt = async section => {
    ...
    if (result.status === 'loaded') {
      ...
      loaded.push({section: result.section, generatedAt: result.generatedAt || '', size: result.size || 0, crossGeneration: result.crossGeneration === true});
  ...
  for (const result of orderedLoaded) {
    ...
    mergeSectionData(data, sectionData, result.section, sourceByPath);
