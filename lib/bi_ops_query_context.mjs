...
// query surface isolation test uses it to capture the exact FileHandle that
// readJsonBounded opened and to prove that a cancellation landing immediately
// after fs.open still reaches the finally close. The seam can only observe and
// abort; it never alters budget, read, or parse semantics.
...
// The whole read lifecycle lives on one FileHandle: fs.open binds a file
// identity once, then handle.stat() and handle.readFile() both target that same
// open file. A path replaced between an old stat() and a later readFile() can
// therefore never swap in a different or larger file behind the budget checks.
//
// Budgets are enforced twice: first against the fstat on-disk size (per-file,
// then the aggregate remaining), and again against the UTF-8 byte length
// actually read, so a file that grows after fstat (or a decode that expands)
// still fails closed and the reported size is what really entered the heap.
// handle.readFile() accepts a native AbortSignal and stops between OS reads.
async function readJsonBounded({file, maxBytes, maxAggregateBytes, aggregateBytes, signal}) {
  ...
  try {
    if (_fileHandleLifecycleHooks?.onOpened) {
      _fileHandleLifecycleHooks.onOpened({handle, file, signal});
    }
    throwIfQueryAborted(signal);
    ...
    const raw = await handle.readFile({encoding: 'utf8', ...(signal ? {signal} : {})});
    throwIfQueryAborted(signal);
...
async function loadOneSection({sectionsDir, section, artifactName = section, artifactKind = 'portal', artifactSection = section, coreMeta, maxSectionBytes, maxAggregateBytes, aggregateBytes, allowUnversionedSections, signal}) {
  ...
  const read = await readJsonBounded({file, maxBytes: maxSectionBytes, maxAggregateBytes, aggregateBytes, signal});
  if (!read.ok) {
    return {
      section,
      artifact: artifactName,
...
export async function loadBiOpsQueryData(questionOrOptions, maybeOptions = {}) {
  ...
  if (isPlainObject(options.coreData)) {
    ...
    const read = await readJsonBounded({file: dataPath, maxBytes: maxCoreBytes, maxAggregateBytes: Number.MAX_SAFE_INTEGER, aggregateBytes: 0, signal});
