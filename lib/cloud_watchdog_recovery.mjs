...
export const DAILY_PROFIT_MART_WARNING = 'profit mart refresh failed';
...
const DAILY_MARKETING_GUARD_TERMINAL_FAILURES = new Set([
  'blocked_low_memory',
  'failed',
]);
...
/**
 * A daily wrapper can retain a historical profit-mart warning after the
 * section queue has successfully rebuilt the formal profit cache. Suppress
 * that stale warning only with a newer, valid profit section artifact.
 */
export function assessDailyProfitSectionRecovery({dailyRefresh, profitSection, nowMs = Date.now()} = {}) {
  const message = String(dailyRefresh?.message || '').trim();
  if (!message.startsWith(DAILY_PROFIT_MART_WARNING)) return failed('daily_warning_not_profit_mart');
  const warningAt = instant(dailyRefresh?.generatedAt);
  if (!Number.isFinite(warningAt)) return failed('profit_warning_time_invalid');
  if (!profitSection || profitSection.error) return failed('profit_section_unavailable');
  if (profitSection.ok !== true || profitSection.section !== 'profit') return failed('profit_section_not_complete');
  const cachedAt = instant(profitSection.cachedAt);
  if (!Number.isFinite(cachedAt)) return failed('profit_section_cache_time_invalid');
  if (cachedAt <= warningAt) return failed('profit_section_not_newer_than_warning');
  if (cachedAt > nowMs + 5 * 60_000) return failed('profit_section_cache_from_future');
  return {
    recovered: true,
    reason: 'newer_profit_section_cache_recovery',
    evidence: {
      type: 'daily_profit_section_recovery',
      dailyDate: dailyRefresh?.date || null,
      warningGeneratedAt: dailyRefresh.generatedAt,
      profitCachedAt: profitSection.cachedAt,
    },
  };
}
...
export function assessDailyMarketingGuardHealth({
  ...
  const hasTodayOk = lastOkState
    && !lastOkState.error
    && lastOkState.status === 'ok'
    && lastOkState.date === clock.date
    && Number.isFinite(instant(lastOkState.generatedAt));
  const hasTodayTerminalFailure = guardState
    && !guardState.error
    && guardState.date === clock.date
    && DAILY_MARKETING_GUARD_TERMINAL_FAILURES.has(String(guardState.status || ''))
    && !hasTodayOk;
  if (hasTodayTerminalFailure) {
    return {healthy: false, pending: false, reason: 'today_terminal_failure', ...base};
  }
...
export function assessDailyMarketingRepairHealth({
  ...
  if (
    queueState?.date === clock.date
    && queueState?.status === 'pending'
    && repairState?.date === clock.date
    && repairState?.status === 'deferred_to_local'
  ) {
    return {healthy: true, pending: true, reason: 'today_repair_queue_deferred_to_local', ...base};
  }
  if (clock.minuteOfDay < finalDeadlineMinute) {
...
function parseSystemdDate(value) {
...
function laterThanServiceExit(completedAt, status) {
  const completed = parseSystemdDate(completedAt)?.getTime();
  const exited = parseSystemdDate(status?.ExecMainExitTimestamp || status?.StateChangeTimestamp)?.getTime();
  return Number.isFinite(completed) && (!Number.isFinite(exited) || completed >= exited);
}

export function assessBusinessRecovery(unit, status, {morningMarker, morningMarkerEvidenceOk = false, orderRecheckState, nowMs = Date.now()} = {}) {
  ...
  if (morningUnits.has(unit)
    && morningMarker?.runDate === shanghaiClock(nowMs).date
    && morningMarker?.businessDate === shanghaiClock(nowMs - 86400000).date
