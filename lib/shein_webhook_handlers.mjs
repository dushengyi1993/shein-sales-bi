...
const COMPLIANCE_TYPE_LABELS = Object.freeze({
  // Official Webhook 3001104: 1=EU responsible person, 3=real product photos.
  '1': '欧盟责任人',
  '3': '商品实拍图',
});
...
function warehouseSyncContext(result = {}) {
  const artifact = object(result.artifact);
  const kind = text(result.kind);
  const businessDate = text(artifact.start || artifact.end).slice(0, 10);
  if (kind === 'order') {
    const order = Array.isArray(artifact.orderRows) ? object(artifact.orderRows[0]) : {};
    const goodsRows = Array.isArray(artifact.goodsRows) ? artifact.goodsRows : [];
    const validRows = goodsRows.filter(row => (
      row?.isValidSale !== false
      && text(row?.isValidSale).toLowerCase() !== 'false'
      && !text(row?.salesExclusionReason)
      && (number(row?.number) || 0) > 0
      && (number(row?.currencyPrice) || 0) > 0
    ));
    const orderStatus = text(order.orderStatus);
    const orderStatusDesc = text(order.orderStatusDesc || order.performStatusDesc);
    const cancelledBeforePickup = goodsRows.some(row => (
      text(row?.salesExclusionReason) === 'cancelled_before_pickup'
      || (
        text(row?.pageStatus).toUpperCase().includes('CANCEL')
        && text(row?.goodsPerformanceStatus) === '6'
      )
    ));
    return {
      kind,
      ...
      orderStatus,
      orderStatusDesc,
      salesQuantity: validRows.reduce((sum, row) => sum + (number(row?.number) || 0), 0),
      salesSar: round2(validRows.reduce((sum, row) => sum + (number(row?.currencyPrice) || 0), 0)),
      cancelledBeforePickup,
    };
  }
  if (kind === 'return') return {kind, businessDate};
  return {};
}
...
function salesSummaryLine(context = {}) {
  const sales = object(context.sales);
  ...
  const unitsAll = count(sales.unitsLifetime);
  const amountAll = decimal(sales.grossSalesLifetimeSar);
  if (!units7 && !units30 && !unitsAll) return '销售：暂无有效成交';
  const parts = [
    units7 ? `近7天 ${units7} 件${amount7 ? ` / ${amount7} SAR` : ''}` : '',
    units30 ? `近30天 ${units30} 件${amount30 ? ` / ${amount30} SAR` : ''}` : '',
    unitsAll ? `累计 ${unitsAll} 件${amountAll ? ` / ${amountAll} SAR` : ''}` : '',
  ...
  const lastSale = cnDateTime(sales.lastSaleDate) || text(sales.lastSaleDate);
  return `销售：${parts.join('；')}${lastSale ? `；最近成交 ${lastSale.slice(0, 10)}` : ''}`;
}

function complianceProductCopy(normalized, urgent) {
  const storeLabel = `${text(normalized.storeKey) || '未知'} 店`;
  const context = object(normalized.productContext);
  const identity = productIdentityLines(normalized);
  const skc = text(normalized.skc || normalized.businessId);
  const typeLabel = COMPLIANCE_TYPE_LABELS[text(normalized.complianceTypeId)] || '必需合规资料';
  const titleTypeLabel = typeLabel.replace(/^商品/u, '');
  const productMissing = normalized.productContextStatus === 'not_found'
    && normalized.productIdentityStatus === 'not_found';
  const lines = productMissing ? [] : [...identity.lines];
  if ((productMissing || !identity.lines.length) && skc) lines.push(`商品编号：${skc}`);
  lines.push(`缺失资料：${typeLabel}`);
  const rawStatus = text(context.lastKnownShelfStatus || context.currentShelfStatus);
  const status = ({'0': '未上架', '1': '已上架'}[rawStatus] || rawStatus);
  if (status) lines.push(`当前状态：${status}`);
  const salesLine = salesSummaryLine(context);
  if (salesLine) lines.push(salesLine);
  const at = cnDateTime(normalized.eventTime);
  if (at) lines.push(`失效时间：${at}`);
  if (productMissing) {
    lines.push('当前核对：BI 和 SHEIN 当前商品列表都没有找到这条商品，通常是历史或已停用商品。');
    lines.push('下一步：无需按在售商品处理；只有在 SHEIN 后台仍能找到时，再补充对应资料。');
  } else if (urgent) {
    lines.push(`下一步：请到 SHEIN 后台为该商品补充${typeLabel}。`);
  } else {
    lines.push('系统已记录；当前不需要紧急处理。');
  }
  const title = productMissing
    ? `${storeLabel}：历史商品：${titleTypeLabel}失效`
    : `${storeLabel}：${identity.label}：${titleTypeLabel}需要补充`;
  return {title, summary: lines.join('\n')};
}
...
export function humanizeSheinWebhookEvent(normalized = {}, severity = {}) {
  ...
  switch (family) {
    ...
    case 'quota':
      ...
      break;
    case 'compliance':
      ({title, summary} = complianceProductCopy(normalized, urgent));
      break;
    case 'price_abnormal':
...
async function enrichProductContext(receipt, webhookRepository, productAuditContextProvider) {
  ...
  const needsProductIdentity = PRODUCT_CONTEXT_FAMILIES.has(normalized.eventFamily)
  ...
  if (needsProductIdentity && productAuditContextProvider?.getProductIdentity && storeKey && skc) {
    try {
      ...
      if (productIdentity && typeof productIdentity === 'object') {
        ...
        enriched = {...enriched, productIdentityStatus: 'not_found'};
      }
    } catch {
      enriched = {...enriched, productIdentityStatus: 'unavailable'};
  ...
  return Object.freeze({
    ...
    async process(receipt) {
      ...
      if (normalized.appScopedOnly === true) {
        return {
          title: normalized.deliveryScope === 'backup_app_status' ? `${text(normalized.storeKey)} 备用应用状态` : `${text(normalized.storeKey) || '未知店铺'} Webhook 应用级验证`,
          summary: normalized.deliveryScope === 'backup_app_status' ? '备用应用的授权或额度变化已记录，未改变主用应用及店铺执行状态。' : '签名与接收链路验证通过；未携带已授权店铺 OpenKey，未执行店铺业务动作。',
          businessKey: '',
      ...
      if (normalized.eventFamily === 'order') {
        ...
        return {
          ...base,
          normalized: {...normalized, warehouseSync: warehouseSyncContext(result)},
          actionState: 'order_warehouse_synced',
          result,
        };
      }
      if (normalized.eventFamily === 'return') {
        ...
        return {
          ...base,
          normalized: {...normalized, warehouseSync: warehouseSyncContext(result)},
          actionState: 'return_warehouse_synced',
          result,
        };
      }
