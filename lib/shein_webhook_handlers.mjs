...
function auditProductCopy(normalized, urgent) {
  ...
  const supplierCode = text(context.supplierCode);
  const skc = text(normalized.skc || context.skc || normalized.businessId);
  if (!urgent) {
    return {
...
async function enrichProductContext(receipt, webhookRepository, productAuditContextProvider) {
  ...
  if (['product_audit', 'product_audit_all_channels'].includes(normalized.eventFamily)) {
    if (!productAuditContextProvider?.getAuditContext) return normalized;
    ...
    try {
      ...
      // The callback's own failure reason is still authoritative and alertable.
      // A read-only negotiation lookup must never delay retries or suppress P0.
      return {...normalized, auditContextStatus: 'unavailable'};
    }
  }
  if (normalized.eventFamily !== 'product_shelves') return normalized;
...
export function createSheinWebhookEventProcessor({webhookRepository, linkOpsRepository = null, orderReturnSync = null, productAuditContextProvider = null} = {}) {
  ...
  return Object.freeze({
    async process(receipt) {
      ...
      normalized = await enrichProductContext({...receipt, normalized}, webhookRepository, productAuditContextProvider);
