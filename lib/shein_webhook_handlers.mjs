...
const PRODUCT_CONTEXT_FAMILIES = new Set([...PRODUCT_FAMILIES, 'rrp_review', 'rrp_validity', 'compliance']);
...
async function applyProductState({receipt, webhookRepository}) {
  const normalized = receipt?.normalized || {};
  const family = text(normalized.eventFamily);
  const action = text(normalized.action);
  ...
  const supported = (family === 'product_shelves' && ['on_shelf', 'off_shelf'].includes(action))
    || (family === 'product_delete_audit' && text(normalized.status) === '2');
  if (!supported || !skc || !order || !webhookRepository?.applyProductState) return null;
  ...
  const applied = await webhookRepository.applyProductState({
    ...
    productContext: object(normalized.productContext),
  });
  return {
  ...
  return Object.freeze({
    ...
    async process(receipt) {
      ...
      if (PRODUCT_FAMILIES.has(normalized.eventFamily)) {
        ...
        const task = await reconcileProductTask({receipt: workingReceipt, linkOpsRepository});
        return {
          ...base,
          ...task,
          ...(productState ? {
