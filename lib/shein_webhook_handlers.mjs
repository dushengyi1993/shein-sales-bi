...
async function applyRiskGate({receipt, webhookRepository}) {
  ...
  if (family === 'authorization') {
    const gate = await webhookRepository.upsertStoreGate({storeKey: normalized.storeKey, gateType: 'authorization', state: 'blocked', reason: 'SHEIN authorization relationship changed', sourceReceiptId: receipt.id});
  ...
  if (family === 'quota' && Number.isFinite(normalized.quota)) {
    ...
    const gate = await webhookRepository.upsertStoreGate({storeKey: normalized.storeKey, gateType: 'quota', state: blocked ? 'blocked' : 'open', reason: blocked ? 'SHEIN product quota is zero' : 'SHEIN product quota restored', sourceReceiptId: receipt.id});
