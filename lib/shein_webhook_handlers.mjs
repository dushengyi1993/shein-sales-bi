...
function shelfProductCopy(normalized, urgent) {
  ...
  lines.push(
    `下架时间：${cnDateTime(eventAt) || eventAt || '平台推送未提供'}`,
    `下架人：${operator || '平台推送未提供'}`,
    `下架原因：${reason || '平台推送未提供'}`,
  );
  if (text(change.recycleState) === '1') lines.push('当前状态：商品已进入回收站');
  const related = number(normalized.relatedEventCount);
  if (related !== null && related > 1) lines.push(`合并说明：平台同一批推送了 ${Math.round(related)} 条站点变化，BI 已合并为一件事。`);
  lines.push(operator && reason
    ? '下一步：请根据上述原因决定恢复上架或保持下架。'
    : '下一步：请到 SHEIN 后台的商品操作记录核对下架人和原因，再决定恢复上架或保持下架。');
