...
export async function verifyLegacyLowEtReceiptContinuation({
  ...
  if (receipt.status !== 'consumed' || receipt.gracefulRemainingSec < 900) {
  ...
  const resultFile = path.join(root, 'outputs/reports', `new-listing-7d-limited-discount-execution-summary-${date}.json`);
  try {
    const result = await readJson(resultFile);
    if (result.workFingerprint === expectedWorkFingerprint && (result.results?.length || result.newGroups)) {
      throw new Error('legacy receipt has existing execution or unknown result evidence');
    }
  } catch (error) { if (error.code !== 'ENOENT') throw error; }
  const journalDir = path.join(root, 'state/marketing-replacement-transactions');
  // A fenced/subset operation may have a different rescue hash. Do not infer
  // no-write merely from absence of the primary deterministic journal name.
  let journals = [];
  try { journals = await fs.readdir(journalDir); }
  catch (error) { if (error.code !== 'ENOENT') throw error; }
  for (const name of journals.filter(name => name.endsWith('.json'))) {
    const journal = await readJson(path.join(journalDir, name));
    if (journal.runPayloadHash === expectedWorkFingerprint
      || journal.createAttempt?.workFingerprint === expectedWorkFingerprint) {
      throw new Error('legacy receipt has persisted operation evidence');
    }
  }
  ...
  const targetSkcs = new Set(rows.map(row => String(row.skc || '')));
  for (const directory of inventoryDirectories) {
    ...
    let lines;
    try { lines = (await fs.readFile(file, 'utf8')).trim().split('\n').filter(Boolean); }
    ...
    for (const line of lines) {
      ...
      while (objects.length) {
        ...
        if (targetSkcs.has(String(value.skc || ''))) throw new Error('legacy receipt has inventory intent or unknown evidence');
  ...
  legacyReceiptCapabilities.set(capability, {
    rescues: new Set(exactPlan.entries.map(entry => evidenceHash(entry.rescue))),
...
export function buildLowEtFastSellerPricingContext({
  ...
  const policy = mergePolicy(marketingPolicy?.lowEtFastSellerPricePullback || {});
  ...
  const exposureIndex = buildExposureTopLinkIndex({storeLinks: linkRows}, marketingPolicy);
  ...
  return {
    ...
    policyEvidenceHash: evidenceHash({...marketingPolicy, lowEtFastSellerPricePullback: policy}),
    byCanonical,
    baselineByLink,
    exposureIndex,
    blockers,
