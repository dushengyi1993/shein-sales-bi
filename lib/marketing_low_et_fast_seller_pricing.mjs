...
export async function verifyLegacyLowEtReceiptContinuation({
  ...
  for (const group of receipt.groups.filter(group => group.mode === 'transaction')) {
    ...
    const fenced = await readJson(path.resolve(root, fencedPath));
    if (!Array.isArray(fenced.rows) || !fenced.rows.length
      || fenced.rows.some(row => !(original.rows || []).some(locked => evidenceHash(locked) === evidenceHash(row)))
      || Object.keys(original).some(key => key !== 'rows' && evidenceHash(original[key]) !== evidenceHash(fenced[key]))) {
