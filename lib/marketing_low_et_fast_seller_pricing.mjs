...
export function buildLowEtFastSellerPricingContext({
  ...
  for (const evidence of byCanonical.values()) {
    ...
    for (const topRow of evidence.topRows || []) {
      const exact = baselineByLink.get(linkKey(topRow.storeKey, topRow.skc));
      if (exact?.ordinaryApprovedPrice > 0) continue;
      blockers.push({
        canonical: evidence.canonical,
        canonicalKey: evidence.canonicalKey,
        storeKey: topRow.storeKey,
        skc: topRow.skc,
        reason: 'top5_missing_exact_link_ordinary_approved_price',
  ...
  return {
    ...
    evidenceHash: evidenceHash({
      ...
      links: [...baselineByLink.entries()].map(([key, value]) => ({
        key,
        ordinaryApprovedPrice: value.ordinaryApprovedPrice,
        ordinaryTargetMargin: value.ordinaryTargetMargin,
...
export function applyLowEtFastSellerPricePullback({
  ...
  let mode;
  if (isTop5) {
    calculatedPrice = positiveNumber(linkBaseline?.ordinaryApprovedPrice);
    ...
    mode = 'top5_restore_latest_approved_ordinary_link_price';
  } else {
  ...
  const audit = {
    ...
    exactLinkOrdinaryApprovedPrice: linkBaseline?.ordinaryApprovedPrice ?? calculatedPrice,
    exactLinkBaselineHash: evidenceHash(linkBaseline || null),
    ...
    contextEvidenceHash: context.evidenceHash,
  };
  return {
    row: {
      ...source,
...
export function revalidateLowEtFastSellerPricePullback({
  ...
  if (planned?.contextEvidenceHash && planned.contextEvidenceHash !== context?.evidenceHash) {
    return {ok: false, reason: 'low_et_price_pullback_context_evidence_drift', current, planned};
  }
  if (planned?.exactLinkBaselineHash && planned.exactLinkBaselineHash !== current.audit?.exactLinkBaselineHash) {
    return {ok: false, reason: 'low_et_price_pullback_exact_link_baseline_drift', current, planned};
  }
...
export async function revalidateLowEtFastSellerRescueArtifact({
  ...
  const results = ordinaryRows.map(row => ({
    ...
    skc: row.skc,
    ...revalidateLowEtFastSellerPricePullback({row, context, costDoc}),
  }));
  return {
