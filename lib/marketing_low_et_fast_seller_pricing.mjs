...
export const DEFAULT_LOW_ET_FAST_SELLER_POLICY = Object.freeze({
  ...
  criteria: {
    ...
    canonicalGoodsValidSales30dAcrossEnabledStoresMinExclusive: 30,
  },
  pricingAction: {
...
export function buildLowEtFastSellerPricingContext({
  ...
  marketingPolicy = {},
  reportDate = '',
  reviewedWorkbookPriceCapability = null,
} = {}) {
  ...
  const minSalesExclusive = Number(policy.criteria.canonicalGoodsValidSales30dAcrossEnabledStoresMinExclusive
    ?? policy.criteria.canonicalGoodsValidSales30dAcross19StoresMinExclusive);
  ...
  return {
    schemaVersion: 1,
    fixedTierContext: buildFixedTierContext({storeLinks:linkRows,classificationCoverageComplete:linksDataDoc?.classificationCoverageComplete}, {reportDate,costDoc,baselineDoc,reviewedWorkbookPriceCapability}),
    reportDate,
    policy,
...
export function applyLowEtFastSellerPricePullback({
  ...
  const source = row || {};
  if (source.reviewedWorkbookPrice && (manualSpecial || source.manualSpecialLimitedDiscount === true)) {
    return {row:{...source},applied:false,blocked:true,reason:'active_manual_special_requires_user_review'};
  }
  if (source.reviewedWorkbookPrice || (!manualSpecial && source.manualSpecialLimitedDiscount !== true)) {
    const fixed = applyFixedTierPrice(source, context?.fixedTierContext);
    if (fixed) return fixed;
  }
  if (context?.fixedTierOnly === true) return {row:{...source},applied:false,blocked:false,reason:'legacy_low_et_inputs_unavailable'};
...
export function revalidateLowEtFastSellerPricePullback({
  ...
  if (row?.reviewedWorkbookPrice || row?.manualSpecialLimitedDiscount !== true) {
...
export async function revalidateLowEtFastSellerRescueArtifact({
  ...
  if (sources.rawLinkHistory) {
    ...
    if (!coverage.complete && reportDate<'2026-09-10') {
      return {
        ok: false,
    ...
    currentLinksDataDoc = {storeLinks: merged.rows,classificationCoverageComplete:coverage.complete};
    rawLinkOverlay = {
      ...coverage,
