...
export const SYSTEMD_SNAPSHOT_PROPERTIES = Object.freeze([
  'Id', 'LoadState', 'ActiveState', 'SubState', 'Result', 'ExecMainCode', 'ExecMainStatus',
  'StateChangeTimestamp', 'ActiveEnterTimestamp', 'ExecMainStartTimestamp', 'ExecMainExitTimestamp',
]);
...
export function parseSystemdShowMany(stdout, expectedNames = [], command = {}) {
  ...
  for (const block of blocks) {
    ...
    if (!name) continue;
    const missingProperties = SYSTEMD_SNAPSHOT_PROPERTIES.filter(property => !Object.hasOwn(data, property));
    units[name] = {
      name,
  ...
  for (const name of expected) {
    ...
    units[name] = {
      ...
      ok: false,
      complete: false,
      missingProperties: [...SYSTEMD_SNAPSHOT_PROPERTIES],
