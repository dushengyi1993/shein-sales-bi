...
const BACKUP_SEGMENT_RE = /备用|backup|bak/i;
...
function classifyImage(name, dimensions = {}) {
  ...
  const isMainCover = !isAbTestCover && /主封面|主图|main\s*cover|main[-_\s]*image/i.test(cn);
...
function orderDetailCandidates(items) {
  ...
  const others = items.filter(item => item.classification.category === 'other')
    .sort((a, b) => a.sequence - b.sequence);
  const frontScenes = scenes.length >= 3 ? scenes.slice(0, 2) : scenes;
  const closingScenes = scenes.length >= 3 ? scenes.slice(2) : [];
  return [...frontScenes, ...selling, ...params, ...others, ...closingScenes];
}
...
export async function planLinkOpsImageRoles({dir, cwd = process.cwd()} = {}) {
  ...
  const warnings = [];
  const blockers = [];
  const ignoredAbTestCovers = all.filter(item => item.classification.isAbTestCover);
  const eligible = all.filter(item => !item.classification.isAbTestCover);
  ...
  return {
    ...
    sourceDir: rootDir,
    rules: {
      ignoredByPath: '路径任一层包含“备用/backup/bak”的图片不使用。',
      ignoredByName: '文件名包含“产品封面/AB测试”的图片只做 AB 测试素材，不提交。',
      mainCover: '前端“细节图11”的第一张才是主图，优先使用主封面。',
      carouselSecondCover: '单独轮播图不是主图，使用主封面之外最适合作第二封面的图片。',
      detailOrder: '其他细节图按场景→卖点→参数排序；场景很多时可留到最后收尾。',
    ...
    candidates: all.map(item => roleItem(item, item.classification.isAbTestCover ? 'ignored_ab_test_cover' : item.classification.category, item.classification.isAbTestCover ? 'ignored' : '')),
    blockers,
    warnings,
  };
}
