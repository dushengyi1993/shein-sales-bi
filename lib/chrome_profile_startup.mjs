...
import {resolvePersistentStoreProfile, resolveSheinPrimaryWorkspace} from './shein_workspace_paths.mjs';
...
export async function inspectManagedStoreSession(store, expected = null, {
  ...
  const workspaceRoot = path.resolve(root) === ROOT ? resolveSheinPrimaryWorkspace({env}) : root;
  const profileDir = resolvePersistentStoreProfile(store, {requireExistingRoot: true, primaryWorkspace: workspaceRoot});
  const refuseLaunch = () => { throw new Error('Managed session unavailable; relaunch forbidden'); };
  await withChromeProfileStartup({root: workspaceRoot, profileDir, storeKey: store.storeKey, port: store.port, env,
