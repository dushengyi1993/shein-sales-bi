...
async function acquireFileLock(file, {timeoutMs, staleMs}) {
  ...
  while (true) {
    try {
      ...
      await handle.writeFile(JSON.stringify({pid: process.pid, createdAt: nowIso(), target: file}));
      return async () => {
...
export function createLinkOpsJsonRepository({
  ...
  async function createSession(session, {idempotencyKey = '', ownerUser = '', actorUser = '', sortOrder = 0} = {}) {
    ...
    const inputMessages = Array.isArray(source.messages) ? source.messages.map(cleanRecord) : [];
    source.messages = inputMessages.map(message => storedRecord(message, 1));
    ...
    const order = integer(sortOrder, {fallback: 0, min: 0, label: 'sortOrder'});
    const requestHash = linkOpsPayloadHash({operation: 'create_session', session: baseForHash, messages: inputMessages, sortOrder: order});
    return mutate('session', store => {
      ...
      for (let index = 0; index < inputMessages.length; index += 1) {
        const messageId = requiredText(inputMessages[index].id || inputMessages[index].messageId, `session.messages[${index}].id`);
        appendLocalEvent(store, {
  ...
  async function appendMessage(chatSessionId, message, {idempotencyKey = '', ownerUser = '', actorUser = '', sequenceNo = null} = {}) {
    ...
    return mutate('session', store => {
      ...
      current.messages.splice(Math.min(order, current.messages.length), 0, inserted);
  ...
  async function flushLegacyActionState() {
    ...
    await writeJsonAtomic(files.action, mirror);
    return mirror;
  }
