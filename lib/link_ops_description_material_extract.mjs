 ...
 * nearest preceding heading / adjacent label (label-only text runs), from an
 * explicit three-language order statement (direction), or, only when the
 * section carries NO explicit language labels at all, from the unique
 * combination of the enclosing `<pre dir>` direction plus the Unicode script
 * of the block text: dir=rtl+Arabic script is 阿文, dir=ltr+Han script is
 * 中文, dir=ltr+Latin-only (no Arabic/Han) is 英文. Every block must carry
 * its own pre dir and classify uniquely; exactly one en/ar/zh-cn is required.
 * Missing direction, non-unique classification, duplicates or mixed scripts
 * reject the whole HTML. Any ambiguity (duplicate/conflicting/missing
 * labels, extra codes, conflicting order statements) also rejects.
 *
...
export function extractTrilingualCoreSellingPoints(htmlText) {
  ...
  return extractCodeAndDisplayboxSection(section);
}

function extractCodeAndDisplayboxSection(section) {
  if (![...section.matchAll(CODE_RE)].length) return extractLabelledPreSection(section);
  ...
  if (zhCode.length > 1) throw extractError('三语卖点存在重复中文 code');
...
// Reviewed V4 uses plain <pre> copy boxes. Each box must have its own
// preceding language heading, and explicit sp-* IDs must agree with it.
function extractLabelledPreSection(section) {
  const pres = [...section.matchAll(/<pre\b([^>]*)>([\s\S]*?)<\/pre\s*>/gi)];
  if (pres.length !== 3 || /<code\b/i.test(section)) throw extractError('三语复制框必须恰好3个独立 pre');
  const result = {};
  let previousEnd = 0;
  for (const pre of pres) {
    const headings = [...section.slice(previousEnd, pre.index).matchAll(HEADING_RE)];
    const label = decodeEntities(stripTagsAndNormalizeBreaks(headings.at(-1)?.[2] || ''));
    const languages = [['en', EN_LABEL_RE], ['ar', AR_LABEL_RE], ['zh-cn', ZH_LABEL_RE]]
      .filter(([, re]) => re.test(label)).map(([language]) => language);
    if (languages.length !== 1 || result[languages[0]]) throw extractError('pre复制框语言标题缺失、冲突或重复');
    const language = languages[0];
    const id = /\bid\s*=\s*["']([^"']+)["']/i.exec(pre[1])?.[1];
    if (id && id !== ({en: 'sp-en', ar: 'sp-ar', 'zh-cn': 'sp-zh'})[language]) throw extractError('pre复制框ID与语言标题不一致');
    result[language] = {lines: requireFiveLines(extractLines(pre[2]), language, 'pre复制框')};
    previousEnd = pre.index + pre[0].length;
  }
  return result;
}

function extractNumberedHeadingSection(htmlText) {
  const html = String(htmlText || '');
  const isCoreHeading = text => decodeEntities(stripTagsAndNormalizeBreaks(text)).trim() === '9. 三语核心卖点';
  const headings = [...html.matchAll(/<h2\b[^>]*>([\s\S]*?)<\/h2\s*>/gi)].filter(m => isCoreHeading(m[1]));
  const sections = [...html.matchAll(/<section\b([^>]*)>([\s\S]*?)<\/section\s*>/gi)]
    .filter(m => [...m[2].matchAll(/<h2\b[^>]*>([\s\S]*?)<\/h2\s*>/gi)].some(h => isCoreHeading(h[1])));
  if (headings.length !== 1 || sections.length !== 1 || /\bid\s*=/i.test(sections[0][1]) || /<section\b/i.test(sections[0][2])) {
    throw extractError('HTML 中没有唯一的无ID第9节三语核心卖点，拒绝猜测章节');
  }
  return extractLabelledPreSection(sections[0][2]);
}
...
function legacyLabelFromAdjacentNote(text) {
  const source = String(text || '');
  const notes = [...source.matchAll(/<div\b[^>]*\bclass\s*=\s*["'][^"']*\bnote\b[^"']*["'][^>]*>([\s\S]*?)<\/div\s*>/gi)]
    .filter(match => !/<code\b/i.test(source.slice(match.index + match[0].length)));
  if (!notes.length) return {language: '', reject: ''};
  const nearest = notes[notes.length - 1];
  const trailingText = decodeEntities(stripTagsAndNormalizeBreaks(source.slice(nearest.index + nearest[0].length)))
    .replace(/一键复制/g, '')
    .replace(/\s+/g, '')
    .trim();
  if (trailingText) return {language: '', reject: ''};
  const normalized = decodeEntities(stripTagsAndNormalizeBreaks(nearest[1]))
    .replace(/\s+/g, ' ')
    .trim();
  if (!normalized) return {language: '', reject: ''};
  const leading = normalized.match(/^(英文|英语|English\b|EN\b|阿文|阿拉伯|العربية\b|Arabic\b|AR\b|中文|汉语|Chinese\b|ZH\b)/i);
  if (!leading) return {language: '', reject: ''};
  return {language: LEGACY_KEYWORD_LANGUAGE.get(String(leading[1]).toLowerCase()) || '', reject: ''};
}
...
function legacyLanguageFromPreDirection(section, codeMatch) {
  ...
  if (!open) return {language: '', evidence: '', direction: '', scripts: {arabic: false, han: false, latin: false}};
  const after = section.slice(codeMatch.index + codeMatch[0].length);
  if (!/^\s*<\/pre\s*>/i.test(after)) {
    return {language: '', evidence: '', direction: '', scripts: {arabic: false, han: false, latin: false}};
  }
  const dirMatch = String(open[1] || '').match(/\bdir\s*=\s*["'](ltr|rtl)["']/i);
  if (!dirMatch) return {language: '', evidence: '', direction: '', scripts: {arabic: false, han: false, latin: false}};
  const text = extractLines(codeMatch[1]).join('\n');
  const hasArabic = /[\u0600-\u06ff]/u.test(text);
  const hasCjk = /[\u3400-\u9fff]/u.test(text);
  const hasLatin = /[\u0041-\u005a\u0061-\u007a\u00c0-\u024f\u1e00-\u1eff]/u.test(text);
  if (hasArabic && hasCjk) {
    throw extractError('section#s9 的 pre/code 同时包含阿文与中文脚本，方向证据冲突');
  }
  const dir = dirMatch[1].toLowerCase();
  const scripts = {arabic: hasArabic, han: hasCjk, latin: hasLatin};
  if (dir === 'rtl') {
    if (!hasArabic || hasCjk) throw extractError('section#s9 的 dir=rtl code 未提供唯一阿文脚本证据');
    return {language: 'ar', evidence: 'pre_dir_rtl+arabic_script', direction: dir, scripts};
  }
  if (hasArabic) throw extractError('section#s9 的 dir=ltr code 含阿文脚本，方向证据冲突');
  return hasCjk
    ? {language: 'zh-cn', evidence: 'pre_dir_ltr+cjk_script', direction: dir, scripts}
    : {language: '', evidence: 'pre_dir_ltr_unlabeled_latin_script', direction: dir, scripts};
}

/**
 * Narrowest unlabeled legacy-s9 recognition: used only when the section has
 * NO explicit language labels (no heading/note/adjacent label on any block).
 * Every block must then be independently and uniquely classifiable from its
 * own <pre dir> plus the Unicode script of its text:
 *   dir=rtl + Arabic script (no Han)            -> ar
 *   dir=ltr + Han script (no Arabic)            -> zh-cn
 *   dir=ltr + Latin script (no Arabic/Han)      -> en
 * Exactly one of each language is required; missing direction, non-Latin
 * non-Han LTR text, duplicates, or an incomplete en/ar/zh-cn set reject.
 */
function assignByUniqueDirectionScript(blocks, sectionLabel = 'section#s9') {
  const classified = blocks.map((block, index) => {
    const direction = String(block.direction || '');
    if (direction !== 'ltr' && direction !== 'rtl') {
      throw extractError(`${sectionLabel} 第 ${index + 1} 个 <code> 缺少可唯一分类的 pre dir 方向证据，拒绝猜测语言`);
    }
    const scripts = block.scripts && typeof block.scripts === 'object'
      ? block.scripts
      : {arabic: false, han: false, latin: false};
    if (direction === 'rtl') {
      if (!scripts.arabic || scripts.han) {
        throw extractError(`${sectionLabel} 第 ${index + 1} 个 <code> 的 dir=rtl 未提供唯一阿文脚本证据，拒绝猜测语言`);
      }
      return 'ar';
    }
    if (scripts.arabic) {
      throw extractError(`${sectionLabel} 第 ${index + 1} 个 <code> 的 dir=ltr 含阿文脚本，方向证据冲突`);
    }
    if (scripts.han) return 'zh-cn';
    if (scripts.latin) return 'en';
    throw extractError(`${sectionLabel} 第 ${index + 1} 个 <code> 的 dir=ltr 文本既无中文也无拉丁字母脚本证据，拒绝猜测语言`);
  });
  const counts = {};
  for (const language of classified) counts[language] = (counts[language] || 0) + 1;
  const duplicates = Object.entries(counts).filter(([, count]) => count > 1).map(([language]) => language);
  if (duplicates.length) {
    throw extractError(`${sectionLabel} 无标签三块按方向+脚本分类出现重复语言：${duplicates.join('/')}，必须唯一`);
  }
  const missing = ['en', 'ar', 'zh-cn'].filter(language => !counts[language]);
  if (missing.length) {
    throw extractError(`${sectionLabel} 无标签三块按方向+脚本分类不构成完整 en/ar/zh-cn（缺少 ${missing.join('/')}），拒绝`);
  }
  return classified;
}
...
function assignLegacyLanguages(blocks, directionOrder) {
  ...
  // No explicit language labels anywhere and no explicit order statement:
  // the only accepted fallback is unique direction+script classification of
  // every block. Any labeled block keeps the strict labeled path below.
  if (!directionOrder && blocks.length === 3 && !blocks.some(block => block.explicitLabel)) {
    return assignByUniqueDirectionScript(blocks);
  }
  if (!directionOrder) {
...
function classifyLegacyCodeBlocks(section) {
  ...
  for (const [index, match] of codes.entries()) {
    ...
    const adjacentNote = legacyLabelFromAdjacentNote(slotText);
    if (adjacentNote.reject) {
      throw extractError(`section#s9 第 ${index + 1} 个 <code> 相邻说明存在歧义：${adjacentNote.reject}`);
    }
    const contextualLanguage = inline.language || adjacentNote.language;
    if (contextualLanguage && headingLabel && contextualLanguage !== headingLabel) {
    ...
    const labeledLanguage = headingLabel || contextualLanguage;
    ...
    blocks.push({
      ...
      explicitLabel: Boolean(labeledLanguage),
      direction: preDirection.direction,
      scripts: preDirection.scripts,
...
export function extractLegacyTrilingualCoreSellingPoints(htmlText) {
  ...
  if ([...sectionMatch[1].matchAll(DISPLAYBOX_OPEN_RE)].length) return extractCodeAndDisplayboxSection(sectionMatch[1]);
...
export function extractTrilingualCoreSellingPointsAuto(htmlText) {
  ...
  return {sectionUsed: 'heading9', extracted: extractNumberedHeadingSection(htmlText)};
}
