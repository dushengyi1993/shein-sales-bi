...
export function validateMarketingPlanPairDocuments({
  ...
  if (!allowStoreCoverageSubset && expectedStoreCount !== null && expectedStoreCount !== undefined && selectionStoreKeys.length !== Number(expectedStoreCount)) {
    throw invalid(`pair must cover ${expectedStoreCount} stores; got=${selectionStoreKeys.length}`);
  }
  if (expectedStoreKeys?.length) {
    if (allowStoreCoverageSubset) {
      const expected = new Set(expectedStoreKeys.map(normalizeStoreKey));
      const outsideEnabled = selectionStoreKeys.filter(storeKey => !expected.has(storeKey));
      if (outsideEnabled.length) {
        throw invalid(`pair contains stores outside enabled store coverage: ${outsideEnabled.join(',')}`);
      }
    } else {
      compareArray(selectionStoreKeys, expectedStoreKeys, 'pair enabled store coverage');
    }
  }
...
export function verifyMarketingPlanRegistrySync({registryFile = '', file = '', registryRoot = '', expectedStoreKeys = [], allowEnabledStoreSubset = false} = {}) {
  ...
  const validated = validateMarketingPlanPairFilesSync({
    ...
    priceOverridesPath,
    requireCurrentBaseline: true,
    expectedStoreCount: expectedStoreKeys.length ? expectedStoreKeys.length : null,
    expectedStoreKeys,
    allowStoreCoverageSubset: allowEnabledStoreSubset,
  });
  ...
  return {
    ...
    rowCount: validated.rowCount,
    storeKeys: validated.storeKeys,
    storeCoverageComplete: !expectedStoreKeys.length || sameStoreKeyArray(validated.storeKeys, expectedStoreKeys),
    missingEnabledStoreKeys: expectedStoreKeys.filter(storeKey => !validated.storeKeys.includes(storeKey)),
    selectionPayloadHash: validated.selectionPayloadHash,
    pricePayloadHash: validated.pricePayloadHash,
    workFingerprint: validated.workFingerprint,
    activityBatch: validated.activityBatch,
...
async function publishVerifiedCurrentPointer({file, root, registry, expectedStores, onBeforeReplace, allowStoreCoverageSubset = false}) {
...
export async function publishMarketingPlanRegistry({
  ...
  try {
    ...
    const validated = validateMarketingPlanPairFilesSync({
      ...
      priceOverridesPath,
      requireCurrentBaseline: true,
      expectedStoreCount: expectedStores.length ? expectedStores.length : null,
      expectedStoreKeys: expectedStores,
      allowStoreCoverageSubset,
    });
