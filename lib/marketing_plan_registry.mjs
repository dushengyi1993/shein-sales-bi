...
function sameStoreKeyArray(left, right) {
  const a = [...(left || [])].map(normalizeStoreKey).sort();
  const b = [...(right || [])].map(normalizeStoreKey).sort();
...
export function readEnabledMarketingStoreKeysSync(storesConfigPath) {
  ...
  if (!enabledStoreKeys.length) {
    throw invalid('enabled stores config must contain at least one enabled store');
  }
...
function sortedStoreKeys(values) {
  return [...new Set((values || []).map(normalizeStoreKey).filter(Boolean))].sort();
}

/**
 * The single decision point for "which stores must this plan cover?".
 *
 * The enabled-store rule used to be applied at every coverage check as an extra pair of
 * parameters (`expectedStoreKeys` plus a subset allowance) that each call site had to forward.
 * Forgetting one forward failed only at the very last check of a production promotion, which is
 * how four separate release attempts died on the same defect. Resolve the expectation once here
 * instead and return the plan's own store set as the contract: a plan covers either exactly the
 * enabled stores, or - behind the explicit allowance - a subset of them, and every later check
 * then compares against the resolved set without needing the allowance itself.
 */
export function resolveStoreCoverageExpectation({
  planStoreKeys = [],
  enabledStoreKeys = [],
  ...
  const planKeys = sortedStoreKeys(planStoreKeys);
  const enabledKeys = sortedStoreKeys(enabledStoreKeys);
  if (!planKeys.length) throw invalid(`${label} must cover at least one store`);
  const countExpectation = expectedStoreCount === null || expectedStoreCount === undefined
    ? null
    : Number(expectedStoreCount);
  if (!enabledKeys.length) {
    // Callers without an enabled-store config (offline fixtures) keep the historical
    // behaviour: the plan's own set is the only expectation, plus an explicit count when given.
    if (!allowEnabledStoreSubset && countExpectation !== null && planKeys.length !== countExpectation) {
      throw invalid(`${label} must cover ${countExpectation} stores; got=${planKeys.length}`);
    }
    return {mode: 'unconfigured', expectedStoreKeys: planKeys, enabledStoreKeys: [], missingEnabledStoreKeys: []};
  }
  if (!allowEnabledStoreSubset) {
    if (countExpectation !== null && planKeys.length !== countExpectation) {
      throw invalid(`${label} must cover ${countExpectation} stores; got=${planKeys.length}`);
    }
    compareArray(planKeys, enabledKeys, `${label} enabled store coverage`);
    return {mode: 'exact', expectedStoreKeys: planKeys, enabledStoreKeys: enabledKeys, missingEnabledStoreKeys: []};
  }
  const outsideEnabledStoreKeys = planKeys.filter(storeKey => !enabledKeys.includes(storeKey));
  if (outsideEnabledStoreKeys.length) {
    throw invalid(`${label} contains stores outside enabled store coverage: ${outsideEnabledStoreKeys.join(',')}`);
  }
  return {
    mode: 'enabled_subset',
    expectedStoreKeys: planKeys,
    enabledStoreKeys: enabledKeys,
    missingEnabledStoreKeys: enabledKeys.filter(storeKey => !planKeys.includes(storeKey)),
  };
}
...
export function validateMarketingPlanPairDocuments({
  ...
  expectedStoreCount = null,
  expectedStoreKeys = [],
  allowStoreCoverageSubset = false,
} = {}) {
  ...
  const storeCoverage = resolveStoreCoverageExpectation({
    planStoreKeys: selectionStoreKeys,
    enabledStoreKeys: expectedStoreKeys,
    expectedStoreCount,
    allowEnabledStoreSubset: allowStoreCoverageSubset,
    label: 'pair',
  });
...
export function verifyMarketingPlanRegistrySync({registryFile = '', file = '', registryRoot = '', expectedStoreKeys = [], allowEnabledStoreSubset = false} = {}) {
  ...
  const enabledStoreKeys = sortedStoreKeys(expectedStoreKeys);
  // Consume the subset allowance here, once, before the immutable pair is read: the resolved
  // expectation is then compared strictly, so the pair-file check below needs no allowance of
  // its own.
  const storeCoverage = resolveStoreCoverageExpectation({
    planStoreKeys: registry.pair?.storeKeys,
    enabledStoreKeys,
    allowEnabledStoreSubset: allowEnabledStoreSubset === true,
    label: 'registry baseline pair',
  });
  const validated = validateMarketingPlanPairFilesSync({
    selectionPath,
    priceOverridesPath,
    requireCurrentBaseline: true,
    expectedStoreCount: storeCoverage.expectedStoreKeys.length,
    expectedStoreKeys: storeCoverage.expectedStoreKeys,
  });
  ...
  return {
    ...
    rowCount: validated.rowCount,
    storeKeys: validated.storeKeys,
    storeCoverageMode: storeCoverage.mode,
    storeCoverageComplete: !enabledStoreKeys.length || sameStoreKeyArray(validated.storeKeys, enabledStoreKeys),
    missingEnabledStoreKeys: enabledStoreKeys.filter(storeKey => !validated.storeKeys.includes(storeKey)),
    selectionPayloadHash: validated.selectionPayloadHash,
    pricePayloadHash: validated.pricePayloadHash,
    workFingerprint: validated.workFingerprint,
    activityBatch: validated.activityBatch,
...
export async function publishMarketingPlanRegistry({
  ...
  try {
    ...
    const enabledStores = expectedStoreKeys.length ? expectedStoreKeys : expectedStoreKeysFromConfig(path.resolve(process.cwd()));
    const validated = validateMarketingPlanPairFilesSync({
      selectionPath,
      priceOverridesPath,
      requireCurrentBaseline: true,
      expectedStoreCount: enabledStores.length ? enabledStores.length : null,
      expectedStoreKeys: enabledStores,
      allowStoreCoverageSubset,
    });
    // That pair check is the only place the enabled-store rule is decided. Afterwards the plan's
    // own resolved store set is the coverage contract: the candidate readback, the terminal
    // readback and the immutable copy all compare against it, so none of them needs the subset
    // allowance forwarded to it. Previously each of them did, and forgetting one forward failed
    // production only at the very last check.
    const expectedStores = validated.storeKeys;
