...
function assertNoForbiddenDistributionData(value, pointer = '$') {
  ...
  if (!value || typeof value !== 'object') {
    ...
    const safeStructuralToken = /\.(?:ruleKey|risk|activation)$/.test(pointer) || /\.(?:scope|tags)\[\d+\]$/.test(pointer);
    const safeMachinePolicyEnum = pointer.includes('.machinePolicy.') && OWNER_KNOWLEDGE_MACHINE_POLICY_ENUMS.has(String(value));
    if (typeof value === 'string' && !structuralHash && !safeStructuralToken && !safeMachinePolicyEnum && ownerKnowledgeTextContainsSensitiveData(value)) {
