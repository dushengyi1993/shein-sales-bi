...
import {normalizeSheinSkc, sameSheinSkc} from './shein_product_identifiers.mjs';
...
function normalizeSpu(value) {
  ...
  return /^[a-z]\d{10,}$/.test(raw) && !/^s(?:v|b)\d+$/.test(raw) ? raw : '';
}
...
export function buildPendingListingImageCorrection({
  ...
  if (!spuName || !skcName || !skuCodes.length) throw new Error('Pending listing image correction requires exact SPU, sv/sb SKC and SKU identity');
