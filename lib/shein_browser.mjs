...
import {assertChromeProfileLeaseOwner} from './chrome_profile_startup.mjs';
...
export async function connectCdp(port, {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      rejectPending(new Error(`CDP websocket open timed out on port ${port}`));
      try { ws.close(); } catch {}
      reject(new Error(`CDP websocket open timed out on port ${port}`));
    }, openTimeoutMs);
...
export async function ensureBrowser(store, {noLaunch = false, headless = false, visible = false, launchFn} = {}) {
  assertChromeProfileLeaseOwner({root: ROOT, storeKey: store?.storeKey});
