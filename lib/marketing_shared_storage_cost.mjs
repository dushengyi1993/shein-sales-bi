...
export function storageMethodForEvidence({
  storageUnitCostSar,
  storageQuantityEvidenceStatus = '',
  baseMethod = '',
} = {}) {
  const unit = numberOrNull(storageUnitCostSar);
  if (unit === null || unit < 0) return 'missing';

  const rawMethod = String(baseMethod || '').trim();
  const method = /^(?:missing|unknown|blocked(?::|$))/i.test(rawMethod)
    ? ''
    : rawMethod;
  const resolvedMethod = method || 'billing_unit_cost';
  const status = String(storageQuantityEvidenceStatus || '').trim();
  if (status && status !== PASSED_STORAGE_EVIDENCE_STATUS) {
    return `${resolvedMethod} / billing_unit_cost / crosscheck_warning:${status}`;
  }
