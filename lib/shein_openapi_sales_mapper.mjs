...
export function isOpenApiCancelledBeforePickup(order, item) {
  ...
  return goodsStatus === 6 && performanceTag === 2;
}

export function isOpenApiPostFulfillmentRefund(order, item) {
  const goodsStatus = statusNumber(item?.newGoodsStatus);
  const performanceTag = statusNumber(item?.performanceTag);
  const estimatedIncome = Number(item?.estimatedIncome || 0);
  return goodsStatus === 6
    && [1, 3].includes(performanceTag)
...
export function toOpenApiGoodsRows(orderDetails) {
  ...
  for (const order of asArray(orderDetails)) {
    ...
    for (const item of asArray(order.orderGoodsInfoList)) {
      ...
      const postFulfillmentRefund = isOpenApiPostFulfillmentRefund(order, item);
      const performanceStatus = cancelledBeforePickup
        ? 6
        : (postFulfillmentRefund ? 4 : (item.newGoodsStatus ?? order.orderStatus ?? ''));
      const performanceStatusDesc = postFulfillmentRefund
        ? '退款后履约状态待复查'
        : statusDescription(performanceStatus, PERFORMANCE_STATUS_DESC);
      const candidate = {
        number: 1,
        ...
        goodsPerformanceStatusDesc: performanceStatusDesc,
        orderStatusDesc: statusDescription(order.orderStatus, ORDER_STATUS_DESC),
        performStatusDesc: postFulfillmentRefund
          ? '退款后履约状态待复查'
          : statusDescription(order.orderStatus, PERFORMANCE_STATUS_DESC),
      };
