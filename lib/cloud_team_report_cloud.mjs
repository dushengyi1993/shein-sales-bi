...
async function sendItem({kind, context, config, deliveryDir, file, summary, spawnImpl, timeoutMs, executionIdentityVerified, signal}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const preflightFailure = raw.spawnError === true
      || (raw.exitCode !== 0 && !raw.stdout
        && /(?:connection|connect|resolve|no route|refused|unreachable|timed out|timeout)/iu.test(raw.stderr));
...
export async function deliverCloudTeamReport({
  ...
  try {
    ...
    try {
      ...
      if(context.automationId==='shein-3' && freshState) {
    ...
    for (const kind of ['summary', 'attachment']) {
      ...
      if(context.automationId==='shein-3' && !freshState && existing?.confirmedFailure!==true) {
      ...
      if(context.automationId==='shein-3') {
        ...
        state.status='unknown';
        // Durable before any external call: a hard exit cannot leave a
        // pending/partial state that later looks safe to resend.
        await persistState(paths.stateFile,state,now);
      }
      ...
      if (response.accepted) {
        ...
        if(context.automationId==='shein-3')item.confirmedFailure=response.retryable===true && response.unknown!==true;
