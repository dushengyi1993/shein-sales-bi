...
function newState(context, now) {
  return {
    ...
    ...(context.semanticDigest ? {semanticDigest: context.semanticDigest} : {}),
...
function validateBundle(bundle) {
  ...
  return {
    ...
    fingerprint,
    // Optional same-day semantic digest (see ops_business_result_pipeline): it
    // is persisted with the state so a later run can tell "the same situation
    // was reported again" from "a new situation", independently of the
    // byte-based delivery fingerprint.
    semanticDigest: typeof bundle.semanticDigest === 'string' && /^[a-f0-9]{64}$/.test(bundle.semanticDigest)
      ? bundle.semanticDigest
      : '',
    summaryBytes,
    attachmentBytes,
    idempotencyKeys: {
