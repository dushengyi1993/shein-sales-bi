...
import {fetchWithIdempotentNetworkRetry} from './bi_ops_query_retry.mjs';
import {validateOwnerKnowledgeDistribution} from './owner_knowledge_distribution.mjs';

export const BI_OPS_CLI_VERSION = '2026.08.24.2';
...
const CACHE_FALLBACK_MAX_AGE_MS = 5 * 60_000;
const FETCH_RETRY_WRAPPER_CODES = new Set(['BI_TRANSIENT_NETWORK_RETRY_EXHAUSTED']);
const STALE_CACHE_FETCH_CODES = new Set(['ECONNRESET', 'ETIMEDOUT']);
...
async function fetchJson(fetchImpl, url, options = {}, timeoutMs = 60_000, responseState = null) {
  ...
  const trackedFetch = async (...args) => {
    ...
    if (response !== undefined && response !== null && responseState) responseState.received = true;
    return response;
  };
  try {
    const response = await fetchWithIdempotentNetworkRetry(trackedFetch, url, {...options, signal: controller.signal});
    ...
    if (controller.signal.aborted && responseState) responseState.aborted = true;
...
function isRecentValidatedCache(validatedCache) {
  const checkedAt = Date.parse(String(validatedCache?.cached?.checkedAt || ''));
  const ageMs = Date.now() - checkedAt;
  return Boolean(validatedCache)
    && Number.isFinite(checkedAt)
    && ageMs >= 0
    && ageMs <= CACHE_FALLBACK_MAX_AGE_MS;
}

function isPreResponseTransientFetchError(error) {
  if (!error || error.responseReceived === true || error.signalAborted === true
    || error.response !== undefined || Number(error.status) > 0) return false;
  const codes = [];
  const nodes = [];
  let current = error;
  for (let depth = 0; current && depth < 6; depth += 1) {
    ...
    if (code && !FETCH_RETRY_WRAPPER_CODES.has(code)) codes.push(code);
    current = current?.cause;
  }
  const fetchFailedMessage = nodes.some(node => /^fetch failed$/i.test(String(node?.message || '').trim()));
  const knownMessage = nodes.some(node => /\b(?:ECONNRESET|ETIMEDOUT)\b/i.test(String(node?.message || '')));
  const unknownCode = codes.some(code => !STALE_CACHE_FETCH_CODES.has(code)
    && !(fetchFailedMessage && code === 'UND_ERR_SOCKET'));
  return !unknownCode
    && (fetchFailedMessage || codes.some(code => STALE_CACHE_FETCH_CODES.has(code)) || knownMessage);
}
...
function staleVerifiedCacheResult(root, validatedCache, {cliVersion, strict}) {
  const recommended = checkCliVersion(validatedCache.data, cliVersion);
  if (strict && validatedCache.data.current === false) {
    throw new Error('负责人规则正在同步 GitHub，真实执行暂时停住，请稍后重试');
  }
  return {
    ok: true,
    updated: false,
    ...
    cacheDir: root,
    source: 'stale-verified-cache',
    checkedAt: validatedCache.cached.checkedAt,
    ...
    warning: '负责人规则服务在收到响应前暂时不可用，已使用5分钟内且重新校验通过的本地缓存',
  };
}

async function ensurePartnerKnowledgeCurrentLocked({
  baseUrl, cookie, root, fetchImpl, cliVersion, strict, fetchTimeoutMs, allowTransientCacheFallback,
}) {
  const manifestFile = path.join(root, 'manifest.json');
  let cached = await readJson(manifestFile, {});
  let validatedCache = await loadValidatedCache(root, cached);
  const responseState = {received: false, aborted: false};
  const commonHeaders = {
  ...
  try {
    ...
    }, fetchTimeoutMs, responseState);
    if (manifestResult.response.status === 304 && !validatedCache) {
      manifestResult = await fetchJson(fetchImpl, `${String(baseUrl || '').replace(/\/+$/, '')}/api/owner-knowledge/manifest`, {headers: commonHeaders}, fetchTimeoutMs, responseState);
    }
    ...
    if (!cacheMatches) {
      const bundleResult = await fetchJson(fetchImpl, `${String(baseUrl || '').replace(/\/+$/, '')}/api/owner-knowledge/bundle`, {headers: commonHeaders}, fetchTimeoutMs, responseState);
    ...
    if (allowTransientCacheFallback
      && !responseState.received
      && !responseState.aborted
      && isPreResponseTransientFetchError(error)
      && isRecentValidatedCache(validatedCache)) {
      return staleVerifiedCacheResult(root, validatedCache, {cliVersion, strict});
    }
    throw error;
  }
}
...
export async function ensurePartnerKnowledgeCurrent({
  ...
  try {
    return await ensurePartnerKnowledgeCurrentLocked({
      baseUrl,
      cookie,
      root,
      fetchImpl,
