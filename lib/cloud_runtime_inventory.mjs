...
// Marketing repair is currently dispatched by its business owner. A disabled,
// inactive timer is intentional; an enabled timer that stopped remains a fault.
export function intentionalCloudTimerPause(name, status = {}) {
  return name === 'shein-bi-cloud-marketing-repair.timer'
    && status.LoadState === 'loaded' && status.ActiveState === 'inactive'
    && status.UnitFileState === 'disabled'
    ? {name, reason: 'marketing_repair_managed_by_business_owner'} : null;
}
...
export const CLOUD_MAINTENANCE_POLICY_ROWS = Object.freeze([
  "shein-bi-backup-sync-nas.service\tinfrastructure",
  "shein-bi-clock-sanity.service\talways",
