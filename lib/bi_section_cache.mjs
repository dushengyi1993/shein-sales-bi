...
async function readJsonFile(file, fallback = null) {
  try {
    ...
    return JSON.parse(decodeUtf8Strict(raw).replace(/^\uFEFF/, ''));
  } catch {
    return fallback;
  }
}
...
function createAbortError() {
  const error = new Error('BI section parse cancelled by deadline or client disconnect');
  error.name = 'AbortError';
  error.code = 'ABORT_ERR';
  return error;
}

let biSectionParseActive = false;
const biSectionParseQueue = [];

/**
 * Run one full section reader at a time. Admission happens before the worker
 * opens the file, while an active worker keeps its slot through read, decode,
 * parse, and close. A queued abort removes only that waiter and never invokes
 * its worker; an active abort is surfaced after the worker reaches its own
 * cleanup boundary.
 */
export function withBiSectionParseSlot(work, options = {}) {
  if (typeof work !== 'function') return Promise.reject(new TypeError('Section parse worker must be a function'));
  const signal = options?.signal || null;
  if (signal?.aborted) return Promise.reject(createAbortError());

  return new Promise((resolve, reject) => {
    const entry = {work, signal, resolve, reject, started: false, settled: false, onAbort: null};
    const removeQueuedEntry = () => {
      const index = biSectionParseQueue.indexOf(entry);
      if (index >= 0) biSectionParseQueue.splice(index, 1);
    };
    const settleQueuedAbort = () => {
    ...
    entry.onAbort = settleQueuedAbort;
    signal?.addEventListener('abort', entry.onAbort, {once: true});
    biSectionParseQueue.push(entry);
    admitNextParseSlot();
  });
}

function finishParseEntry(entry, error, value) {
  if (entry.settled) return;
  entry.settled = true;
  entry.signal?.removeEventListener('abort', entry.onAbort);
  biSectionParseActive = false;
  if (entry.signal?.aborted) entry.reject(createAbortError());
  else if (error) entry.reject(error);
  else entry.resolve(value);
  admitNextParseSlot();
}

function admitNextParseSlot() {
  if (biSectionParseActive) return;
  let entry;
  while (biSectionParseQueue.length && !entry) {
    const candidate = biSectionParseQueue.shift();
    if (!candidate.settled) entry = candidate;
  }
  if (!entry) return;
  entry.started = true;
  biSectionParseActive = true;
  Promise.resolve()
    .then(() => entry.work(entry.signal))
    .then(value => finishParseEntry(entry, null, value), error => finishParseEntry(entry, error, undefined));
}
