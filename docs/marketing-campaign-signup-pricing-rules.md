...
2. **每日巡检必须有 live 覆盖。** 优先使用云端可用浏览器；没有云端浏览器时使用本地店铺 profile，按 `3-5` 店小批次运行，跑完立即关闭，避免影响本机。
3. **限时折扣是所有在售运营链接必备层。**
...
7. Top5 收回价必须来自最新批准全量 baseline 中同一 `store + SKC` 的普通档价或显式 `ordinaryLinkApprovedPrice`。同货号不同店铺可以有不同普通档价，禁止用同货号中位数统一覆盖；缺该链接精确普通档价时 fail closed。
