...
- `按货号汇总` 固定追加当前 ET 可售库存、在途库存和近 7 天销量；同一标准货号只展示一份 canonical 仓储证据，不得用“另 N 行缺失”描述链接级字段空值。
- 每次用户确认后，把当期利润率、货号归并和例外吸收到下一版基准；下一版不再保留“上次确认备注”列，只保留本次可填写的备注列。
...
- 用户在确认表备注中点名“同一货号/归并”的，必须先更新全局 alias 并重新归并成本、仓储、库存、曝光和活动行，完成后才能计算价格和提交。
- 典型归并：
...
2. 用户指定固定价（A1 明确价格优先：用户明确指定的商品/店铺价格如 FY 37 SAR 作为唯一结构化权威事实贯穿策略与执行；用户已指定价时，利润线只生成提示，不能换价、剔除、拒绝或再次要求例外批准；整数价格保持原值，跳过微调；成本未知如实显示“利润暂算不出”，不伪造利润，也不因成本未知篡改用户明确价格。没有明确指令的自动任务继续使用原本默认定价策略）
3. 用户指定利润率
- 从用户确认本轮方案起，到下一轮方案确认前，限时折扣也按同一份已确认基准派生，不能继续沿用更早方案、旧逐行价格或默认利润率。
4. 新品保护价
5. 库存沉淀 / 清货候选价
6. 默认利润率规则
...
- 营销活动用的仓储费/件必须来自 BI `profit.productStorageDaily`：
  ...
  - 整箱仓储明细必须先统一成“件”单位：`fact.et_box_item.real_quantity` 非正数时不能压过正数 `case_quantity`，应优先取正数 `real_quantity`、否则取正数 `case_quantity`、都不可用才取 `0`。否则箱费会进入分子、箱内件数却从分母消失或被负数冲减。
  - `productStorageDaily.storage_quantity` 只代表同一仓储费分子覆盖的已单位化计费库存，因此仍是移动余额的主分母；不能直接改用销量，也不能无条件改用更大的运营库存，以免刚到货但尚未计费的新库存稀释历史仓储成本。
  - 成本映射必须用 fresh、matched 的 `inventoryDepletion.current_sellable_quantity + et_damaged_qty` 做交叉校验：与仓储明细最新日期相差不得超过 `2` 天，件数相对差异不得超过 `15%`。任一历史日出现“有仓储费但计费数量缺失/非正数”，或当前证据未匹配、过期、非正数、超差时，仓储费/件必须置空并阻断含仓储自动判断，不能从可疑分母算价。
  - `data.json` 与独立 `profit` / `inventoryTrend` section 并存时必须按 `generatedAt` 选择同等或更新鲜的 section；不得让旧全量快照覆盖已刷新的库存 section。
  - 一旦成本映射已给出仓储证据冲突状态，普通活动审核与价格栈审核都不得再用“零销量共享仓储估算”补回仓储费；该回退仅允许用于没有冲突状态、且库存 matched、库存与费用日期差不超过 `2` 天的历史兼容行。
  - 成本映射必须同时记录已单位化计费库存、运营可售/破损、实际采用分母、候选仓储费/件、分母来源、日期差、数量差和证据状态。`storageCurrentQuantity` 继续保留为计费库存兼容字段，不能误写成运营库存。
