...
- SHEIN Webhook 正式回调为 `https://sa.dushengyi.cc/api/shein/webhook/v1/events`，复用标准 443 但仍保持独立来源校验。HAProxy 仅允许 Cloudflare 直接来源进入该 SNI 的 HTTPS 分支并转到 Caddy `10443`；Caddy 只在此受控上游后接受 Cloudflare 写入的原始客户端 IP，Nginx 再按 SHEIN 官方推送 IP 放行，最终反代到 `127.0.0.1:8792`。`8443` 只保留为受限故障回退，不是平台生产回调；不要对全网开放，也不要让 Node 服务直接暴露公网。
