...
- 本轮稳定化 manifest 版本为 `2026.08.24.2`。运行时 `BI_OPS_CLI_VERSION` 必须在集成时同步，且既有包测试必须证明两者相等后才能发布。该版只对 GET/HEAD、429/502/503/504 与连接重置等可安全重放的读取增加有界退避；execute 及全部非幂等方法仍只调用一次。
