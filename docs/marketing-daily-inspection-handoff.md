...
巡检完成与全部修复完成分开说明：前者核验当天 19 店扫描、stack review 和 guard 原始证据；后者还必须核验同日修复终态、19 店最终价格栈 readback/guard 重建和原报告。11:30 只读取既有工件并展示原结论及附件；修复仍在进行或失败时明确待处理项，不把时间已到当成整轮完成，不创建后续轮询或重跑业务。核验飞书文字与同一附件的持久交付状态后才称飞书已发送。云端成功发送的部分跳过，发送前持久记录未知状态；未收到明确成功回执时禁止自动重发，也不得通过改报告指纹绕过。
