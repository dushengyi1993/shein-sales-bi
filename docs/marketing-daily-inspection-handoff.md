...
按用户 2026-09-07 确认，云端负责巡检、修复和最终飞书报告，Codex 只在次日 00:30（Asia/Shanghai）通过现有 `shein-2` 汇总前一业务日的结果一次。从每日 11:00 开始预留 13 小时 30 分钟；这是宽裕的汇总时间，不是完成保证。近期存在 23:18 仍更新且未终态的队列，不得用 pending/failed 的更新时间冒充成功完成耗时。禁止恢复 11:10 启动或每 20 分钟轮询。

称为已完成之前必须核验该业务日终态、19 店最终 stack review/价格栈 readback/guard 重建和原报告，并核验飞书文字与同一附件的持久交付状态。00:30 只读取这些原始工件：齐备则展示同一结论及附件；仍未结束或失败则只输出一次准确状态、原因和是否需要用户处理，不把时间已到当成完成，不创建后续轮询或重跑昨日业务。云端成功发送的部分跳过，发送前持久记录未知状态；未收到明确成功回执时禁止自动重发，也不得通过改报告指纹绕过。
...
guard 尚未结束时由现有云端链继续处理，Codex 不轮询；次日 00:30 按 2.0 节汇总。获准的大批修复另入队列，由 worker 完成，不能再与巡检同步串行。
...
guard 的 `runId` 写入不可覆盖的 `state/cloud_marketing_live_guard/reports/marketing-live-guard-<date>-<runId>.json`。云端每天 `11:00` 只启动一个只读 coordinator；普通活动 review、价格栈 scan 或 guard 报告瞬时失败时，仅在同一个 run 内有界重试失败阶段，不再于 13:00/16:00 重跑整套。现有云端 worker 持续处理同一精确队列，本机自动化只在次日 `00:30` 读取结果。只有精确回读成功的组才可跳过，最终 19 店回读完成后只发送一次群报告。
...
日内变化写入云端原报告，不逐项通知。次日 00:30 汇总一次，包括无新增动作或尚未完成的准确状态；仅确实必须由用户处理才能继续的事项可中途提醒。
