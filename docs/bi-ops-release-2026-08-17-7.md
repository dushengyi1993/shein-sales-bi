...
- 灾备：现有数据库备份同轮流式生成 AES-256-GCM Profile/session v2 归档并完整复验；create/verify/restore 共享资源硬门，hard-link 发布后失败会撤销本次正式输出，认证记录保存 numeric uid/gid、mode/mtime，拒绝 `chown(-1)` 哨兵并在应用身份后精确回读，避免 root 恢复破坏运行用户访问。Linux restore 绑定父目录 fd 与 captured staging dev:ino，只通过 `renameat2(RENAME_NOREPLACE)` 发布且 parent fsync 必须精确为 `ok`；任何失败均保留 captured staging，竞争 destination、替换 staging、已发布 destination 与耐久性未确认状态都不递归删除并要求人工核验；COS tar 必须逐成员验证类型、路径、清单和实际内容 hash 后才可删除本地。数据库备份 final 目录发布同时拒绝同名 collision，并按 staging dev:ino 做终态回读，避免普通 `mv` 把 staging 嵌入已存在目录。
...
2. 下载并 live 核验 Release 两份 attestation、Tag、immutable Release 终态与 exact CI attempt；备份 tracked diff、运行态、Portal 输出和 Profile/session。
...
6. 生成/验证真实 encrypted browser-state backup，以 no-replace 原子发布恢复到全新 staging，确认 parent fsync=`ok` 后做逐文件演练；异机保存恢复密钥副本。
...
源码候选必须通过 `npm test`、四分片 CI、`git diff --check`、独立 GPT 对抗审查和同 SHA main-push CI。Link Ops 还必须证明无 claim 不写、claim/结果均为单任务 CAS、post-commit 异常不会诱导重试、终态不能二次提交；任何聊天通用异常也不得邀请写操作盲重试。生产完成还必须同时满足：schema v3 deployment marker（`shein-bi-deployed-release/v3`）、Portal/Query/Webhook/watchdog 健康、Query 无副作用、Portal 重启不影响 Query、五个事故 section 为当前 generation 的 200/非 stale/无 revision 追逐、Profile 真实 staging restore、无 OOM/restart 回退，以及自动化分阶段恢复回读。
