...
- 商品接收、审核、上下架和删除审核：进入“平台动态”。公网接收进程不读取可变运营任务表；由登录态 Portal 内的独立特权 reconciler 读取已验签、已成功、已归一化的 receipt，以店铺 + 至少两项强商品身份唯一匹配任务并追加回读。Portal 只能调用受限数据库函数登记 `attached / unmatched / ambiguous`，不能读取密文或任意修改 receipt。
...
- `3001461` 按官方字段归一化 `placeRequestId / deliveryNo / changeTime`，平台动态显示下单编号、运单包裹号和创建时间，不再显示泛化“状态已更新”。
- `3001792` 按官方 `audit_state=1/2/3/5/6` 翻译为审核中、通过、未通过、部分未通过、撤回，并补商品货号/名称/链接；未通过和部分未通过进入 BI 待处理，不直接改价。
- `3001793` 归一化 `end_effective_date`；已到期或 7 天内到期进入 BI 待处理，`9999-12-31` 展示为长期有效。
- 商品接收失败、商品审核、上下架、删除审核和合规事件统一补只读商品上下文；数据库未匹配时再用当前店铺 OpenAPI 按 SKC 补货号、名称和当前状态，回调及权威读源都没有的字段不猜、不展示。合规失效通知必须把官方 `complianceTypeId` 翻译为具体缺口（当前 `1=欧盟责任人`、`3=商品实拍图`），并显示货号/商品、SKC、当前状态、可用销售摘要和失效时间；BI 与 SHEIN 当前商品列表都查不到的历史/停用商品降为页面关注项，不再向飞书发送紧急告警。
- 官方字段契约：
  - [3001461 SHEIN 合作物流单下单通知](https://open.sheincorp.com/documents/msgdoc/detail/3001461)
  - [3001792 建议零售价审核状态更新](https://open.sheincorp.com/documents/msgdoc/detail/3001792)
  - [3001793 建议零售价有效期变更](https://open.sheincorp.com/documents/msgdoc/detail/3001793)
...
- `shelfState=0` 但没有历史上架或明确下架证据时，不直接推断“已下架”。worker 会用该店 OpenAPI 权限按 SKC 查询商品，再调用 `spu-info` 回读 SHEIN-SA 当前状态与生命周期时间。官方 OpenAPI 通常只给 `0 非已上架 / 1 已上架`；只有同时出现“从未上架”的两枚平台占位时间、且未进回收站时，才把 `0` 可靠细分为“待上架”。已售罄与历史下架仍由后续明确事件或日更四态快照补全。
- 商品审核、涨价审核或建议零售价审核明确通过时同样只把事件当成回读触发器；只有“当前非上架 + 从未上架证据”成立时才更新为待上架，不能把所有“审核通过”直接硬编码成待上架。
...
`3000848` 的 `shelfState=0` 只证明“当前不是已上架”，不能单独证明“曾经上架后被下架”。新商品审核通过、进入待上架流程时也会收到该值，并伴随平台占位时间 `1970-01-01 08:00:01` / `2018-08-28 00:00:00`。系统必须丢弃这两个占位时间；只有回调携带有效首次上架时间、明确的下架动作，或仓库能证明该 SKC 以前确实上架/成交时，才按下架告警。其余情况仅作为普通未上架状态留档，不进入 BI 重点动态或飞书。
...
飞书：复用 `lark-cli im +messages-send` 的现有通知身份，目标优先使用 `config/lark_report.json` 的团队 `recipientChatId`，不再默认私聊负责人；仍只发送需要立即处理的事项。普通事件按 receipt 幂等，官方无事件 ID 的授权重签按 10 分钟时间桶去重，不恢复已暂停的问数服务。消息正文直接给出业务影响和下一步，不使用“原因：英文代码”“处理原则”之类机器话。详情与普通事件留在 BI，避免刷屏。
...
2026-07-23 扩展回读：19/19 App 的正式/测试回调继续为标准 443 地址且审核通过；新增的 `3001461`、`3001792`、`3001793` 均逐店回读为 3/3，当前每店总订阅为 13/13。扩展订阅形成 57 条技术验证 receipt，全部 `succeeded + P3 + appScopedOnly`，`alerted_at`、`business_key`、经营 gate 引用和失败/dead-letter 均为 0。
