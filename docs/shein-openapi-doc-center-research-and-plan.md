...
> 基础研究时间：2026-07-03；2026-07-19 复核官方目录为 243 条（OpenAPI 220、Webhook 23），新增 `3001903 商品删除审核通知`。来源：SHEIN 开放平台文档中心公开页面（通过浏览器 CDP 逐页读取）+ 官方目录接口抓取。本文档不包含任何密钥、Cookie 或店铺授权值。
...
| API 文档 | 220 条接口（2026-07-19） | 官方目录接口 `queryAllApiDocCategoryList?categoryType=1` 抓取 |
| WebHook 文档 | 23 条事件（2026-07-19） | 官方目录接口 `queryAllApiDocCategoryList?categoryType=2` 抓取 + 详情复核 |
| 图片方案系统文档 | 1 篇 | 浏览器 CDP 完整读取 |

### 研究方法

1. 通过 `scripts/sync_shein_openapi_doc_catalog.mjs` 抓取官方目录接口；2026-07-19 当前为 243 条接口/事件。
...
| 指标 | 数量 |
|------|------|
| 官方目录接口总数 | 243（2026-07-19） |
| OpenAPI 接口 | 220（2026-07-19） |
| WebHook 事件 | 23（2026-07-19） |
| 只读接口 | 137（2026-07-03 基础快照；新增 3 个 OpenAPI 尚未在本文重分类） |
| 写接口 | 80（2026-07-03 基础快照；新增 3 个 OpenAPI 尚未在本文重分类） |
...
| `webhook_candidate` / first phase integrated | 23 | 官方消息能力；首批 10 类已接收、落库、处理和展示 |
...
| 商品删除审核通知 | `/product_delete_audit` | 删除获批或审核失败均进入 P0 平台动态 |
...
```
SHEIN 平台 → Cloudflare/Caddy/Nginx 来源过滤 → receiver 验签/解密
                                                     ↓
                                            AES 密文可靠落库 + 幂等队列
                                                     ↓
                                               异步事件分发器
                       ...
                       ┌─────────────────────────────┼─────────────────────────────┐
                       ↓                             ↓                             ↓
                 BI“平台动态”              订单/退货定向只读补采          仅 P0 飞书摘要
                 （普通事件入口）           授权/额度写闸门               （不恢复问数）
```
...
| WebHook 接收服务 | 已完成第一阶段 | 商品生命周期、订单/退货、授权/额度/合规；真实回调以各 App 订阅审核为准 |
...
### 阶段三：WebHook 接入（P2，第一阶段已完成）

- 已实现独立 receiver、官方验签/AES、只存密文的幂等队列和租约 worker。
- 已实现商品生命周期、订单/退货按单增量、授权/额度/合规风险处理和 BI“平台动态”。
- 飞书只发 P0；Webhook 不直接执行 SHEIN 写操作。
- 待平台侧工作：在 19 个 App 配置统一 callback、订阅首批 10 个事件并完成真实加密回调验收。
