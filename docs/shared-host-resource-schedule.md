...
| `11:15–11:27 / 12:15–12:27 / 14:45–14:57` | 半托 | 晨间缺店恢复 | 仅补当天缺失店；marker 已完成则不打开浏览器；避开10:20 ET，Portal低优先队列主动让路 |
| `09:12–09:27` | 半托 | OpenAPI/成本/利润补充阶段 | 必须等 `morning-links-ready`；商品详情轮转由 `07:12` 库存任务负责；不再内嵌 RTV |
| `15:15–15:27` | 半托 | 库存自动守卫主执行 | 依赖 `morning-links-ready` 与当天 `15:12` 后的 `stock-refresh` |
| `15:45–15:57` | 半托 | 库存自动守卫条件重试 | 仅主执行未完成时启动；已有完成 marker 直接退出，禁止重复写 |
| `11:50` | 全托 | 日更缺口补采 | 关键任务优先 |
| `12:45` | 全托 | COS 归档 | 非上班前业务数据；给完整受控窗口，避免 partial 大文件 |
| `14:20–14:27` | 半托 | ET 仓储费 | 只读账单、利润 cache 和对账 |

其它半托 ET 检查点保留 `07/10/13/17/20/23:20`；营销只读 guard 为 `11:00/13:00/16:00`，不再占 `:30`。负责人本机在 `11:10/13:10/16:10` 用单店串行 headless Chrome 消费长期授权工作；服务器平时不跑营销写。只有到晚间仍未闭环时，云端在 `20:45–20:57`、`21:15–21:27` 两段各兜底最多 1 店/1组；`21:02–21:13` 保留全托首页，`20:57/21:27` 后不得派新组，锁忙或剩余预算不足直接保留到下一段/次日。
...
- repair 以本机后台浏览器为主；云端应急 repair 先用 session HTTP 重扫避免重放，再拿 host 锁、半托项目锁和营销域锁，每窗最多 1 组。
- 本地与云端 Profile/session 独立维护，不以复制 Cookie 作为日常同步。云端 `00:45` session manager 继续服务服务器；本地只在营销队列命中店铺时探测/恢复对应 Profile。
- 本机 Chrome 磁盘缓存放在持久 Profile 之外并限制 100MB；批次结束清理可丢弃模型/缓存，登录 Cookie、Local/Session Storage、IndexedDB 永不进入清理目标。
