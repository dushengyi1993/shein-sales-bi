...
| 每小时 `:12` | 半托 | 19 店当前库存主轮 | 轻量快车道；09:12 成功写 `stock-refresh` marker |
...
| `08:00–08:27` | 半托 | 链接/业务域前 14 店 | 仅抓取，写 `morning-chunk-1`，不提前合并 |
| `08:45–09:00` | 半托 | 剩余 5 店 + 19 店合并 | 依赖 chunk 1；同步发布 `linksData`，写 `morning-links-ready` |
| `08:55–09:11` | 半托 | OpenAPI/成本/利润补充阶段 | 必须等 `morning-links-ready`；不再内嵌 RTV |
| `09:15–09:27` | 半托 | 库存自动守卫 | 同时依赖 `morning-links-ready` 与 09:12 `stock-refresh` |
...
flowchart LR
  ...
  B --> Y
  C1["08:00 前14店 fetch-only"] --> C2["08:45 后5店 fetch-only"]
  C2 --> M["19店证据合并 + linksData"]
  M --> D["08:55 日更补充"]
  M --> I["09:15 库存守卫"]
  ST["09:12 stock-refresh"] --> I
```
