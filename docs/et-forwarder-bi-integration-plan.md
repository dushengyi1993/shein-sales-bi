...
财务口径建议：成本表仍是头程和上架等已整理费用的主口径；ET 财务用于核对和补充“仓储费、拣货费、出库相关费用”。仓储费货号层主路径使用 `ExportStoreFee` SKU/箱号明细；只有缺明细日期才按“库存量 × 体积 × 天数”的 stock-days 模型摊分，页面明确标注为估算分摊。
...
ET 财务账单中已能抓到每日仓储费总账（sort_name=仓储费）。`/Finance/IncomeBill/GetDetailGridJson` 对仓储费仍可能为空，但网页端行工具 `ExportStoreFee` 可下载当日 SKU/箱号仓储费 CSV 明细；该导出明细是货号层正式来源。只有某些日期缺明细时，才按 ET 体积 × 库存天数估算并校准到每日实际仓储费总额。
