...
更新时间：`2026-05-15T23-05-00`
当前状态：ET 已完成云端接入和生产入仓；本地 `SHEIN-Sales-ETForwarder-0420` 已禁用，本文件早期 `04:20/07:00` 调度安排仅作为历史设计。当前生产由 `shein-bi-cloud-et-forwarder.timer` 按奇数小时 `:20` 高频抓取 ET 货代仓/出库单，入仓后只轻量刷新订单/物流/售后相关 section。
...
> 2026-06-20 云端全量回灌和用户业务确认后，生产 `fact.et_ship_order` 当前全量为 `95` 单，`fact.et_ship_order_item` 约 `107` 行，箱明细约 `2512` 行；这不是缺采。点名发货单 `F2602034904752` 已存在于 ET 发货单事实中。后续页面/告警不得再把 95 单或“ET发货单累计 < ET库存 + 历史销量”写成“发货单缺采”，只能标为 `ET库存/销量匹配待核`，用于排查货号归并、销量或库存匹配问题。
...
| 物流仓服账单明细 | `/Finance/IncomeBill/GetDetailGridJson?id=...` / `ExportStoreFee` | 按账单 | 拣货费等可回到 SKU；仓储费以网页端 `ExportStoreFee` 导出 CSV 为 SKU/箱号正式明细源 |
...
财务口径建议：成本表仍是头程和上架等已整理费用的主口径；ET 财务用于核对和补充“仓储费、拣货费、出库相关费用”。仓储费货号层主路径使用 `ExportStoreFee` SKU/箱号明细；若历史明细合计与 ET 总账不一致，则保留明细分布并缩放到总账，只有完全缺明细日期才按“库存量 × 体积 × 天数”的 stock-days 模型摊分，页面明确标注为估算分摊。
...
- 当前生产：`01:20/04:20/07:20/10:20/13:20/17:20/20:20/23:20` 按经营检查点同步 ET 货代仓/出库单；早期每两小时与单次 `04:20` 排班只作为历史设计参考。
...
- 旧 `07:00` BI 每日流水线只作为历史设计参考；当前由云端销售刷新、晨间链路和 section 预热分工完成。
...
5. 定时任务与提醒：当前 ET 已由云端 `shein-bi-cloud-et-forwarder.timer` 高频执行；BI/日报只消费已入仓的 ET 状态，旧 `04:20 ET` / `07:00 BI` 设计不再作为生产排班。

## 10. 需要你确认的点

1. ET 当前已按奇数小时 `:20` 高频同步；如需再调整，只能以生产资源排班和 watchdog 新鲜度为准，不恢复旧 `04:20` 单次日更。
2. 仓储费明细缺失日期，是否接受按“库存体积 × 天数”估算分摊并在 BI/营销中标注兜底口径？
...
- 财务仓储费普通明细接口对仓储费可能为空，但网页端 `ExportStoreFee` 已确认可下载 SKU/箱号日明细；缺明细日期才估算分摊。
...
ET 财务账单中已能抓到每日仓储费总账（sort_name=仓储费）。`/Finance/IncomeBill/GetDetailGridJson` 对仓储费仍可能为空，但网页端行工具 `ExportStoreFee` 可下载当日 SKU/箱号仓储费 CSV 明细；该导出明细是货号层正式来源。若历史明细合计与总账不一致，则以 ET 总账为金额真相、用下载明细提供货号分布并按日缩放；只有某些日期完全缺明细时，才按 ET 体积 × 库存天数估算并校准到每日实际仓储费总额。
