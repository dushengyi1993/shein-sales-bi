...
- 默认禁止在 heartbeat 中向写入型脚本传 `--execute`，包括优惠券取消、券预算补额度、无证据结束限时折扣、无计划创建/修改限时折扣。唯一默认可自动执行的写入例外是“新链接 / 新上架 7 天 / 漏限时折扣兜底”的限时折扣创建或重建；也必须满足身份校验、价格栈校验、库存/平台校验、dry-run 和执行后 live 回读。
