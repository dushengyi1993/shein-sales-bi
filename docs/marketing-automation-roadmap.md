...
> 当前状态：2026-08-02。本文只沉淀系统规则、动作边界和后续实现顺序；生产排班以 `infra/systemd/*.timer` 为准，不写 SHEIN session、BI 登录密码、OpenAPI 密钥或 `.local` 运行态。

## 1. 核心不变量

营销活动、限时折扣、优惠券和订单成交价必须作为同一个价格栈管理。系统目标不是“看起来都报了活动”，而是每个 `店铺 + SKC + 时间窗口` 的最终成交价尽量等于本期目标价，且不得低于利润/底价红线。

- 普通营销活动：主价格层，平台会给流量扶持；符合策略且在报名窗口内必须报名，且优先级最高。报名窗口和生效窗口分开，平台最低降幅按活动页/API 动态读取（常见 `10%`，也可能 `5%` 或 `VIP档`）；平台最低档即使低于用户批准价也必须直接报名并标风险，不再阻断或重复确认。
...
8. **已批准普通活动截止前自动补漏。** 对已经获得用户批准并处于执行/已执行状态的普通活动批次，报名截止前 live review 新出现的 `extraAvailableRows`、`outOfPlanRows` 或 `applyGoodsNum < allowGoodsNum` 差额，巡检必须当轮自动生成 supplement、锁定原批准来源、dry-run、提交并定点回读，不再逐次确认。价格优先克隆同店同 SKC 已批准行，否则按当前最终基准和既定规则计算；未批准的新活动方案、永久库存增加和优惠券不在该授权内。已获报名授权的普通活动可在报名事务中临时补到平台精确最低库存，但必须在 `finally` 恢复原可用库存。

### 1.2 2026-08-02 报名库存事务与低库存畅销品价格收回
...
- 名称：`SHEIN 每日营销巡检`
- 计划：完整巡检独立运行。11:00 guard 完成后 OnSuccess 启动同一云端 repair；原 repair timer 在 11–20 点每小时 :45 和 21:15 续跑延期队列，不增加 heartbeat 或第二套 timer。
...
- 云端 guard timer 负责只读 live scan、精确计划和当天队列；修复由现有 cloud-primary repair 独占执行，每轮最多 32 组、每次一项/一组串行，遵守资源错峰和期限。业务任务负责监看与权威回读，不与云端重复执行。长期授权内任务不需要 V6 审批或临时开窗；只有代码缺陷及发布交给 V6。延期后原 timer 自动续跑，跨日不得重放旧队列。
...
- 默认禁止本地 heartbeat 向无授权的写入型脚本传 `--execute`。长期授权例外包括：已批准普通活动在报名截止前新出现的可报差额、限时折扣价格漂移修复、登记中的人工特殊折扣恢复、新链接/新上架 7 天/重新上架无活动/漏限时折扣兜底，以及满足严格 7 日指标的高点击低转化专属折扣。上述例外由现有云端 repair 为主执行，不逐次索要人工确认，但每轮必须自动计算并校验精确 payload/work hash，同时通过授权 ID/上下文、身份、价格栈、库存/平台校验、dry-run 和执行后 live 回读。未批准的新普通活动、优惠券取消、补预算和无证据写入仍不得自动执行。
...
- 真实写入动作必须单一、确定、可回读且属于当前长期授权白名单；已批准普通活动截止前新增差额及其报名库存事务可自动执行，未批准的新普通活动仍不在此列。优惠券取消和补预算继续禁止自动执行。
...
| 高点击低转化专属折扣 | 当前在售、销量明确为 `0`，且满足曝光 `>3000` + 点击率 `>4%`，或曝光 `>=3000` + 加车访客 `>=20` | 最新 linksData、批准价、成本、特殊价保护登记 | Top5 利润率再降 2 个百分点，默认库存 10、周期 7 天 | 两个入口均已获长期自动执行授权；执行前重读指标；若命中 ET 低库存畅销品则让位于价格档位收回 | 精确计划/work hash、保护登记、活动 ID、逐条 live 回读、滚动 7 日曝光/加车/点击率/销量方向性效果 |
| 报名最低库存事务 | 已获授权的普通活动或限时折扣报名，live/query/dry-run 可用库存低于平台最低值 | 活动 live/query_goods/dry-run、实时库存三字段、报名状态 | 锁内临时补到精确最低可用库存，提交/readback 后 finally 恢复原可用库存 | 不授权永久增库存、优惠券补库存或脱离报名的库存修改；恢复或恢复后验收失败即 fail closed | 补量前后库存快照、锁/幂等键、活动报名回读、恢复结果 |
| ET 低库存畅销品价格收回 | 新报名/补报/重建，ET 当日可售 `<=10` 且标准货号跨 19 店 30 天有效销量 `>30` | ET 当日匹配库存、跨店有效销量、全局 7 天曝光 Top5、批准基准 | Top5 回普通链接批准价，普通链接目标利润率加 5 个百分点 | 不改正常旧活动，不覆盖人工特殊价，不污染基准；平台允许价与安全线优先 | 标准货号销量/库存证据、原档/收回档、平台上限、用户审核项 |
...
- 高点击低转化自动实验使用两个并行入口，均要求当前在售、`c7_sale_cnt = 0`：点击率入口为 `c7_eps_uv > 3000`、`c7_goods_uv / c7_eps_uv > 4%`；加车访客入口为 `c7_eps_uv >= 3000`、`c7_cart_uv >= 20`。guard 生成 `highClickLowConversionSpecial` 审计区及精确计划；加车入口首批已于 2026-07-29 经用户确认并开启长期自动执行。worker 在登记前重读最新链接指标，防止 stale guard 给已经出单/跌出阈值的链接继续降价；平台活动库存下限高于计划量时使用报名库存事务，不永久改量。
...
- 高点击专属价按同标准货号最新已批准全局曝光 Top5 商品成本利润率再降 2 个百分点，最低利润率 15%，活动库存 10、周期 7 天。执行顺序固定为“价格优先级预判 -> 写保护登记 -> dry-run/报名库存事务/事务替换/live readback -> 恢复原库存 -> 回写 activityId”；若命中 ET 低库存畅销品，自动高点击降价不执行。不可覆盖的 plan 副本保留报名基线，供后续效果日报读取。
...
- 所有普通活动/限时折扣报名的库存不足统一使用 2026-08-02 报名库存事务：只按 live/query/dry-run 最低值，在 `store + SKC/SKU` 锁内记录三字段快照，临时补到精确最低可用库存；提交/readback 后在 `finally` 恢复原可用库存，再回读库存与报名状态。人工特殊活动的精确登记价不受 ET 低库存价格收回自动覆盖。
- 临时覆盖总量必须按“平台要求可用库存 + 恢复时实时锁定/不可用量”计算，实际覆盖量进入幂等键。恢复失败、活动恢复后失效或无法精确恢复原可用量均 fail closed。该规则明确取代旧 ET 足够才补、ET 不足阻断以及普通活动库存不授权的边界；但不授权永久增加、优惠券补量或任何脱离报名事务的库存修改。
...
- 目标价漂移、新链接/漏兜底与普通活动报名共用同一报名库存事务；dry-run 返回活动库存不足时逐键临时补量、提交、恢复和双回读。不得因为漂移链路先遇到混合旧活动就跳过库存事务，也不得把临时覆盖留成长期库存。
...
2026-07-17 已完成在售链接漏兜底的系统性收口：多 SKC 店铺批次已改为逐 SKC 执行 ET 当日实盘门控和有界库存写入重试，单条阻断只从执行子集排除，不再拖死整店。新链接/重新上架/漏兜底计划器对普通兜底采用安全价下限判定：现价不低于目标价不自动降价，低于目标价仍进修复；人工特殊价保持精确匹配。同一份最终 19 店 live 快照修复前仍误报 DX 1 条，修复后 `actionable=0 / blocked=0`。
