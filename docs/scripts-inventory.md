...
- BI 仓库/门户：
  ...
  - `prewarm_bi_portal_sections.sh`：云端 Portal section 预热脚本；默认先预热首页销售/订单/售后关键 section，再把较慢的 `profit` 放到后段并补跑 `homeProfit`。前端会拒绝 `staleSource=true` 或 `sourceGeneratedAt` 不匹配的旧利润摘要。
  - `serve_bi_portal.ps1`
  - `open_bi_portal.ps1`
