...
- 销售/日报/看板：
  ...
  - `fetch_shein_sales.mjs`
    ...
    - `fetch_shein_sales.mjs` 支持 `--transport browser|webapi|auto`、`--session-dir`、`--refresh-session`；当前用于最终日 WebAPI 独立核对、尚未 API 化的数据域和人工灾备，不再承担半托当天销售的定时全店轮询。
...
- `lib/shein_webhook_repository.mjs` / `infra/warehouse/migrations/20260719_001_shein_webhook_runtime.sql` / `infra/warehouse/migrations/20260721_001_shein_webhook_product_context.sql` / `infra/warehouse/migrations/20260723_001_webhook_primary_sales.sql` / `infra/warehouse/migrations/20260727_002_shein_webhook_product_state.sql` / `infra/warehouse/migrations/20260727_003_shein_webhook_product_state_four_states.sql`：只存密文的幂等 receipt/queue、安全前端投影、下架站点事件合并、受限商品/销售上下文函数、租约重试、按单 OpenAPI 入仓、可回滚的正式销售晋升、授权/额度店铺闸门，以及“Webhook 触发 + OpenAPI 当前态/生命周期证据回读”的店铺+SKC四态实时覆盖层。
...
- `lib/marketing_high_click_special_policy.mjs`：高点击低转化候选、Top5 利润率下调、保护登记冲突、执行前重判和效果跟踪的共享规则。

- `lib/marketing_shared_storage_cost.mjs`：为零销量但仍有 ET 实盘和仓储费的标准货号计算共享仓储费/件，供营销审核与 stack review 共同使用。
...
- `scripts/inventory/build_daily_inventory_replenishment_plan.mjs`：19 店只读计划器。ET 事实读取独立 `inventoryTrend.json.cachedAt`，销量、曝光和链接四态读取 `linksData.json.cachedAt`，库存读取各店 OpenAPI 快照。四态优先于 OpenAPI 回退状态：待上架、已下架一律排除；已售罄链接若同店同标准货号已有其他已上架链接也排除。`ET > 10` 时，近 7 天开过单的合格链接在库存大于 10 时降到 10、小于 5 时补回 10、5-10 保持；未开单链接在平台库存不大于 20 时目标为 `min(100, ET)`。全店同款同时售罄时不再形成死锁：计划器先按店去重，再按曝光/访客/销量稳定排序，只选一个全局种子并固定目标 `10`；种子和持久锁证据进入 payload hash，锁种子不作为其它售罄链接的跨店在售证据。目标等于当前库存的 `set_exact` 归入稳定，不进入可执行写集。`ET <= 10` 时只在合格链接中按同标准货号全局近 7 天曝光 Top5 平均分配 ET 整数库存，余数给更高曝光，非 Top5 目标为 0。任一合格链接缺曝光、多 SKU 或证据不新鲜时整货号失败关闭。
...
- `scripts/inventory/execute_daily_inventory_replenishment_plan.mjs`：只执行当天、无全局 blocker、policy 版本一致且 hash 完整的精确计划。2026-08-03 起由受限云端上下文自动传入当天同一个 `payloadHash`，不再逐日人工确认；执行器同时支持增库存、降到 10、Top5 配额、非 Top5 清零和全店售罄单种子自举。每条执行前重查 linksData 四态、同店同款已上架链接、SKU、ET、近 7 天销量/曝光、种子整组排序/预算、持久锁 hash 和实时库存；新种子还会在标准货号级互斥锁内重抓 19 店商品目录身份指纹并逐条 OpenAPI 回读整组链接，发现新增/删除/SKU 漂移或其它店已恢复可售即失败关闭。自举库存写入前先原子预占 `pending` 锁，精确回读成功后升级为 `active`；两种状态都只允许重试同一种子，`pending` 且库存已到目标时会生成只回读激活动作，dry-run 不写锁，锁种子后续固定维护为 `10` 且不向其它售罄链接扩散。精确保留锁定/不可用量并回读可用库存等于批准目标；单条失败继续记录，最后统一汇总。首次建立 VI 库存行时先查询商家仓库列表，只有唯一仓或站点国家唯一匹配仓时才带 `warehouseCode` 提交，多仓歧义失败关闭。
- 回滚含自举规则的发布时，必须先停止并禁用每日库存守卫和 ET 低库存守卫、确认无在途执行，再切回不识别锁账本的旧代码；不得删除锁账本，也不得仅回滚源码后继续自动执行。
