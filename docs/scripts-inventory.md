...
- 营销活动半自动：
  ...
  - `marketing/verify_ordinary_activity_enrollment.mjs`：普通营销活动提交后的已报/审核中集合回读器，按 `selection-plan + price-overrides` 校验 `missingRows/priceMismatchRows/badPacketActivities/extraAvailableRows`。`extraAvailableRows` 来自执行页 `outOfPlanRows` 或 `totalGoods > expectedSelectedCount`，用于发现“计划内都报了但页面还有漏抓商品”的系统级漏报；补报后必须回读到 `extraAvailableRows=0`。只补少量店铺时优先传 `--stores` 和 `--fill-results-dir` 做最小范围复核，不要求每天全店开前端。
