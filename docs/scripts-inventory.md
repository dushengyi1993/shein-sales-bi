...
- `scripts/inventory/build_daily_inventory_replenishment_plan.mjs`：19 店只读计划器。ET 事实读取独立 `inventoryTrend.json.cachedAt`，销量、曝光和链接四态读取 `linksData.json.cachedAt`，库存读取各店 OpenAPI 快照。四态优先于 OpenAPI 回退状态：待上架、已下架一律排除；已售罄链接若同店同标准货号已有其他已上架链接也排除。`ET > 20` 时，近 7 天开过单的合格链接在库存大于 10 时降到 10、小于 5 时补回 10、5-10 保持；未开单链接在平台库存不大于 20 时目标为 `min(100, ET)`。`ET <= 20` 时只在合格链接中按同标准货号全局近 7 天曝光 Top5 平均分配 ET 整数库存，余数给更高曝光，非 Top5 目标为 0，合计不得超过 ET。任一合格链接缺曝光、多 SKU 或证据不新鲜时整货号失败关闭。
...
- `scripts/cleanup_local_shein_browser_profile_cache.mjs`：本机 SHEIN Profile 缓存治理。默认只预演，`--apply` 才清理；活动中的 Profile 永远跳过。只删除 Chrome 优化模型、Crashpad 和 Cache/Code Cache/GPU Cache 等可丢弃目录，明确保护 Cookies、Login Data、Local Storage、Session Storage、IndexedDB。配合 `launch_store_browser.mjs` 的 100MB 外置缓存，避免19店重复下载多GB模型。
...
- `scripts/inventory/build_et_low_inventory_safety_plan.mjs` / `scripts/cloud_et_low_inventory_guard.sh`：每次 ET 正式同步成功后，从完整计划中只保留 `ET<=20` 且目标低于当前库存的动作，生成独立 64 位 hash，并在 `cloud_et_low_inventory_guard` 常驻授权上下文中只减不增、逐条回读。`ET=0` 清零；`ET=1-20` 执行全局曝光 Top5 配额和非 Top5 清零。当前库存低于配额时安全跳过，绝不由快速守卫补库存；写前直读已变成不可售的链接记为安全跳过，不再让无库存意义的旧链接导致高频守卫反复失败。
