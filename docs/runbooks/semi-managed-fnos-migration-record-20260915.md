...
+### 迁移后第 12 批：营销基线换代 + 启用店铺子集的单一决策点（2026-09-17 01:45–01:50 CST，正式版本 `2026.09.16.12`，部署 `deploy13-20260917`）

- 目标：`current.json` 从 07-29 的 19 店 / 750 行老基线，换成本轮 before16 的 **758 行 / 20 店**（含 LG；HY 平台无报名入口，属事实）。第 4 次尝试才成功，前 3 次都死在同一个缺陷上。
- 根因（设计问题，不是数据问题）：「启用店铺子集」是**每个覆盖校验各自持有的参数**，必须由调用点手工转发（计划对校验 → 候选回读 → 终态回读 → promoter 预检）。漏一处就只在**最后一道校验**失败，所以每次发版都往前挪一格：939 → 729 → 429 → 1049 行。
- 结构性修法（PR #200，commit `59fac9bc84`）：
  1. 新增 `resolveStoreCoverageExpectation()`：一处判定「全量精确」还是「显式允许的子集」，并**始终返回计划自身的店铺集合**作为契约。
  2. `publishMarketingPlanRegistry` 在首道计划对校验后归一化 `const expectedStores = validated.storeKeys`；候选回读 / 终态回读 / 不可变副本此后一律**严格比较**，不再需要任何开关转发。
  3. `verifyMarketingPlanRegistrySync` 从注册表**自己声明的** `pair.storeKeys` 解析期望，开关只在解析处消费一次；对外 `storeCoverageComplete` / `missingEnabledStoreKeys` 语义不变（guard/repair 的 19/21 提醒照旧）。
  4. 顺手修掉同类漏转发：CLI `verify` 子命令解析了 `--allow-enabled-store-subset` 却没用；CLI 的启用店铺期望**默认取本仓库 `config/stores.json`**（从前不给 `--stores-config` 就等于不校验，却还打印 green 的覆盖行）。
- 部署踩坑（下一次务必照做）：
  1. **发布证明要在部署前装好**。这批的 GitHub Release tag 是 `untagged-<sha>`（armed-draft 发布形态），所以 `gh release view <version>` 与 `gh release download <version>` 都会报 `release not found` —— 要用 `gh release list --json name,tagName` 拿到 tagName（或用 asset id）下载。两份资产（`release-attestation.json` + `.sha256`）先 `install -o root -g sheinops -m 0644` 到 `/srv/shein-bi/runtime/release-attestations/<version>/`，再跑部署脚本。这次是在 marker 步才 `ENOENT`，此时已经 `pause` + `git reset --hard` 过，生产停在 maintenance=all 且 checkout 已是新 commit。
  2. **续跑脚本必须保留原脚本第 1–13 行**（变量 + `log()`/`fail()` + `J()` + `cd "$ROOT"`）。少 `cd`/`J()` 会在相对路径上报 `MODULE_NOT_FOUND scripts/check_release_source_state.mjs`（错误信息里只有被截断的 `/home/dushengyi/scripts/...`）。`BSHA` 也要在续跑脚本里按已装的 bundle 重新算一遍。
  3. `/srv/shein-bi/runtime/source-release-bundles/` 普通用户不可穿越 → 续跑脚本必须 `sudo` 跑，否则 `test -f` 失败会误报「bundle missing」；另外以 root 重定向写一个 dushengyi 拥有的 `/tmp` 文件可能 `EACCES`，换个新的 root 文件名即可。
  4. rotation 的 `rotation-{stage,finalize}-preflight.json` 已存在会 `EEXIST` → 在 `log "stage"` 前插 `rm -f` 两件。上一次失败留下的 `baselines/composite-before16-8b28be2f/` 也要在 promote 前删掉。
- 读回：`baselineId=composite-before16-8b28be2f`、`rowCount=758`、`storeKeys=20`、`promotedAt=2026-09-16T17:48:48.772Z`、`canonicalRegistryHash=9c424986a197ecbc1a8c8f00b540b36c43c043666daf625d08126fe6846d909a`；workFingerprint `8b28be2f…` 与锁定件一致；readback `ok=true planned/checked=758 missing=0 priceMismatch=0`；库存 generation **117**、`assert aligned: true`、source fingerprint `dc50bddd…`、维护已 resume（generation 386→…）。
- 遗留（同批发现，未在本批修）：`shein-bi-et-low-inventory-guard.service` 与 `shein-bi-et-low-inventory-recheck.service` 仍 failed。真实原因见执行器 journal（`/srv/shein-bi/runtime/et-low-inventory-guard/results/*.json.journal.ndjson`）：唯一那条降库存动作（`XL` / `sv260102130015778541580` / `KJ-102S三明治机和早餐机`）被围栏拒绝，`error=INVENTORY_WRITE_FENCE_DOMAIN_INVALID:INVENTORY_JOURNAL_CROSS_REFERENCE_INVALID:duplicate resolution`。该 intent 是 **2026-09-03** 的历史意图（`bad0e90e-f211-4dbc-b9bd-968b047c23dd`），既被列为 `deferredHistorical`（计划认为未结），又在 journal 里已有终态/人工决议（`lib/durable_inventory_write.mjs:1079` 因此判重复）。两套簿记不一致导致每小时重复尝试、每小时 fail-closed；需要修生产者侧（已结的 intent 不应再进 `deferredHistorical`、也不应再发跨 journal 决议），不是放宽围栏。
### 迁移后第 13 批：库存跨库重复终结让每日库存永久 fail closed（2026-09-17 07:10–07:39，正式版本 `2026.09.16.14`）

- 现象：当日 morning-chain 在库存阶段永久 fail closed。`shein-bi-cloud-morning-chain.service` 07:10:04 起跑、07:39:31 结束、ExecMainStatus=78；pipeline-markers/2026-09-17/ 只有 morning-links-ready / morning-supplements / inventory-started 与 morning-all(failed)，没有 daily-operating-refresh.json。子链日志三次同一未捕获异常：INVENTORY_JOURNAL_CROSS_REFERENCE_INVALID:duplicate resolution（lib/durable_inventory_write.mjs 的跨库终结校验）。当日 run 目录 daily-inventory-replenishment/runs/2026-09-17/<hash>/ 的 plans/results/detail-targets 全空——plan 根本没写出来，当天零库存写入。
- 根因（一个 intent 被两个 journal 各自跨库终结）：intent bad0e90e-f211-4dbc-b9bd-968b047c23dd（XL / sv260102130015778541580 / I9mjweo3tyo2c3，runDate 2026-09-03，源 journal daily-inventory-replenishment/results/daily-inventory-replenishment-2026-09-03.json.journal.ndjson）被两处 cross_journal_resolution 关闭：① runs/2026-09-15/<hash>/results/...journal.ndjson 第 85 行（newRunDate 2026-09-15），② ET 低库存 guard 的 et-low-inventory-guard/results/et-low-inventory-et-daily-2026-09-16-…json.journal.ndjson 第 3 行（newRunDate 2026-09-16）。两行的 sourceSha256 / intentHash / logicalActionKey / disposition 完全相同，只有 newRunDate 与 recordedAt 不同。
- 为什么会重复写：ET 低库存 guard 的 unit 把 SHEIN_BI_INVENTORY_JOURNAL_DIRS 写成 daily-inventory-replenishment/results:et-low-inventory-guard/results——**不含 runs 树**，于是它的计划视图看不到 09-15 写在 run 目录里的终结，仍把该 intent 当 pending 又终结一次（executor 的 stale-pending supersede 分支）。morning 链自己的 executor 因为当前 journal 就在 runs 树下，能看到，所以没重复。
- 修法：
  1. **journal 域收敛成一个来源**：lib/inventory_journal_discovery.mjs 新增 DEFAULT_INVENTORY_JOURNAL_DOMAIN_DIRECTORIES（含 runs 树）与 inventoryJournalDomainDirectories()；lib/shein_openapi_client.mjs 的同名方法、guard 脚本、executor 全部改用它，域不再靠 unit env 手工枚举。
  2. **写入侧**：appendInventoryReconciliationRecord 追加前用聚合域扫描（findInventoryIntentClosures）该 sourceJournal+intentId 是否已被任一 journal 终结；已终结就幂等跳过并返回证据（closures[].matchesProposedClosure），不写第二条。executor 在 supersededHistorical 里标 supersedeRecord: already_terminal_in_journal_domain 保留证据。
  3. **读取侧**：readInventoryIntentJournals 把**同一闭包**（身份只忽略 recordedAt 与 newRunDate）的重复视为幂等无操作并记入 duplicateCrossJournalResolutions；两行内容不一致仍然 fail closed。这一条让已经写进生产的两行不再永久堵死后续每一天。
  4. **受控审计工具** scripts/inventory/audit_duplicate_cross_journal_resolutions.mjs：只读，输出每份 journal 的 sha256/行数、重复的两条原始行及其行哈希、该 intent 的终态次数，journalLinesModified: false。**不改写、不删除任何 ndjson。**
...
### 迁移后第 14 批：淘汰链接审核的抓数口径在迁移后失效（2026-09-17，正式版本 `2026.09.16.16`）

- 现象：淘汰链接审核的正式入口在现网跑不通——`scripts/link_retire_review.mjs` 写死 `cloudHost='shein-bi-tencent'` 并拒绝任何其它 `--cloud-ssh`，采集/认领/回读/投递四处都 ssh 到已停用的机器；`collect_link_retire_review_evidence.mjs` 的 `COLLECTOR_HOST`、lib/link_retire_review_evidence.mjs 的 `EVIDENCE_HOST`、lib/cloud_team_report_common.mjs 的 `CLOUD_TEAM_REPORT_CLOUD_HOST` 同样写死。结果只能退化成 linksData 快照 + 离线策略器，拿到的是上限而不是事实。
- 修法：新增 `lib/production_cloud_host.mjs` 作为生产云主机的唯一来源（显式 --cloud-ssh > 环境变量 SHEIN_BI_PRODUCTION_CLOUD_HOST / CLOUD_TEAM_REPORT_SSH_HOST > 默认 shein-bi-fnos，带主机名安全校验），上述四处全部改为从它解析；collector 额外记录 `host`（调用方注入的 ssh 身份）与 `collectorHostname / deployedRelease`（本机自证），manifest 的 host 绑定随之指向解析结果。
- 附带缺陷（数据，本次不可修）：新宿主 `/data/shein-bi/outputs/shein_links/<store>/` 只有 2026-09-14/15/16 三天（`shein_links_raw` 同），保护期要 09-02…09-14；`fact.link_master_snapshot` 只有 09-16（2990 行）+1 条 08-18，等于不保留按日状态历史；`fact.visible_inventory_snapshot` 有 2025-10-25…09-16（只缺 09-05）；`fact.link_performance_daily` 有 04-30…09-16。旧主机 `shein-bi-tencent` 已不可解析，`SHEIN_BI_BACKUP_OFFSITE_ENABLED=0` 且 `/lhcos-data` 未挂载（无异地副本），本地 `/srv/shein-bi/backups/auto/` 只有 20260915 之后 4 份。结论：**09-14 之前的历史原始工件在本机不可恢复**，只能从 09-14 起逐日累积，约 09-29 起 15 天窗口才完整。
- 保留窗口：确认 `scripts/cloud_disk_maintenance.sh` 的 `OUTPUT_RETENTION_DAYS` 默认 30 天（≥建议值），且三处安全兜底（offsite 关闭 / COS 未挂载 / dry-run）都保留文件，只有成功归档后才删；新增契约测试把 30 天保留、15 天保护窗口和这些兜底钉死。
