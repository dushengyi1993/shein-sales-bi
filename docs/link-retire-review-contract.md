...
本入口在云端以 shein-3 + 审核日原子认领一次发送机会。该日已有任意交付工件，或已有成功、失败、部分、未知发送认领时，均禁止再次发送，包括换目录或重新生成不同 XLSX 字节。保留原认领、receipt/readback 并只读核验，未知结果不清除认领、不自动补发；需要修正时先明确核验已有结果，再由人工处理。自动化不得绕开此入口改用通用发送器创建第二份当日报告。
