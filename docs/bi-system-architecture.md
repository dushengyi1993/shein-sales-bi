...
- `2026-05-05` 早晨正式自动日志失败后已手动恢复；最新手动验证日志 `bi-daily-pipeline-20260505-111437.log` 为 `success`，下一次正式自动验证为 `2026-05-06 06:40`。
- `2026-05-02 06:40` 的 `267014` 是已修复的历史失败记录，保留作排障证据。
- 链接表现每日任务为 `SHEIN-Sales-15Stores-LinkManagement-0530`，每天 `05:30`，用于抓前一完整业务日链接数据，写本地 / PostgreSQL / BI，不再写飞书链接表。
...
## 当前入仓状态（2026-05-05 截面）

当前 BI 仓库已经按 15 店写入 `2026-05-05` 销售/业务截面，链接表现为前一完整业务日 `2026-05-04`：
...
- `2026-05-05` Docker / WSL 文件系统异常已手动恢复；系统状态页会保留“已修复，待下次正式自动验证”的提醒，直到下一次正式自动任务跑通。
