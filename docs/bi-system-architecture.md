...
- 半托与全托共用主机的当前排班、分阶段资源令牌、单一晨间 coordinator 和 Portal 单队列见 [shared-host-resource-schedule.md](shared-host-resource-schedule.md)。
- 晨间链路每天 `07:10` 只启动一个 coordinator；逐店/逐阶段 checkpoint 可断点续跑，21 店完整后只原子发布一次，普通页面不能并发生成重 section。
- 云端 `shein-bi-cloud-yesterday.timer`：每天 `02:45`，在 session/backup marker 通过后收齐前一天21店 OpenAPI 完整日切片并复核稳定日。
