...
5. **现有生产链路不冒险迁移**
   - Windows 计划任务、SHEIN Windows Chrome 登录态、飞书同步和日报继续运行。
   - 新系统双写验证稳定后，再逐步替换。
...
1. **飞书生产链路**
   - 继续负责销售同步、Base 协同、正式看板和飞书日报。
   - 在 BI 系统稳定前不得中断、替换或降级。
...
3. **本地 BI 经营门户**
   ...
   - 当前服务默认只绑定 `127.0.0.1`，未开放局域网/公网，未改防火墙。
   - 通过本机服务打开时，动作状态写入 `state/bi_action_state.json`；直接双击 HTML 打开时，动作状态保存在浏览器本地。

当前团队访问状态：

- 已具备：本机门户、服务端动作状态文件、深链接、动作清单复制、CSV 导出、系统巡检页。
- 未完成：正式账号/权限、固定局域网或服务器地址、多人编辑冲突控制、动作状态入库、HTTPS、备份和权限分级。

当前自动任务状态：

- 任务名：`SHEIN-BI-Daily-Pipeline-0640`
- 时间：每天 `06:40`
- 入口：`wscript.exe` + `scripts/run_scheduled_hidden.vbs` + `scripts/scheduled_bi_daily_pipeline.ps1`
- `2026-05-03 06:40` 正式运行已成功，Windows Last Result 为 `0`。
- `2026-05-02 06:40` 的 `267014` 是已修复的历史失败记录，保留作排障证据。
- 链接表现每日任务为 `SHEIN-Sales-15Stores-LinkManagement-0530`，每天 `05:30`，用于抓前一完整业务日链接数据、同步飞书链接底表并刷新 BI。
...
## 当前入仓状态（2026-05-02 截面）

当前 BI 仓库已经按 15 店写入 `2026-05-01` 经营截面：

- 销售日期：`2026-05-01`
- 业务日期：`2026-05-01`
- 链接日期：`2026-05-01`
- 店铺覆盖：`CX DL DX FY HL JY LQ MZ NM QH QY TS XL YJ ZL`
- 已覆盖业务域：销售订单、订单商品、链接主数据、链接表现、货号店铺覆盖、链接建议、售后/退货、发货面单、正确展示库存、商品质量、商品评价、营销活动、首页财务摘要、gsfs 财务收入概览、gsfs 在途收入订单。

当前已知限制：

- gsfs 财务明细目前只覆盖 HL `1/15` 店，其它店需要后续逐步切换主账号或补齐财务权限。
- 趋势模块目前只积累单日数据，门户只做趋势准备度提示，不做假环比或假 7 日趋势。
- 正式 BI 自动任务 `2026-05-03 06:40` 已恢复成功，上一轮结果为 `0`；`267014` 仅作为历史失败记录保留。
