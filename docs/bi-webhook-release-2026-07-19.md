...
> 2026-07-22 补充：商品审核失败回调即使不携带失败原因，也会先用 SPU + version 只读查询审核状态，再按精确 SKC 查询议价单，补齐货号、失败原因、我方申报价、平台建议价和剩余议价次数；不再只发一个 SKC 让运营自行进后台查。
...
- 商品下架通知不再只显示 SKC。Webhook worker 与 BI 只读接口会通过受限数据库函数补齐货号、商品名称、首次上架时间、近 7 天/30 天/累计成交件数与成交额、最近成交日期；一个货号只保留一个业务主体，不额外展示款式字段。
...
- 平台一次下架可能在一秒内按多个子站重复推送。receipt 继续逐条保留作审计，但 BI 时间线、摘要 KPI 和飞书告警只对“同店铺 + 同 SKC + 同动作”的回调静默去重；不同 SKC 必须分别展示和告警。用户文案不展示底层回调条数，避免被误解为商品链接数量。
