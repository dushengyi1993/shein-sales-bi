...
> 当前权威状态：2026-05-03。本文只保留接手和日常运维需要的信息；历史排障过程见 `.codex/plans/2026-05-01T14-11-55-shein-link-management-system.md`。
...
| 05:30 | `SHEIN-Sales-15Stores-LinkManagement-0530` | 前一完整业务日链接表现抓取，写飞书链接底表，并刷新 BI。旧 `0340` / `0510` 任务不要恢复。 |
| 06:40 | `SHEIN-BI-Daily-Pipeline-0640` | 刷新 PostgreSQL BI 仓库、BI 体检、本地门户和 Markdown 晨报。`2026-05-03 06:40` 已正式成功，Last Result = `0`。 |
...
- 新 `05:30` 链接任务下一次/首次正式自动验证是 `2026-05-04 05:30`。
...
- 执行内容：抓前一完整业务日 15 店链接数据、同步飞书链接底表、刷新 BI。
...
- `fact.product_comment` 已按开店以来全量评价入库：当前 `1757` 条，`1755` 条有 SHEIN 平台译文，`translation_provider='shein-platform'`；日常新增评价由业务域同步直接写入平台译文，不再运行旧的本地翻译脚本。
...
## 15. 2026-05-03 评价全量与平台翻译口径（当前权威）
...
- 当前仓库核验结果：`fact.product_comment` 共 `1757` 条，覆盖 15 店，最早评价日期 `2025-10-04`、最新评价日期 `2026-05-02`；`1755` 条有 SHEIN 平台译文，剩余 2 条为原文为空，无需翻译。
