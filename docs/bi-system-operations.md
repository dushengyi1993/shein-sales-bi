...
- BI Portal API section 会在 `outputs/bi-portal/sections/` 缓存；首页首屏优先加载轻量 `homeRankings` / `homeProfit`，完整 `rankings` / `profit` 只供服务端和完整利润域使用。货号筛选改走不落盘的请求态 `productProfit?q=<标准货号>`：服务端在当前同代 `profit` 缓存内筛选并按账号店铺权限返回小结果，浏览器不再下载整份利润缓存。两个首页 section 都读取最后一次完整发布的 cache，再由 `liveSalesToday` 覆盖当天新增、取消、售后和利润变化；不得为了生成首页卡片同步等待整套移动成本重算。首页轻量 section 使用独立串行 lane，不能被 `linksData` / `productSalesDaily` 等慢 section 堵住。订单 Webhook 只刷新 `liveSalesToday`，当前日订单列表、排行和成交价散点均在浏览器用同一份正式事实覆盖，不为每个在线浏览器重复重建大缓存；同一个 live token 在服务端也只能完成一次强制生成。core 换代且 section 尚未生成时才显示缓存兜底提示；同代实时刷新不弹顶部大黄条，单次后台失败会自动重试，连续重试耗尽后才显示人话错误。`inventoryTrend` 是展示库存趋势 section，来自 `fact.visible_inventory_snapshot`，用于“前台展示库存每日快照”趋势；它不同于 ET 货代仓实盘可售，也不同于成本表供给。`cloud_bi_refresh.sh` 会启动 section 预热脚本；`serve_bi_portal.mjs` 还会用 core `generatedAt` watcher 在服务启动和首页访问时兜底预热，避免新 core 后用户首开页面才生成慢 section。长期开着的网页不再每 60 秒轮询：订单 Webhook 入仓后由 PostgreSQL `NOTIFY` + SSE 立即推送当天销售；可见页面只保留 5 分钟低频兜底检查，用于弥补断线或漏掉的 core 通知。
- 当天销售和利润分两阶段但不混造数据：新订单，或已有订单行的金额/数量变化，先直接从 `fact.order_item` 进入实时销售；只要事实行与利润 cache 的金额/数量不一致，就返回 `accountingPending=true`，不能继续展示旧净营收。Portal 将订单/退货事件合并 45 秒后重建移动加权成本台账与利润 cache，再推送一次利润更新。服务重启会补一次未处理事件，失败会在 5 分钟后自动重试。利润补账完成前页面明确显示“新订单已计入销售；利润正自动补成本”，不得拿静态均价或旧 cache 伪造当前利润。`homeProfit` 仍只从同一代 `profit` section cache 派生；若页面首页利润异常偏低，先核对 `homeProfitSummary.sourceGeneratedAt` 与当前 `data.json.__sections.generatedAt` 是否一致，并确认 `staleSource=false`。
...
- 商品成本从首个可信 ET 实盘切点起来自移动加权台账：期初种子只能取生效日前一日 ET 结存，后续按入库、销售、已证实的 RTV 09 回流等事件推进。`mart.product_cost_batch_timeline` 统一成本表和 ET 物理追踪：未关联 ET 的批次可用成本表发货日；已关联 ET 后只认实际发出/离仓节点，最保守可用已到仓时间反推“至少已发”，成本表日期不能覆盖 ET 仍待发状态；建单/审核时间不算发货。切点前因缺少批次消耗证据，保留明确标注的 `legacy_pre_cutover_estimate`：先按订单日期前已到仓批次累计加权；当时没有到仓记录时，只能使用订单日前已实际发出的批次。两类当时证据都没有就显示待成本，不再拿未来批次倒灌。切点后成本库存不足时，优先使用订单当时已有的在途批次成本，否则使用最近移动加权成本，并记录 `cost_estimated_quantity`；完全没有成本证据才暂时缺失。未冻结期间后续真实入库会按 FIFO 结算负库存：把实际成本与原估算的差额回补到原销售成本，同时从剩余库存价值中转出，防止利润和库存双重失真；已冻结期间绝不回写。任何估算都不声称精确批次。
...
- 成本估算订单仍参与利润，但页面分别显示“库存缺口估算”和“历史成本估算”；完全缺成本的订单行不参与真实利润额计算，并显示缺成本数量。利润率分子只允许除以同一批有成本依据的净营收。
...
- 当前团队入口为云端 `https://sa.dushengyi.cc/`，旧 IP `http://43.165.167.135/` 仅作兜底，通过 BI 应用内登录和账号权限限制访问。
...
- 检查云端 BI 门户：打开 [https://sa.dushengyi.cc/#tab=system](https://sa.dushengyi.cc/#tab=system)。
- 检查云端健康：未登录公网访问 `https://sa.dushengyi.cc/api/health` 应返回 `401` 或跳转登录；服务器本机或带有效 BI 登录会话访问应返回 `200`。
- 检查首页利润缓存：带有效 BI 登录会话访问 `https://sa.dushengyi.cc/api/bi/section/homeProfit`，确认 `data.homeProfitSummary.staleSource=false` 且 `sourceGeneratedAt` 等于当前 `data.json.__sections.generatedAt`；服务器侧可读 `/opt/shein-bi/app/outputs/bi-portal/sections/{profit,homeProfit}.json` 做同样核对。
